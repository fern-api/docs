...
<ParamField path="settings.type-dates-as-strings" type="boolean" default="false" toc={true}>
  Controls how `string` schemas with `format: date` or `format: date-time` are typed. When `false`, they become Fern's dedicated `date` and `datetime` types. When `true`, they stay `string` with the format kept as validation, so the API Reference renders them as `string` with a `format: "date"` or `format: "date-time"` label and SDKs type them as strings.
</ParamField>
