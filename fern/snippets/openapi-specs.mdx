...
<ParamField path="overrides" type="string | list of strings" toc={true}>
  Path to an OpenAPI [overrides](/learn/api-definitions/overrides) file, or a list of paths to multiple override files applied sequentially. Consider using `overlays` instead for a standards-based approach.
