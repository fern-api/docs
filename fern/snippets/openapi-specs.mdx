...
<ParamField path="settings.respect-optional-request-body" type="boolean" default="false" toc={true}>
  Whether to import a request body with `required: false` as an [optional request body](/learn/api-definitions/ferndef/endpoints/overview#request-body). When enabled, the generated SDK method takes the body as an optional argument, and snippets, cURL, and the API Explorer omit the body when it isn't provided. When disabled, the body is imported as required.
</ParamField>
