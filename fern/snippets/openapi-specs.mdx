...
<ParamField path="settings.ignore-tags" type="boolean" default="false" toc={true}>
  When enabled, operation-level OpenAPI `tags` are ignored when determining SDK structure. Endpoints fall back to the root package (or their explicit `namespace`), and method names are derived from each operation's `operationId`. Explicit [`x-fern-sdk-group-name` and `x-fern-sdk-method-name`](/learn/api-definitions/openapi/extensions/method-names) overrides still take priority.

  Use this to derive a clean SDK surface from a standards-compliant spec whose tags would otherwise produce awkward sub-clients, without editing the spec or maintaining overlays.
</ParamField>
