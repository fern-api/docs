...
<ParamField path="settings.coerce-enums-to-literals" type="boolean" default="false" toc={true}>
  Controls whether enums are converted to literal types during code generation. When set to `false`, enums are preserved as enums rather than being converted to literal types. When set to `true`, single-value enums are converted to literal types. The default changed from `true` to `false` in CLI 2.0.0-rc0.
</ParamField>
