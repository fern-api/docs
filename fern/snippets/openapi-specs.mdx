...
<ParamField path="settings" type="object" toc={true}>
  OpenAPI-specific generation settings for this individual spec. To apply the same settings across all OpenAPI specs, use global [`api.settings`](/learn/sdks/reference/generators-yml#settings) instead. Use [`error-responses`](/learn/sdks/reference/generators-yml#settingserror-responses) to give 4xx/5xx responses a shared typed body during import.
</ParamField>
