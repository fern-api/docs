...
<ParamField path="settings.filter" type="object" toc={true}>
  Filter to apply to the OpenAPI specification. Use this to limit which endpoints are included in the generated SDK based on their paths.

  For tag-based filtering instead of path-based filtering, use [`audiences`](/learn/sdks/reference/generators-yml#groups) at the group level.
</ParamField>
