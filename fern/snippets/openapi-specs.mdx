...
<ParamField path="openapi" type="string | git source" required={true} toc={true}>
  Path to the OpenAPI specification file. Accepts either a local file path (string) or a [remote git reference](#git-remote-sources).
  ...
  When using a git source, the CLI shallow-clones the repository at the specified `ref` and resolves the `path` locally. Authentication uses your environment's native git credential resolution (credential helpers, SSH keys, `GIT_ASKPASS`). No tokens are required in YAML.
</ParamField>
