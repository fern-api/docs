...
<ParamField path="settings.filter.endpoints" type="list of strings" toc={true}>
  Endpoints to include in the generated SDK. Specify endpoints in the format `METHOD /path` (e.g., `POST /users`, `GET /users/{id}`). Only the listed endpoints will be included in the generated SDK; all other endpoints will be excluded. If your API uses [namespaces](#namespace), prefix with the namespace and `::` (e.g., `iam::POST /users`).
</ParamField>
