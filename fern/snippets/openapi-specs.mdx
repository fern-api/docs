...
<ParamField path="settings.any-of-sibling-properties-as-object" type="boolean" default="false" toc={true}>
  Converts a schema whose `anyOf` branches only mark sibling `properties` as `required` (an "at least one of" constraint) as a single object with optional fields instead of an undiscriminated union. Enable it when request bodies that set several of those properties drop all but one in generated SDKs. Off by default because it changes generated types. All the properties are generated as optional, so the SDK doesn't enforce that at least one is set; the API rejects a request that omits all of them.
</ParamField>
