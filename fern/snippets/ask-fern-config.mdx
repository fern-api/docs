...
<ParamField path="ai-search.location" type="list of strings" required={false} toc={true}>
  ...
  Most users should enable Ask Fern for both `docs` and either `slack` or `discord`. See the [Slack app documentation](/learn/ask-fern/features/ask-fern-slack-app) for Slack setup instructions. For Discord setup, reach out to support@buildwithfern.com.
</ParamField>
...
<ParamField path="ai-search.system-prompt" type="string" toc={true}>
  By default, Ask Fern uses [system prompts](https://github.com/fern-api/fern-platform/blob/main/packages/fern-docs/search-server/src/utils/system-prompt.ts) to finetune AI search results. Add a custom prompt here to override it. 
