...
All authentication schemes in Fern support environment variables through the `env` property. This allows your SDK users to configure authentication using environment variables instead of hard-coding credentials.

For example, to configure an API key to be read from an environment variable:
...
auth-schemes:
  ApiKeyAuth:
    header: X-API-Key
    type: string
    env: MY_API_KEY  # SDK will check for this environment variable
```

The SDK will automatically check for the specified environment variable before requiring explicit configuration:

```typescript
// Uses process.env.MY_API_KEY if available
...
This will generate an SDK where the user would have to provide 
mandatory arguments called `username` and `password`. 
