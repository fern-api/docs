...
<ParamField path="destination" type="string" required={true}>
  The path you want to route to. Can be an internal path (`/new-path`) or an external URL (`https://example.com`). External URLs must include the full address, including the scheme. Supported external schemes are `https://`, `http://`, `mailto:`, and `tel:`.
</ParamField>
