...
<Step title="Allowlist the Fern redirect URL">
The Fern Dashboard shows a read-only **Redirect URL** in the Authentication card once you select OAuth2. Copy it and add it to your OAuth provider's allowlist. If you use both a `.docs.buildwithfern.com` and a custom domain, allowlist both.
</Step>

<Step title="Configure OAuth in the Fern Dashboard">
Navigate to the [Fern Dashboard](https://dashboard.buildwithfern.com/), select your docs site, and open the **Settings** page. In the **Authentication** card, select **OAuth2** from the method selector, then fill in the following fields:

- **Client ID**
- **Client Secret**
- **Authorization URL** (e.g. `https://<your-oauth-tenant>/oauth2/authorize`)
- **Token URL** (e.g. `https://<your-oauth-tenant>/oauth2/token`)
- **Scopes** (optional, e.g. `openid profile email`)
- **Issuer URL** (optional, e.g. `https://<your-domain>`)
- **Logout URL** (optional, e.g. `https://<your-oauth-tenant>/oauth2/logout`)
- **Roles claim** (optional — the token claim Fern reads for user roles; defaults to `roles`)

<Note title="Specifying an audience">
If your client is connected to an API, you may need to specify an audience in the authentication request. Append `?audience=<your-api-identifier>` to the Authorization URL.
</Note>

Click **Save** to activate OAuth. Advanced options (public paths / allowlist, API key injection toggle) are available under an expandable section in the card.
</Step>
...
Some OAuth providers have strict requirements for custom claims. If you need to use a claim other than `roles`, set the **Roles claim** field in the Authentication card to the claim name your provider uses.
</Warning>
