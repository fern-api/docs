...
RBAC is evaluated per request, which distinguishes it from [audiences](/learn/api-definitions/openapi/extensions/audiences): audiences prune your API definition at publish time and produce one output for every reader, so gating endpoints by role requires `viewers` on the endpoints or API sections in your navigation.
