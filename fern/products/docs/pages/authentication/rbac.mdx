...
Use the `<If />` component to [conditionally render content](/learn/docs/writing-content/conditionally-rendered-content) based on user roles:
...
You can specify multiple roles. Content is visible to users who have **any** of the specified roles:
...
You can also combine `roles` with `products` and `versions` props. See the [conditionally rendered content](/learn/docs/writing-content/conditionally-rendered-content) page for full details on all available props, combining conditions, and inverting conditions.
