...
Fern's internal engineering site defines the following roles:
...
Unauthenticated users can access only the RBAC demo product; other roles unlock additional products. The demo page mixes content for different roles with the [`<If />` component](/learn/docs/writing-content/components/if): unauthenticated users see every section title but not the content beneath restricted ones.

<Frame caption="The `docs.yml` defines roles and gates products by viewer, then the [live site](https://engineering.ferndocs.com/rbac-demo/public-content/) shows the public view.">
  <video
    autoPlay
    muted
    loop
