...
The `<If>` component also supports `products` and `versions` props, which you can combine with `roles` to further narrow the audience. When multiple props are specified, **all** conditions must match:
