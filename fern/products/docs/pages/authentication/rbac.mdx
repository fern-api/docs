...
Then, set up the `fern_token` using either method:

- **[JWT setup](/learn/docs/authentication/setup/jwt)** — You handle the entire auth flow and set the cookie yourself, including a `fern.roles` array in the JWT payload.
- **[OAuth setup](/learn/docs/authentication/setup/oauth)** — Fern initiates an OAuth flow with your provider. Configure your token response to include user roles.
