...
Fern supports any SAML 2.0 or OIDC provider (Okta, Google Workspace, Auth0, Azure AD, OneLogin, etc.). Configure SSO directly from the [Fern Dashboard](https://dashboard.buildwithfern.com/).

<Steps>
<Step title="Open the Authentication card">
Navigate to the [Fern Dashboard](https://dashboard.buildwithfern.com/), select your docs site, and open the **Settings** page. In the **Authentication** card, select **SSO** from the method selector.
</Step>

<Step title="Enable SSO">
Click **Enable SSO**. Fern derives the domain from your admin email automatically.
</Step>

<Step title="Verify your domain and connect an identity provider">
The Authentication card shows a dual-capability panel:

- **Domain verification** — follow the instructions to add a DNS record that proves ownership of your domain.
- **Identity provider connection** — click **Open WorkOS Admin Portal** to connect your SAML 2.0 or OIDC provider. The portal walks you through provider-specific configuration.

The card displays live status for both domain verification and IdP connection. Sign-in is enabled once either is complete.
</Step>
</Steps>
