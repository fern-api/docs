...
By default, Fern reads each user's WorkOS organization role. To source roles from a different access token claim instead (for example, a directory-group claim synced from your identity provider), ask Fern to set `roles_claim` to that claim's name during setup. Fern then reads roles from that claim rather than the organization role.

The claim can be an array of role slugs or a space-delimited string (`"botanists growers"`). Given a `roles_claim` of `https://plantstore.com/groups`, this access token grants the user the `botanists` and `growers` roles:

```json
{
  "https://plantstore.com/groups": ["botanists", "growers"]
}
```

Fern matches the parsed slugs against the roles you declare in `docs.yml`, so the claim must emit those same slugs.
