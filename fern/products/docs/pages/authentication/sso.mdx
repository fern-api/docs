...
Fern reads the WorkOS organization role by default. To source roles from a different token claim instead (for example, a directory-group claim), set `roles_claim` on your SSO auth configuration. Fern then parses that claim from the WorkOS access token, accepting either an array of role slugs or a space-delimited string.
