...
<Steps>
<Step title="Configure JWT in the Fern Dashboard">
Navigate to the [Fern Dashboard](https://dashboard.buildwithfern.com/), select your docs site, and open the **Settings** page. In the **Authentication** card, select **JWT** from the method selector, then fill in the following fields:

- **Issuer URL** — the issuer identifier for your JWT tokens
- **Redirect URL** — your login page; users are redirected here after clicking **Login**
- **Secret** — the shared secret Fern uses to verify signed tokens

Optional fields:

- **Logout URL** — where users are sent after logging out
- **Return-to query param** — the query parameter name Fern appends to the redirect URL so your login page can send users back to the page they came from
- **API key injection toggle** — enable to allow the `fern_token` to carry API credentials for the [API Explorer](/learn/docs/api-references/api-explorer)
- **Public paths** — paths that remain accessible without authentication (allowlist)

Advanced options are available under an expandable section in the card. Click **Save** to activate JWT authentication.
</Step>
