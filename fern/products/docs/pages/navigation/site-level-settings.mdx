...
<ParamField path="redirects" type="list of objects, string, or list of strings" required={false} toc={true}>
  An array of paths you want to configure to permanently redirect to another path, or the filepath(s) of YAML files holding that array. Learn more about the
...
To keep a long list out of `docs.yml`, set `redirects` to the filepath of a YAML file that holds the list under its own `redirects` key, or to a list of such filepaths:
