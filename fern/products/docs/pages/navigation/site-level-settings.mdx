...
A: An instance is the backend of a distinct docs site, published to its own `docs.buildwithfern.com` URL and selected with `fern generate --docs --instance <url>` — typically one for staging and one for production. `custom-domain` is the vanity domain that fronts an instance.
</llms-only>
