...
<ParamField path="settings.disable-analytics" type="boolean" required={false} default="false" toc={true}>
  If set to true, disables the [built-in web analytics dashboard](/learn/dashboard/metrics/web-analytics) and all providers configured in the [`analytics` configuration](#analytics-configuration). Built-in analytics is enabled by default and requires no configuration.
</ParamField>
