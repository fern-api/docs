...
<ParamField path="settings.content-security-policy" type="object" required={false} toc={true}>
  Removes `'unsafe-inline'` and `'unsafe-eval'` from the [Content Security Policy](/learn/docs/security/content-security-policy) on sites with custom JavaScript, Google Tag Manager, or analytics providers.

  <Indent>
  <ParamField path="settings.content-security-policy.style-hashes" type="list of strings" required={false} toc={true}>
    Hashes of the inline `<style>` tags that scripts insert at runtime, such as `sha256-Xn86JLDplvyEYS5Le9qI7kfofm28Yt4KSLpywXweBXo=`. Setting this key, even to an empty list, removes `'unsafe-inline'` from `style-src` and blocks any `<style>` tag whose hash isn't listed.
  </ParamField>

  <ParamField path="settings.content-security-policy.allow-eval" type="boolean" required={false} toc={true}>
    Whether `script-src` includes `'unsafe-eval'`. Defaults to `true` on sites with custom JavaScript or Google Tag Manager. Set to `false` to remove it, which breaks GTM Custom JavaScript variables and any script that calls `eval()`.
  </ParamField>
  </Indent>
</ParamField>
