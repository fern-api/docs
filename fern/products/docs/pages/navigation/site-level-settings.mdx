...
<ParamField path="layout.mobile-toc" type="boolean" required={false} default="false" toc={true}>
  ...
  Only applies to pages using the `guide` and `overview` layouts. The bar is hidden when a page has fewer than two headings or when `hide-toc: true` is set in the [page frontmatter](/learn/docs/configuration/page-level-settings#table-of-contents).
</ParamField>
