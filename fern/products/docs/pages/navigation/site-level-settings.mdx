...
theme:
  ...
  site-switcher:
    enabled: true
    order: [/dynamo, /brev, /nemo]
    hide: [/internal-playground]
    labels:
      /holoscan/sdk-user-guide: Holoscan SDK
    show-products: true
```
...
<ParamField path="theme.site-switcher" type="object" required={false} toc={true}>
  Adds a **site switcher** to the header for [multi-source](/learn/docs/preview-publish/multi-source-docs#site-switcher) (fleet) docs sites: a dropdown listing the other independently published sites on the same host. Only rendered when `enabled: true` and at least two sites are visible. Distinct from the product switcher: entries are whole sites, not intra-site products.

  Set this in a [global theme](/learn/docs/customization/global-themes) so every site on the fleet picks it up automatically.
</ParamField>

<ParamField path="theme.site-switcher.enabled" type="boolean" required={false} default="false" toc={true}>
  When `true`, the site switcher is rendered in the header (and in the sidebar when `switcher-placement: sidebar`). Opt-in per fleet.
</ParamField>

<ParamField path="theme.site-switcher.order" type="list of strings" required={false} toc={true}>
  Basepaths to pin at the top of the switcher, in that order (e.g. `[/dynamo, /brev, /nemo]`). Sites not listed follow alphabetically by label.
</ParamField>

<ParamField path="theme.site-switcher.hide" type="list of strings" required={false} toc={true}>
  Basepaths to exclude from the switcher (e.g. `[/internal-playground]`). Hiding a site also hides its nested children.
</ParamField>

<ParamField path="theme.site-switcher.labels" type="map of string to string" required={false} toc={true}>
  Override the display label for specific basepaths (e.g. `/holoscan/sdk-user-guide: Holoscan SDK`). Falls back to the site's `title` from `docs.yml`, then a humanized version of the basepath leaf.
</ParamField>

<ParamField path="theme.site-switcher.show-products" type="boolean" required={false} default="false" toc={true}>
  When `true`, each site's top-level products are listed indented directly beneath it in the dropdown. Products link to `/{basepath}/{product-slug}/` (or the product link's href for external products).
</ParamField>
