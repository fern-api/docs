...
<ParamField path="settings.default-search-filters" type="boolean" required={false} default="false" toc={true}>
  ...
  Use [`settings.search`](#settingssearch) instead for finer control over [product-aware search](/learn/docs/customization/search#product-aware-search).
</ParamField>

<ParamField path="settings.search" type="object" required={false} toc={true}>
  Controls [product-aware search behavior](/learn/docs/customization/search#product-aware-search) for sites with multiple products or versions.
