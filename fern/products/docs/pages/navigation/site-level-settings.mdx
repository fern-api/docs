...
<ParamField path="settings.substitute-env-vars" type="boolean" required={false} default="false" toc={true}>
  ...
  A [self-hosted](/learn/docs/self-hosted/set-up#per-deployment-values) container can resolve a variable on each request instead, so one image serves deployments that differ only in those values.
