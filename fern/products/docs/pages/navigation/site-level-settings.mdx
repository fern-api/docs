...
<ParamField path="layout.breadcrumbs.current-page" type="boolean" required={false} default="false" toc={true}>
  When set to `true`, the current page's title is appended to the page-header breadcrumb trail as a non-clickable item (rendered as a `<span>` with `aria-current="page"`). Ancestor breadcrumb items remain clickable links. Defaults to `false` — no change for existing sites.
</ParamField>
