...
<ParamField path="layout.show-nav-availability-badges" type="boolean" required={false} default="false" toc={true}>
  If set to true, availability badges (`Beta`, `Deprecated`, and similar) and colored status dots render next to navigation items in the sidebar. Defaults to `false`. This controls only the sidebar badges; the page-header availability badge always renders regardless of this setting.
</ParamField>
