...
<ParamField path="settings.show-headers-in-examples" type="boolean" required={false} default="false" toc={true}>
  When set to `true`, endpoint response example panes render an additional **Response Headers** code block showing the response headers as raw HTTP lines. For example:

  ```http
  HTTP/1.1 200 OK
  Content-Type: application/json
  Dropbox-API-Result: {"content_type":"image/jpeg"}
  Retry-After: 10
  ```

  The block is only shown when the selected example declares response headers. Defaults to `false`; existing sites are unaffected.
</ParamField>
