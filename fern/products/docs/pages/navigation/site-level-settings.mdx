...
Add a `tabs` key to your `docs.yml` with each tab's display name and path. Each tab can have its own navigation tree. Tabs appear in the header or sidebar depending on your `layout.tabs-placement` setting (`header` or `sidebar`).
...
Set `css: ./assets/styles.css` in `docs.yml` to inject a custom stylesheet. For JavaScript, use the `js` key with a path and optional `strategy` (`beforeInteractive`, `afterInteractive`, or `lazyOnload`).
