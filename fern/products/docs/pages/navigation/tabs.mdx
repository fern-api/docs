...
Tabs display in the left sidebar by default. To move them to the header, set [`theme.tabs.placement`](/learn/docs/configuration/site-level-settings#themetabsplacement) to `header`, or use the `layout.tabs-placement` property.
...
You can change the visual style and alignment of tabs using the [`theme.tabs`](/learn/docs/configuration/site-level-settings#themetabs) setting. Set `alignment` to `center` to horizontally center header tabs.
...
<Note>Centering only applies when `placement` is `header`. When `placement` is `sidebar`, alignment has no effect.</Note>

For style-only configuration, the string shorthand is still supported:
