...
Hidden versions won't appear in the version selector dropdown, site search results, or search engine indexes, but users can still access them directly via URL if they know the link.

<Warning>
The default version (the first version in your `versions` list) can't be hidden. If you need to hide what's your default version, reorder your versions list to make a different version the default first.
</Warning>
