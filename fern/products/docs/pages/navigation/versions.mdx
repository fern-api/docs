...
Hidden versions are only accessible via URL and won't appear in the version selector dropdown, site search results, or search engine indexes. The default version (the first version in your `versions` list) can't be hidden.
