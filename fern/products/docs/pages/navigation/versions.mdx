...
<Step title="Hide versions from navigation">

You can hide specific versions from navigation, search, and indexing while keeping them accessible via direct URL by setting `hidden: true`. This is useful for deprecating old versions or maintaining internal-only versions without removing them entirely.
...
Hidden versions will not appear in the version selector dropdown, site search results, or search engine indexes, but users can still access them directly via URL if they know the link.

<Warning>
The default version (the first version in your `versions` list) cannot be hidden. If you need to hide what is currently your default version, reorder your versions list to make a different version the default first.
</Warning>
