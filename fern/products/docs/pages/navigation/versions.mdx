...
<Tip>
  You can use the `<If>` component to conditionally render content within a page based on the current version. For example, `<If versions={["v2"]}>` will only show its content when the reader is viewing version `v2`. Learn more about [conditionally rendered content](/learn/docs/writing-content/conditionally-rendered-content).
</Tip>
