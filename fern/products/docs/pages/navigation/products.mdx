...
<Warning>
A top-level `navigation` block cannot coexist with `products`. `fern check` rejects this configuration, and it would otherwise override your products and render an empty site. Define navigation inside each product's file instead.
</Warning>
