...
<Tip>
  You can use the `<If>` component to conditionally render content within a page based on the current product. For example, `<If products={["api-reference"]}>` will only show its content when the reader is viewing the `api-reference` product. Learn more about [conditionally rendered content](/learn/docs/writing-content/conditionally-rendered-content).
</Tip>
