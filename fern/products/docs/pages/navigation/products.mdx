...
Products and versions nest: a product holds one or more versions, and each version defines its own information architecture. Navigation, tabs, sections, and API references live in each version's `.yml` file, so versions of the same product can differ in structure, not only in content.

<Note>For standalone versioning without products, see our [Versioning guide](/learn/docs/configuration/versions).</Note>
