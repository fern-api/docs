...
Set availability badges on pages, sections, or folders to show their status in the sidebar next to the title, without hardcoding it in the title text. Options are: `stable`, `generally-available`, `in-development`, `pre-release`, `deprecated`, or `beta`.

`beta`, `pre-release`, `in-development`, and `deprecated` render a badge next to the item in the sidebar. `stable` and `generally-available` represent the default available state and don't render a sidebar badge. `deprecated` also renders the title with a strikethrough.
