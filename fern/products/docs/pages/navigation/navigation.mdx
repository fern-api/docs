...
Sidebar availability badges are off by default. Set [`layout.show-nav-availability-badges: true`](/learn/docs/configuration/site-level-settings#layout-configuration) in `docs.yml` to render them next to navigation items. The page-header availability badge always renders regardless of this setting.
