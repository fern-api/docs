...
Control how sections and folders appear in the sidebar when the page loads using the `collapsed` property.

| Value | Behavior |
|---|---|
| `true` | Collapsed on load. The user can expand it. |
| `false` (default) | Expanded on load. The user can collapse it. |
| `open-by-default` | Expanded on load, but collapsible. Unlike `false`, the section displays a toggle so the user can collapse it. |

{/* <!-- vale off --> */}
```yaml {8, 10, 14} Example config with collapsed sections
navigation:
...
In this example, **Getting started** is expanded by default, **Advanced topics** and the **Features** folder start collapsed, and **API guides** starts expanded but can be collapsed by the user.
