...
The overview page also supports a `layout` frontmatter property that overrides the site-wide changelog layout for this specific changelog. Set it to `classic` to render the changelog as stacked full entries instead of the default searchable timeline:
...
The classic layout displays each entry's full content inline, preserving code formatting, copy buttons, and links. The timeline layout (default) shows condensed entry cards with plain-text excerpts.

<Note>
This per-changelog override takes priority over the site-wide [`layout.changelog-layout`](/learn/docs/configuration/site-level-settings#changelog-layout) setting in `docs.yml`.
</Note>
