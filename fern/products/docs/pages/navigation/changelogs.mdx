...
- `blog` — a blog-style listing of entry cards. This layout is available only as a per-changelog override; the site-wide `layout.changelog-layout` setting accepts only `timeline` and `classic`.
...
```mdx title="changelog/overview.mdx"
---
layout: blog
blog-layout: list
authors:
...
- `slug` overrides the date-derived URL.
...
Entry files still require a leading date in the filename. Files named `overview`, `index`, or `summary` are treated as the overview page, and the date can't be overridden in frontmatter. Search, tag filtering, entry pages, and RSS, Atom, and JSON feeds work the same way as they do for a changelog. Everything else about a changelog is unchanged.
