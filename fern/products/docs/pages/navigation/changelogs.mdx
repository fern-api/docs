...
Fern exposes two machine-readable endpoints for every changelog:

- `/changelog.md` — returns an aggregated Markdown document of the 20 most recent public entries, opened by the overview page (or a `# <title>` heading when none is configured). See [Changelog pages](/learn/docs/ai-features/markdown#changelog-pages) in the Markdown access guide for the full format.
- `/changelog/llms.txt` — lists every public entry under an `## Entries` heading without inlining entry bodies, giving agents a complete index to fetch entries on demand.
