...
## Hiding a section, tab, or version

Hide an entire section, tab, or version from the sidebar — for example, a legacy API version, an internal tab, or a section of internal tooling docs.

Unlike hiding a page or endpoint, hiding a section, tab, or version only removes the group from navigation. The individual pages within it remain indexed by search engines and AI because `hidden` applies to the grouping, not to each page. To also exclude individual pages from search results, add `noindex: true` to each page's frontmatter.
