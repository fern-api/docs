...
<ParamField path="disable-header" type="boolean" required={false} default={false}>
  ...
  This can be set globally in the [global configuration](/learn/docs/configuration/site-level-settings#layout-configuration). A page cannot show a header that the global configuration hides.
</ParamField>

<ParamField path="hide-header-tabs" type="boolean" required={false} default={false}>
  Hides the tabs row on this page while keeping the header bar itself. Applies only when [`tabs-placement`](/learn/docs/configuration/site-level-settings#layout-configuration) is `header`.
</ParamField>
