...
<Tip>
**Migrating from Mintlify?** In Mintlify, `description` is the visible subtitle and `og:description` is the meta description. In Fern, `subtitle` is the visible text and `description` is the meta description — so Mintlify's `description` maps to Fern's `subtitle`, and Mintlify's `og:description` maps to Fern's `description`.
</Tip>
