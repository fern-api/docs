...
<ParamField path="search-metadata" type="object" required={false}>
  Custom key-value pairs merged into the page's Algolia search record. Keys are strings; values are strings or arrays of strings. Fern's built-in search filters ignore these fields, so surfacing them requires [configuring facets in Algolia and building your own search UI](/learn/docs/customization/search#custom-search-metadata).
</ParamField>

<CodeBlock title="cuda-programming-guide.mdx">
```mdx
---
