...
<ParamField path="subtitle" type="string" required={false}>
  Renders as visible text below the page title. This is an on-page element — readers see it directly. If [`description`](#meta-description) is not set, `subtitle` is also used as the HTML meta description and in [`llms.txt`](/learn/docs/ai-features/llms-txt#page-descriptions).
</ParamField>

For example, scroll to the top of this page you're visiting now and you'll see the subtitle "Set titles, descriptions, slugs, layout, and visibility using frontmatter".
...
<ParamField path="description" type="string" required={false}>
  Sets the HTML [meta description](https://web.dev/learn/html/metadata#description) for SEO. This text appears in search engine result snippets and in [`llms.txt`](/learn/docs/ai-features/llms-txt#page-descriptions). It is **not** displayed on the page itself — use [`subtitle`](#subtitle) for visible on-page text. When `description` is not set, Fern uses `subtitle` as the meta description instead.

  If [`og:description`](#seo-metadata) is not set, `description` is also used as the OpenGraph description for social sharing previews. For more on meta descriptions and SEO, see [Google's guidelines](https://developers.google.com/search/docs/appearance/snippet#meta-descriptions).
</ParamField>
...
### How `subtitle`, `description`, and `og:description` relate

These three fields control different outputs. Set whichever combination your page needs:

| Field | Where it appears | Fallback when unset |
|-------|-----------------|---------------------|
| `subtitle` | Visible text below the page title | — |
| `description` | HTML `<meta name="description">` tag, search snippets, `llms.txt` | Uses `subtitle` as the meta description |
| `og:description` | Social sharing previews (OpenGraph) | Uses `description` as the OG description |

In most cases, setting `description` is sufficient — it covers both SEO and social sharing. Add `subtitle` when you also want visible on-page text, and `og:description` only when the social preview should differ from the meta description.

<Note>
**Migrating from another docs platform?** Some platforms use a single `description` field for both visible subtitle text and OpenGraph metadata. In Fern, these are separate: use `subtitle` for visible page text and `description` for the HTML meta description.
</Note>
...
<ParamField path="og:description" type="string" required={false}>
  The description shown in social sharing previews (e.g., Slack, Twitter, Facebook). If unset, falls back to [`description`](#meta-description).
</ParamField>
