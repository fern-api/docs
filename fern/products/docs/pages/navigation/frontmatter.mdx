...
Fern emits [JSON-LD structured data](/learn/docs/seo/setting-seo-metadata#structured-data-json-ld) on every indexable page. Use `jsonld: false` to suppress all built-in output, or `jsonld:breadcrumb` to override only the breadcrumb trail.
