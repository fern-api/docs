...
## Header
<ParamField path="disable-header" type="boolean" required={false} default={false}>
  Hides the site header (logo, search, navbar links, and tabs) on this page. Useful for a custom landing page that supplies its own navigation.

  To hide the header across the site, set [`layout.disable-header`](/learn/docs/configuration/site-level-settings#layoutdisable-header) in `docs.yml`. A page cannot show a header hidden by this site-level setting.
</ParamField>

<ParamField path="hide-header-tabs" type="boolean" required={false} default={false}>
  Hides the tabs row on this page while keeping the header bar itself. Applies only when [`layout.tabs-placement`](/learn/docs/configuration/site-level-settings#layouttabs-placement) or [`theme.tabs.placement`](/learn/docs/configuration/site-level-settings#themetabsplacement) is `header`.
</ParamField>
