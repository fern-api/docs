...
<ParamField path="subtitle" type="string" required={false}>
  Renders as a subtitle on the page. If `description` is not set, `subtitle` is also used as the meta description tag and in [`llms.txt` and `llms-full.txt`](/learn/docs/ai-features/llms-txt/overview#page-descriptions).
</ParamField>
