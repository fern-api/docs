...
<Note title="Customizing folder behavior">
  ...
  **Title source:** Add `title-source` to control how page and section titles are derived. Options are `frontmatter` (use the `title` field from each file's frontmatter) or `filename` (derive title from the file name). Defaults to `filename`.
  ```yaml docs.yml
  navigation:
  - folder: ./pages/guides
    title: Guides
