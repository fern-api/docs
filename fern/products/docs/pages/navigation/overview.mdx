...
<ParamField path="title-source" type="'filename' | 'frontmatter'" required={false} default="filename" toc={true}>
  Determines how page and section titles are derived for files in this folder. By default (`filename`), titles are derived from file names. Set to `frontmatter` to use the `title` field from each file's frontmatter instead (falls back to filename if not set).
</ParamField>
