...
This page provides an overview of the file and folder structure of a Fern Docs project, based on the [docs starter](https://github.com/fern-api/docs-starter). The structure is customizable to fit your needs.
...
<Files>
  <Folder name="fern" defaultOpen>
    ...
    <File name="openapi.yaml" comment="OpenAPI specification" />
    <File name="asyncapi.yaml" comment="AsyncAPI specification" />
...
A Fern Docs project has the following top-level files and folders:

- `docs/`: Contains `pages/`, `assets/`, and `changelog/` subfolders. The folder name is customizable.
- `snippets/`: Contains reusable MDX snippets that can be included in pages.
- `docs.yml`: Defines the navigation, theme, and hosting details of your documentation.
- `openapi.yaml`: Your OpenAPI specification file (if you have a REST API Reference section).
- `asyncapi.yaml`: Your AsyncAPI specification file (if you have a WebSocket API Reference section).
- `generators.yml`: References your spec location and configures SDK generation.
- `fern.config.json`: Specifies your organization name and CLI version.
- `styles.css`: Custom CSS styles for your documentation site.
...
<CodeBlock title="fern/docs.yml from the docs starter">
```yml
instances:
...
The docs starter places spec files directly in the `fern/` directory and references them in `generators.yml`:
