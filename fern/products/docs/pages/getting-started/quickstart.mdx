...
**Q: What is the `fern generate --docs` command?**
...
**Q: Why does `fern generate --docs` say "No docs instance found"?**

This error means the `--instance` flag value doesn't exactly match the URL configured in `docs.yml`. The instance URL in `docs.yml` must include the `https://` prefix, but the `--instance` flag value must not. For example, if `docs.yml` has `url: myorg.docs.buildwithfern.com`, run `fern generate --docs --instance myorg.docs.buildwithfern.com`.
