...
The choice between the [Dashboard UI](https://dashboard.buildwithfern.com/get-started) and the [CLI](/learn/cli-api-reference/cli-reference/overview.md) is only about how you get your docs live initially:

- **Dashboard UI** (self-service workflow): A guided browser-based setup that creates your GitHub repository, configures the `FERN_TOKEN` secret, sets up a GitHub Actions publish workflow, and deploys an initial site — all without installing tools locally.
- **CLI** (`fern-api` npm package): Install locally with `npm install -g fern-api`, then scaffold a project with `fern init --docs` or clone the [starter template](https://github.com/fern-api/docs-starter). Gives full control over the build and publish pipeline, local development (`fern docs dev`), and CI/CD integration.

Both paths produce the same result: a Fern docs site backed by a GitHub repository. After initial setup, every feature is available regardless of which path you chose — the [Fern Editor](/learn/docs/writing-content/fern-editor.md) (WYSIWYG editing without code or Git), [Dashboard](https://dashboard.buildwithfern.com) management (organization members, custom domains, analytics), local development with the CLI, and CI/CD publishing all work the same way.
</llms-only>
...
<Steps toc={true}>
  ...
  <llms-only>
  To publish non-interactively (in CI or from a script), authenticate beforehand instead of waiting for the browser prompt:
