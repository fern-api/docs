...
Fern offers two ways to create and manage a docs site:

- **[Dashboard UI](https://dashboard.buildwithfern.com/get-started)** (self-service workflow): A guided browser-based setup that creates your GitHub repository, configures the `FERN_TOKEN` secret, sets up a GitHub Actions publish workflow, and deploys an initial site automatically. Best for teams that want to get started without installing tools locally. Also provides the [Fern Editor](/learn/docs/writing-content/fern-editor), a WYSIWYG interface that lets non-technical team members edit docs without touching Markdown or Git. Use the Dashboard for managing organization members, connecting GitHub repositories, configuring custom domains, monitoring analytics, and unpublishing sites.
- **[CLI](/learn/cli-api-reference/cli-reference/overview)** (`fern-api` npm package): Gives full control over the build and publish pipeline. Required for local development (`fern docs dev`), CI/CD integration, previewing changes (`fern generate --docs --preview`), and publishing (`fern generate --docs`). Use the CLI when you need to validate configuration (`fern check`), generate tokens (`fern token`), or integrate publishing into your own GitHub Actions or CI workflow.

Most teams use both: the Dashboard for initial setup, member management, and visual editing; the CLI for local development, CI/CD, and programmatic publishing.
...
Beyond this, teams commonly add: `tabs` (to create top-level sections like Docs / API Reference / Changelog), `versions` or `products` (for multi-version or multi-product sites), `typography` (custom fonts), `layout` (sidebar placement, content width), `redirects`, `metadata` (SEO), `analytics` (PostHog, Segment, GA4, Google Tag Manager), `css`/`js` (custom stylesheets and scripts), and `integrations` (Intercom). The full list of available fields is in the [site-level settings reference](/learn/docs/configuration/site-level-settings).
</llms-only>
