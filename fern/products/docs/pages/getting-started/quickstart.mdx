...
Once your docs site is published, [Fern's MCP server](/learn/docs/ai-features/fern-mcp-servers) gives your agent access to Fern's documentation, configuration reference, and your organization's live data: sites, configuration, page source, deployment history, analytics, search queries, and reader feedback.

**Prerequisites**: your docs must be published and you must be logged in (`fern login`).
...
This registers the MCP server with every supported agent found on your machine — Claude (Claude Code / Claude Desktop), Cursor, Codex, and GitHub Copilot — reusing the token from `fern login`. No separate authentication step is required.

### Verify the connection

After running the command, open your agent and check that Fern tools are listed as available. Because agents load MCP servers at startup, the tools appear in the next session after installation.
