...
Once your site is published, [Fern's MCP server](/learn/docs/ai-features/fern-mcp-servers) gives your agent Fern's documentation and configuration reference, plus read-only access to your live data: sites, configuration, page source, deployment history, analytics, search queries, and reader feedback.

### Prerequisites

- A published docs site (`fern generate --docs`).
- An authenticated CLI: run `fern login` if you haven't already.

### Install the MCP server

The CLI registers the MCP server with every supported agent on your machine, reusing the token from `fern login`:
...
1. Detects the AI agents installed on your machine (Claude Desktop, Cursor, Windsurf, and others).
2. Registers the Fern MCP server with each detected agent.
3. Scopes the connection to your organization using your existing login token, so no extra credentials are needed.

### Verify the installation

After running the command, restart your agent and confirm the Fern MCP server appears in its list of connected tools. You can then ask your agent questions like:

- "What pages are in my docs site?"
- "Show me the deployment history for my docs."
- "What are readers searching for?"

### Supported agents

`fern mcp install` configures the MCP server for every supported agent it finds on your machine. If your agent isn't auto-detected, add the server manually with the commands in the [MCP setup guide](/learn/docs/ai-features/agent-setup).
