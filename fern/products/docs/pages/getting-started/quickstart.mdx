...
<Steps toc={true}>
  ...
  <llms-only>
  **CI/CD usage**: `fern generate --docs` prompts for confirmation before deploying. To skip this in CI or scripts:
  ...
  **Authentication**: The CLI checks for a `FERN_TOKEN` environment variable first, then falls back to a cached user token from `fern login`. There is no `fern whoami` command. In GitHub Actions, store the token as a repository secret named `FERN_TOKEN`. See [publishing your docs](/learn/docs/getting-started/publishing-your-docs.md) for full CI workflow examples.
  </llms-only>
