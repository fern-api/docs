...
Create a `changelog/` folder directly inside your `fern/` folder (not inside `pages/`). Name each entry file by date using one of these formats: `YYYY-MM-DD.md`, `MM-DD-YYYY.md`, or `MM-DD-YY.md`. Register it in `docs.yml` either as a tab (using `changelog: ./changelog` under the tab definition) or as a section-level entry (`- changelog: ./changelog`). Subdirectories within the `changelog` folder are not supported.
