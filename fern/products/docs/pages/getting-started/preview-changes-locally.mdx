...
By default, each run generates a new URL with a unique UUID. To create a **stable, named preview link** that stays the same across multiple runs, use the `--id` flag:

```bash
fern generate --docs --preview --id my-feature
```

The `--id` value is used as part of the preview URL (for example, `https://your-org-preview-my-feature.docs.buildwithfern.com`). Subsequent runs with the same `--id` overwrite the previous preview, so the URL never changes. This is useful when you want to share a link with reviewers and keep it up to date as you push changes.

You can also [list all preview deployments](/learn/cli-api-reference/cli-reference/commands#fern-docs-preview-list) for your organization and [delete a preview deployment](/learn/cli-api-reference/cli-reference/commands#fern-docs-preview-delete) when it's no longer needed:
