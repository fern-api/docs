...
When you reuse an `--id` that already exists, the CLI detects the duplicate and prompts for confirmation before overwriting. Use `--force` to skip this prompt, which is useful for CI/CD pipelines where the CLI may not detect that it's running in an automated environment.
