...
<ParamField path="playground" toc={true} type="object">
  Settings for the [API Explorer](/learn/docs/api-references/api-explorer) that affect all endpoints in this API section. Can also be set on individual sections or endpoints in `layout` to override. Accepts the following properties:

  - `oauth` (`boolean`): When `true`, enables the [OAuth client-credentials flow](/learn/docs/api-references/api-explorer#oauth-client-credentials) in the API Explorer.
