...
The `layout` option orders sections, endpoints, and pages. It doesn't order the properties within a request, response, or schema. Property order in the API Reference follows a fixed set of rules:

1. **Required properties render first, optional properties second.** A property is required when its name appears in the schema's `required` array. Its position in the `properties` block doesn't affect this: a property defined last but marked required still renders at the top.
2. **Within each group, properties follow the order they're defined** in the `properties` block of your API definition.
3. **Deprecated properties sort after non-deprecated ones** within each group.

<Note>
Schemas that compose other schemas with `allOf` (OpenAPI) or `extends` (Fern Definition) are the exception to rule 2: their properties are sorted alphabetically by name within each group rather than by definition order. To preserve definition order, inline the composed properties. For OpenAPI specs, setting `inline-all-of-schemas` to `true` in `generators.yml` inlines `allOf` schemas automatically.
</Note>

There is no configuration to interleave required and optional properties or to specify an arbitrary property order.
