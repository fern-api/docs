...
When you use Fern's SDK Generator, you can automatically display SDK code snippets in your API Reference. These snippets show code examples using your generated Fern SDKs, as opposed to [HTTP snippets](/learn/docs/api-references/http-snippets) which show generic HTTP client code like cURL or Python's `requests` library. SDK snippets are configured using the `snippets` key under your API Reference in the navigation section.
