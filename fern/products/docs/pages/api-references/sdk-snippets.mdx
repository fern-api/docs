...
<Note title="SDK snippets vs. HTTP snippets">
SDK snippets show code examples using your generated Fern SDKs. They're different from [HTTP snippets](/learn/docs/api-references/http-snippets), which show generic HTTP client code (like cURL or Python's `requests` library). SDK snippets are configured using the `snippets` key under your API Reference in the navigation section, while HTTP snippets are configured using `settings.http-snippets`.
</Note>
