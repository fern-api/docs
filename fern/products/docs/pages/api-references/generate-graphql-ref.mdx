...
Fern generates a page for every named type in your schema, collected under a `Types` section at the root of the API Reference. Within that section, pages are grouped by the kind each type was declared with: Objects, Inputs, Enums, Scalars, Interfaces, and Unions. A kind your schema doesn't declare produces no group, and every GraphQL schema in the API section contributes to the same `Types` section.

Type pages live at `<api-reference>/types/<kind>/<type-name>`, such as `/api-reference/types/objects/plant`. An interface page lists the interface's own fields along with the types that implement it.

Types that exist only to group operations, such as a `Mutation.plants: PlantMutations` namespace type, don't get a page. Their fields are already documented as operations.
