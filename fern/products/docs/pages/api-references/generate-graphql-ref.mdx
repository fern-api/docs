...
### Customize operation display

Set a custom `title`, `slug`, `hidden`, or `availability` on each operation:
...
<Note>
For the full set of layout options that apply to all API types (alphabetizing, flattening, adding pages and links, and more), see [Customize API Reference layout](/learn/docs/api-references/customize-api-ref).
</Note>
