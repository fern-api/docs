...
navigation:
  - api: GraphQL API Reference
    summary: pages/graphql-overview.mdx
```

### Add pages and links

Include Markdown pages and external links within your GraphQL reference:
...
navigation:
  - api: GraphQL API Reference
    layout:
      - page: GraphQL Guide
        path: ./docs/pages/graphql-guide.mdx
      - link: GraphQL Spec
...
- `alphabetized: true` — Sort all sections and operations alphabetically
- `flattened: true` — Remove the API Reference title and display sections at the top level
- `paginated: true` — Display each operation on a separate page
- `display-errors: true` — Show error response schemas on operation pages
