...
Credentials stay in the browser. The Explorer keeps them in `sessionStorage` under the `playground-auth-state` key, unencrypted, and the browser discards them when the tab closes. Fern never stores them server-side. Requests route through Fern's proxy by default, so credentials pass through the proxy in transit; [`disable-explorer-proxy`](/learn/docs/configuration/site-level-settings#settingsdisable-explorer-proxy) sends requests from the browser directly to your API instead.
