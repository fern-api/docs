...
The API Explorer supports streaming endpoints, including Server-Sent Events (SSE). When an endpoint is configured as streaming in your API specification, the API Explorer displays a **STREAM** badge next to the HTTP method and streams the response in real-time as data arrives. See [Hume's Text-to-Speech API](https://dev.hume.ai/reference/text-to-speech-tts/synthesize-json-streaming) for a live example.
...
<Frame caption="Streaming endpoint in Hume's API Explorer">
