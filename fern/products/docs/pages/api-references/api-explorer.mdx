...
The API Explorer supports streaming endpoints, including Server-Sent Events (SSE). When an endpoint is configured as streaming in your API specification, the API Explorer displays a **STREAM** badge next to the HTTP method and streams the response in real-time as data arrives.

To configure a streaming endpoint, use the [`x-fern-streaming`](/learn/api-definitions/openapi/extensions/server-sent-events) extension in your OpenAPI specification or the [`response-stream`](/learn/api-definitions/fern-definition/endpoints/server-sent-events) key in your Fern Definition.

<Frame caption={<a href="https://dev.hume.ai/reference/text-to-speech-tts/synthesize-json-streaming">Example of a streaming endpoint in Hume's API Explorer</a>}>
  <img src="sse-api-explorer.png" alt="Streaming endpoint in API Explorer" />
</Frame>
