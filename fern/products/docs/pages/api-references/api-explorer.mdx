...
To limit an environment to certain readers, tag its `servers` entry with [`x-fern-audiences`](/learn/api-definitions/openapi/extensions/audiences) alongside `x-fern-server-name`. Unlike audience filtering elsewhere, environments are filtered per viewer at request time: an environment appears when it has no audiences, or when one of its audiences matches a role on the authenticated viewer. This requires [RBAC roles](/learn/docs/authentication/features/rbac) — on a site without them no viewer has a role, so a tagged environment is hidden from everyone while untagged environments stay visible to all.
