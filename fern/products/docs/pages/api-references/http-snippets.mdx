...
HTTP snippets are different from [SDK snippets](/learn/docs/api-references/sdk-snippets). HTTP snippets show generic HTTP client code (like cURL or `requests`), while SDK snippets show code using your generated Fern SDKs. If you have SDK snippets configured, those display by default. HTTP snippets display automatically when no SDK snippets are available, so the language dropdown you see in your API Reference is likely showing HTTP snippets unless you've configured SDK snippets.
</Note>
