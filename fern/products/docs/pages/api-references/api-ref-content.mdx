...
Use the `api:` link syntax to create links to API endpoints by HTTP method and path, instead of hardcoding URL slugs. Fern resolves these links to the correct endpoint URL at build time.

The format is `api:METHOD/path`:
...
paths:
  /orders:
    post:
      ...
      description: |
        Creates a new order. To retrieve an existing order,
        use [Get order](api:GET/v2/orders/\{orderId\}).
```
</CodeBlock>
<CodeBlock title="Fern Definition">
```yaml
...
<CodeBlock title="Markdown page">
```mdx
Check out the [Create payment](api:POST/v2/payments) endpoint to get started.
```
</CodeBlock>
</CodeBlocks>
...
Here's a live example — this link points to the [Algolia search credentials](api:mcp-tools:GET/api/fern-docs/search/v2/key) endpoint in the Fern Public API.

For more details on link syntax, see [Markdown basics](/learn/docs/writing-content/markdown-basics#link-to-an-api-endpoint).
