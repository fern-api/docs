...
The input source can be a `git` URL (parsed remotely on Fern's servers) or a local `path` (parsed locally with the `--local` flag).
...
Run the CLI command to generate MDX files from your library source code. Which command you run depends on whether your library uses a `git` or `path` input.
...
The `--local` flag parses library source on your machine instead of on Fern's servers, which is useful for iterating on documentation without pushing to a remote repository. It doesn't require you to be logged in to Fern.
