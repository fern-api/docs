...
<Tip>For an example of how to set up the `fern_token` cookie, see the demo implementation below.</Tip>
...
async function mintJwtToken({ secret, apiKey }: { secret: string; apiKey: string }) {
    const jwtPayload = {
        fern: {
            playground: {
                initial_state: {
...
// After minting the token, set it as a cookie:
// cookies.set("fern_token", fern_token, { secure: true, httpOnly: true });
```
