...
<Tip>
Here's an example of how to mint and set the `fern_token` cookie using the [jose](https://github.com/panva/jose) library:

```typescript Example: Minting and setting the fern_token cookie maxLines=10
import { SignJWT } from "jose";

const encoder = new TextEncoder();

async function mintFernToken(secret: string, apiKey: string): Promise<string> {
  const jwtPayload = {
    fern: {
      playground: {
        initial_state: {
...
const fernToken = await mintFernToken(SECRET, API_KEY);

cookies().set("fern_token", fernToken, {
  httpOnly: true,
  secure: true,
  sameSite: "lax",
  domain: "your-docs-domain.com",
});
```
</Tip>
