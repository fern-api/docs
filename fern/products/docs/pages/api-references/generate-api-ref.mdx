...
<Note>
  Your APIs must be declared in `generators.yml` before they can be rendered in your documentation. The `generators.yml` file serves two roles: it declares which APIs your project contains and configures code generation (for example, SDKs). See [OpenAPI generators.yml reference](/learn/api-definitions/openapi/generators-yml-reference).
</Note>
