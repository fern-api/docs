...
Every page carries a hash-based [Content Security Policy](/learn/docs/security/content-security-policy) with no `'unsafe-inline'` in `script-src`. Sites with custom JavaScript or Google Tag Manager can remove the remaining `unsafe-` tokens in `docs.yml`.
