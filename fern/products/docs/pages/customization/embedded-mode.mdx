...
## Hide only the header or footer

To remove one piece of chrome while keeping the other, use the `header` and `footer` query parameters instead of `embedded`:

| Parameter | Effect |
|-----------|--------|
| `header=false` | Hides the header only |
| `footer=false` | Hides the footer only |

```jsx HTML
<iframe
  src="https://elevenlabs.io/docs/overview/intro?header=false"
  style="width: 100%; height: 500px; border: none; border-radius: 8px;"
></iframe>
```

Inside embedded mode, `header=true` or `footer=true` opts that element back in. For example, `?embedded=true&footer=true` hides the header but keeps the footer.
