...
## Fine-grained chrome control

`?header=false` and `?footer=false` can be used independently of `?embedded=true` to hide only one piece of chrome at a time. Inside embedded mode, `?header=true` or `?footer=true` opts that piece back in.

| URL parameter(s) | Header | Footer |
|---|---|---|
| `?embedded=true` | hidden | hidden |
| `?header=false` | hidden | shown |
| `?footer=false` | shown | hidden |
| `?embedded=true&header=true` | shown | hidden |
| `?embedded=true&footer=true` | hidden | shown |
