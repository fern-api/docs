...
<Note>
These settings apply to keyword search only. [Ask Fern](/learn/docs/ai-features/ask-fern/overview) queries the same Algolia index and benefits from its global ranking, but does not receive the per-product boost from `prioritize-current-product`.
</Note>
