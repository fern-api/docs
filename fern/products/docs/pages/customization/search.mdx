...
For multi-product sites, you can configure search to automatically filter results by the active product. Set [`default-search-filters: true`](/learn/docs/configuration/site-level-settings#settingsdefault-search-filters) in your `docs.yml` settings to apply product filters by default when users open search. Users can still remove the filter to search across all products.
