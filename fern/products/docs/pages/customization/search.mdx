...
### Add custom metadata for faceting and filtering

You can attach arbitrary key-value metadata to any page by adding a `search-metadata` object to its frontmatter. Fern reads these values and writes them as extra attributes on the page's Algolia record, so you can use them to build faceted search, custom filters, or targeted queries against the index.
...
Any key-value pairs nested under `search-metadata` are passed through to Algolia as-is. Key names must be valid Algolia attribute names (alphanumeric characters and hyphens). Values must be strings.

Custom `search-metadata` attributes are stored on the Algolia record but are not surfaced as filters in Fern's built-in search UI. To query or filter on them, use the [Algolia credentials](/learn/docs/customization/search#making-search-requests) provided by the Fern team to make direct API requests or build a custom search integration.
