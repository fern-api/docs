...
For sites with multiple [products](/learn/docs/configuration/products) or [versions](/learn/docs/configuration/versions), you can scope search results to the user's current context using the `settings.search` object in `docs.yml`:
...
<ParamField path="settings.search.prioritize-current-product" type="boolean" required={false} default="false">
  Boosts results from the current product and version higher in the ranking without hiding results from other products. Uses Algolia `optionalFilters` under the hood.
</ParamField>

<ParamField path="settings.search.default-filter-by-current-product" type="boolean" required={false} default="false">
  Auto-selects the current product as a facet filter so only results from the current product are shown by default. Users can remove the filter from the search UI to broaden their search. Equivalent to the legacy [`settings.default-search-filters`](/learn/docs/configuration/site-level-settings#settingsdefault-search-filters) setting.
</ParamField>

These two options can be used independently or together. `prioritize-current-product` is the softer option: results from other products still appear but are ranked lower. `default-filter-by-current-product` is stricter: only current-product results are shown unless the user clears the filter.
