...
To edit a theme without access to the control repository, [download](/learn/cli-api-reference/cli-reference/docs-commands#fern-docs-theme-download) it from Fern's registry into a local directory, modify `theme.yml` or its assets, and upload it again:
...
Global themes work in self-hosted deployments when the theme is vendored into the Docker image. Run `fern docs theme download --name <name>` to pull the theme into a local directory, then copy that directory into the image. See [Global themes in self-hosted](/learn/docs/self-hosted/set-up#global-themes-in-self-hosted) for the Dockerfile pattern and fallback behavior.
