...
By default, custom JavaScript adds `'unsafe-eval'` to `script-src` and `'unsafe-inline'` to `style-src` in your site's [Content Security Policy](/learn/docs/security/content-security-policy). Set `settings.content-security-policy` in `docs.yml` to remove them.
