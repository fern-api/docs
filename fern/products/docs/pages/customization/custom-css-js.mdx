...
In `docs.yml`, specify the path to the `styles.css` file. `css` is a root-level key — place it at the top of the file, not nested inside `navigation`, `instances`, or any other block:
...
colors:
  accent-primary:
    light: "#418326"
    dark: "#ADFF8C"
css: ./styles.css   # root-level — not nested
