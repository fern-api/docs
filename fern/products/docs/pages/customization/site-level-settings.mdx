...
<ParamField path="settings.default-search-filters" type="boolean" required={false} default="false" toc={true}>
  By default (`false`), search displays results for pages across all products and versions. 
  
  If set to true, search results are automatically filtered to the product and version the user is currently browsing. Users can manually remove the filter from the search UI to search across all products and versions. This setting is useful for sites with multiple products where users typically want results scoped to the product they are viewing.
</ParamField>
