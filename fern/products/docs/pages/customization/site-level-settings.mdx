...
<ParamField path="settings.default-search-filters" type="boolean" required={false} default="false" toc={true}>
  Controls whether search results are filtered by the active product by default. 
  
  When set to `false` (the default), search displays results across all products and versions. When set to `true`, opening search automatically applies a product filter for the active product and version the user is browsing. Users can remove this filter from the search UI to search across all products and versions.
  
  This is useful for multi-product documentation sites where you want search scoped to the product the user is viewing. Learn more about [search configuration](/learn/docs/customization/search).
</ParamField>
