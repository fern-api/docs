...
Configure the severity of validation rules run by [`fern check`](/learn/cli-api/cli-reference/commands#fern-check). Each rule can be set to `"warn"` (non-blocking) or `"error"` (blocking).
...
check:
  rules:
    ...
    no-non-component-refs: error
    valid-local-references: warn
