...
These colors are automatically available as CSS custom properties in your [custom stylesheets](/learn/docs/customization/custom-css-js#built-in-css-color-variables), so you can reference them directly without redefining them.
...
<ParamField path="launch" type="'github' | 'dashboard'" required={false} default="github" toc={true}>
  Controls the behavior of the "Edit this page" button. When set to `dashboard`, clicking the button opens a screen where users can choose between starting a [Fern Editor](/learn/docs/writing-content/fern-editor) session for that page or going straight to the source in GitHub. Defaults to `github`, which links directly to the file on GitHub.
</ParamField>
