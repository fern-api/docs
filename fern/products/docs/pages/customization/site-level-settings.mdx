  ...
  options:
    custom:
      ...
      - title: Report issue
        ...
        default: true  # This becomes the default action instead of copy-page
```
...
<ParamField path="page-actions.options.custom[].default" type="boolean" required={false} default="false" toc={true}>
  ...
  When set to `true`, this custom action overrides the [`page-actions.default`](/learn/docs/configuration/site-level-settings#page-actionsdefault) setting and becomes the default action displayed prominently in the page actions area.
</ParamField>
