...
<ParamField path="settings.folder-title-source" type="'filename' | 'frontmatter'" required={false} default="filename" toc={true}>
  Sets the default title source for all [folder navigations](/learn/docs/configuration/navigation#add-a-folder). When set, this value applies to any folder that doesn't specify its own `title-source`. Set to `frontmatter` to derive titles from the `title` field in each file's frontmatter, or `filename` to derive titles from file names.
</ParamField>
