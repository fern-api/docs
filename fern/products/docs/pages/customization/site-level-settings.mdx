...
To remove all page actions from your documentation, set each default-enabled option to `false`:
...
To customize which page actions appear and set a default, use the `default` and `options` properties together:
...
This example hides the default actions and shows only an "Open in Cursor" button.
