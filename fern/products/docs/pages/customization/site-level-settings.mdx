  ...
  options:
    custom:
      - title: Open in Windsurf
        ...
        default: true
      - title: Report issue
        url: "https://github.com/your-org/docs/issues/new?title=Issue on {slug}"
        icon: ./assets/icons/github-issue.svg
```
...
<ParamField path="page-actions.options.custom[].default" type="boolean" required={false} default="false" toc={true}>
  When set to `true`, this custom action becomes the default action displayed prominently in the page actions area. If a custom action has `default: true`, it takes precedence over the `page-actions.default` setting, which only applies to built-in actions.
</ParamField>
...
Configure experimental and AI features in the `experimental` section of your `docs.yml`.

```yaml docs.yml
experimental:
  # AI-generated examples (enabled by default)
...
<ParamField path="experimental.ai-example-style-instructions" type="string" required={false} toc={true}>
  Instructions to guide the content and style of [AI-generated examples](/learn/docs/ai-features/ai-examples) in your API Reference. Use this to align examples with your brand or domain. Example: "Use names and emails that are inspired by plants."
</ParamField>

<ParamField path="experimental.ai-examples" type="boolean" required={false} default="true" toc={true}>
