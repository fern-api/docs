...
<ParamField path="theme.tabs.alignment" type="string" required={false} default="left" toc={true}>
  Sets the horizontal alignment of the tabs. `left` aligns tabs to the left; `center` horizontally centers them.
...
<ParamField path="theme.tabs.placement" type="string" required={false} default="sidebar" toc={true}>
  ...
  This property takes precedence over `layout.tabs-placement` when both are set.

  <Note>This setting is ignored when `layout.disable-header` is set to true.</Note>
</ParamField>
