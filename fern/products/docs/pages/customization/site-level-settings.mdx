...
The `theme` configuration isn't available as a Schema type. The available options are:
...
The `settings` configuration isn't available as a Schema type. The available options are:
...
- `disable-explorer-proxy`: If set to true, the API Explorer will bypass the proxy when sending requests directly to your API. Your API must have Cross-Origin Resource Sharing (CORS) enabled.
...
The `page-actions` configuration isn't available as a Schema type. The available options are:
