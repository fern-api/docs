...
<ParamField path="page-actions.options.cursor" type="boolean" required={false} default="true" toc={true}>
  When enabled, displays an "Open in Cursor" button that allows users to open the page content in Cursor IDE with AI-powered code assistance. Requires [Ask AI](#page-actionsoptionsask-ai) to be enabled.
</ParamField>

<ParamField path="page-actions.options.vscode" type="boolean" required={false} default="false" toc={true}>
  When enabled, displays an "Open in VS Code" button that allows users to open the page content in Visual Studio Code for editing and development. Requires [Ask AI](#page-actionsoptionsask-ai) to be enabled.
</ParamField>
