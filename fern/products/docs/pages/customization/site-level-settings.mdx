...
Configure directives that are prepended to every page when it's [served to AI agents](/learn/docs/ai-features/llms-txt#per-page-directives). By default, Fern prepends a directive that tells AI agents how to access clean Markdown, `llms.txt`, and `llms-full.txt` for your site. To customize it, set `agents.page-directive`:
