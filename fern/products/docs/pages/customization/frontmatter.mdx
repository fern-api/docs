...
Frontmatter values are parsed as MDX, which means certain characters have special meaning and may need to be escaped.

Angle brackets (`<>`) in the `title` field are interpreted as HTML tags. To display literal angle brackets, escape the opening bracket with a backslash:
...
Without the backslash, `<Conference>` would be treated as an HTML tag and stripped from the output, leaving only "noun" as the title.

<Note>
The `sidebar-title` field doesn't require escaping angle brackets.
</Note>
