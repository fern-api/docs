...
Frontmatter uses YAML syntax, so certain characters require quoting. Wrap values in single or double quotes when they contain `:`, `#`, `<`, `>`, `&`, `*`, `!`, `[`, `]`, `{`, `}`, or `%`.

<CodeBlock title="Examples">
```yaml
---
title: "OAuth: A complete guide"      # Colon requires quotes
description: "C# SDK reference"        # Hash requires quotes
subtitle: "Using <Callout> components" # Angle brackets require quotes
---
```
</CodeBlock>

To include quotes within a quoted string, use the opposite quote style or escape with a backslash:

```yaml
title: 'The "best" practices'          # Single quotes around double quotes
title: "The plant API's guide"         # Double quotes around apostrophe
title: "The \"best\" practices"        # Escaped double quotes
```
