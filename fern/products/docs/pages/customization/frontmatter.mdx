...
Frontmatter uses YAML syntax, so certain characters have special meaning and require quoting or escaping. The most common special characters are colons (`:`), angle brackets (`<` and `>`), quotes (`"` and `'`), and ampersands (`&`).

### When to use quotes

Wrap your value in quotes when it contains any of these characters:

| Character | Meaning in YAML | Example |
|-----------|-----------------|---------|
| `:` | Key-value separator | `title: "OAuth: A complete guide"` |
| `#` | Comment | `title: "C# SDK reference"` |
| `<` `>` | Tag indicators | `title: "Using <Callout> components"` |
| `&` | Anchor | `title: "Terms & conditions"` |
| `*` | Alias | `title: "Wildcards: Using * in queries"` |
| `!` | Tag indicator | `title: "Important! Read this first"` |
| `[` `]` | Flow sequence | `title: "Array syntax [0] explained"` |
| `{` `}` | Flow mapping | `title: "Object syntax {key: value}"` |
| `%` | Directive | `title: "100% complete"` |

### Quoting styles

You can use either single or double quotes:

<CodeBlock title="Single quotes">
```yaml
---
title: 'OAuth: A complete guide'
description: 'Learn about <Callout> & other components'
---
```
</CodeBlock>

<CodeBlock title="Double quotes">
```yaml
---
title: "OAuth: A complete guide"
description: "Learn about <Callout> & other components"
...
If your value contains the same type of quote you're using to wrap it, either escape the inner quote or use the other quote style:

<CodeBlock title="Escaping quotes">
```yaml
---
# Use single quotes to wrap a value containing double quotes
title: 'The "best" practices guide'

# Or use double quotes and escape inner double quotes
title: "The \"best\" practices guide"

# Use double quotes to wrap a value containing single quotes
title: "The plant API's authentication guide"
---
```
</CodeBlock>
...
Values starting with special characters or containing a colon followed by a space need quotes:

<CodeBlock title="Incorrect">
```yaml
---
title: OAuth: A complete guide
description: <Callout> usage tips
---
```
</CodeBlock>
