...
<ParamField path="last-updated" type="string" required={false}>
  Displays a "Last updated" message in the page footer. Use this to show readers when the content was last modified. The value is displayed as-is, so you can use any date format you prefer.
</ParamField>
...
You can use a GitHub Actions workflow to automatically update the `last-updated` field whenever MDX files are modified in a pull request. This workflow detects changed MDX files, updates or adds the `last-updated` frontmatter with the current date, and commits the changes back to the PR branch.

<Accordion title="GitHub Actions workflow">
<CodeBlock title=".github/workflows/update-last-updated.yml">
```yaml
...
The workflow runs on pull requests to the `main` branch and performs the following steps: it checks out the PR branch, identifies which MDX files have changed, and for each changed file either updates an existing `last-updated` field or adds one to the frontmatter. The changes are then committed and pushed back to the PR branch automatically.
