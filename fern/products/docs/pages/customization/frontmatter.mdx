...
<ParamField path="sidebar-title" type="string" required={false} default="Page name from docs.yml">
  Overrides the title displayed in the sidebar navigation. By default, the sidebar uses the page name defined in the navigation section of `docs.yml`. Use this option when you want a shorter or different title in the sidebar while keeping a longer, more descriptive title on the page itself.
</ParamField>

For example, you might have a long page title like "Getting started with authentication" but want to display just "Authentication" in the sidebar:
...
In `docs.yml`, the page name is still used as the default if neither `title` nor `sidebar-title` is set in frontmatter:
```yaml title="docs.yml"
navigation:
  - page: Authentication Guide  # Default for both page title and sidebar
