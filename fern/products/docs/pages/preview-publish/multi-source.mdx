...
When multiple repositories publish to the same host, Fern can add a **site switcher** to every site's header: a dropdown listing all the other live sites on the domain. Visitors can jump between sites without the fleet owner hand-maintaining links in every repository.

The switcher is populated at publish time from the domain's live site registry. A newly published sibling appears on the other sites at their next publish.

To enable it, add `theme.site-switcher` to your [global theme](/learn/docs/customization/global-themes) (recommended, so every site on the fleet picks it up from one place):

```yaml theme.yml
theme:
  site-switcher:
    enabled: true                       # required to show the switcher
    order: [/dynamo, /brev, /nemo]      # optional: pin sites first
    hide: [/internal-playground]        # optional: exclude sites
    labels:                             # optional: override display names
      /holoscan/sdk-user-guide: Holoscan SDK
    show-products: true                 # optional: list each site's products indented beneath it
```

How the switcher works:

- The root site (`/`) is never listed. The logo is the way back to it.
- Nested basepaths (e.g. `/nemo/relay`) appear directly after their parent in the list, at the same level. A nested site is still a whole site, not a sub-item of its parent.
- A basepath whose parent is not published appears as a top-level entry.
- The trigger shows the current site's title; on the root site it reads "Home".
- Clicking an entry navigates to that site's root (`/{basepath}/`). Relative paths are not preserved across sites.

The [`theme.site-switcher`](/learn/docs/configuration/site-level-settings#theme-configuration) reference lists every option.
