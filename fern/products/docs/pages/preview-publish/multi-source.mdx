...
<Note>
`multi-source: true` is only supported on Fern-hosted multi-tenant domains. In self-hosted deployments, the container strips this field with a warning at build time so the same `docs.yml` can be used for both hosted and self-hosted publishes without modification.
</Note>
