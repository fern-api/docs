...
DNS records apply to a hostname, not to a path. The shared domain is configured for DNS exactly once, no matter how many sub-paths or repositories publish into it.

<Tip>
Adding a sub-path or a new source repository never requires a new DNS record.
</Tip>

Configure the shared domain once by following [Bring your custom domain](/learn/docs/preview-publish/setting-up-your-domain) and expanding the section that matches the shared domain's shape. For a shared subdomain like `docs.example.com`, the most common multi-source shape, that's one CNAME record for the subdomain plus one TXT record for verification.

Each source repository's only domain-related responsibility is [declaring its own unique basepath](#set-up-multi-source-docs) at the end of both `url` and `custom-domain`, with `multi-source: true`.

### Who serves the bare root?

Three shapes are possible:

- **A Fern-published project owns the root.** It declares the bare domain as its basepath and acts as the landing page, like the homepage in the [plantstore example](#example-a-live-multi-source-site).
- **No Fern project owns the root.** Set the **Default path** in the Dashboard's [multi-repo settings](#set-up-multi-source-docs) so the bare domain redirects into a sub-path such as `/home`.
- **Something outside Fern serves the root**, like a separate marketing site, and only the sub-paths are Fern-managed, as with `docs.nvidia.com`. Routing Fern sub-paths from a root domain that Fern doesn't serve is a subpath setup, which requires a [reverse proxy](/learn/docs/preview-publish/reverse-proxy).
