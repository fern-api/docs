...
Multi-source documentation publishes a single docs site to a custom domain, split into multiple sub-paths. Each sub-path is sourced from its own repository, so teams ship updates independently while a shared [global theme](/learn/docs/customization/global-themes) keeps branding consistent across the site. The theme can also carry the [product list](/learn/docs/customization/global-themes#share-products-across-sites), so the product switcher spans all repositories instead of only the one that published a given sub-path.
