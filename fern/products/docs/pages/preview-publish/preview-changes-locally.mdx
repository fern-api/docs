...
<Warning>
On Windows, `fern docs dev` requires [long path support](https://learn.microsoft.com/en-us/windows/win32/fileio/maximum-file-path-limitation#enable-long-paths-in-windows-10-version-1607-and-later) to be enabled. The docs bundle contains deeply nested `.pnpm` paths that can exceed the 260-character `MAX_PATH` limit, which causes extraction to silently truncate files and the dev server to fail.

If you run `fern docs dev` on a Windows machine without long path support enabled, the CLI prints a warning with the command below. Run it in an elevated PowerShell prompt, then restart your terminal:
