...
- [pnpm](https://pnpm.io/installation), available on your `PATH`
- [The Fern CLI](/learn/cli-api-reference/cli-reference/overview#install-fern-cli)
</Info>

<Note>
`fern docs dev` uses pnpm internally to install the dependencies it needs to render your preview (for example, `esbuild`). pnpm must be installed globally on your machine even if your project uses npm or yarn — you don't need to change your project's package manager.
</Note>
