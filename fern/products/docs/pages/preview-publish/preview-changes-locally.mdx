...
|---------|----------------|
| `fern docs dev` | Not required |
| `fern generate --docs --preview` | Required |
| `fern generate --docs` | Required |

`fern docs dev` renders your site entirely on your machine, which is why [some features are disabled](#local-development) in local development. Both `fern generate --docs` variants publish to Fern's servers under your organization, so Fern has to know which organization you belong to: `--preview` publishes to a `*.docs.buildwithfern.com` URL, and `fern generate --docs` publishes to production. Locally, the CLI prompts you to authenticate. In CI, supply a `FERN_TOKEN` repository secret, generated from the [API keys](/learn/dashboard/configuration/api-keys) page in the Dashboard or by running [`fern token`](/learn/cli-api-reference/cli-reference/commands#fern-token).

`--preview` means two different things depending on whether `--docs` is present. `fern generate --docs --preview` publishes a hosted, shareable [docs preview link](#preview-links) and requires authentication. `fern generate --preview` without `--docs` is the [SDK workflow](/learn/cli-api-reference/cli-reference/overview#cli-workflows-by-product): it generates SDKs into a local `.preview/` folder instead of publishing them.
