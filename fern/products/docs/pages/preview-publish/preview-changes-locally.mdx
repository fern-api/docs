...
These workflows require a `FERN_TOKEN` [repository secret](https://docs.github.com/en/actions/security-guides/using-secrets-in-github-actions#creating-secrets-for-a-repository). If you used the guided workflow, this secret is added automatically. Otherwise, generate a token from the [Fern tokens](/learn/dashboard/configuration/fern-tokens) page in the Dashboard or by running [`fern token`](/learn/cli-api-reference/cli-reference/commands#fern-token) in your terminal, then add it in your repository's **Settings > Secrets and variables > Actions** with the name `FERN_TOKEN`.
