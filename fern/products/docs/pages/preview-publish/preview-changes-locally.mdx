...
Preview URLs follow the pattern `{org}-preview-{hash}.docs.buildwithfern.com`, where `{hash}` is either a generated UUID or the custom `--id` you specified.

#### List previews

Use `fern docs preview list` to display all active preview deployments for your organization, including URLs and creation timestamps:

```bash
fern docs preview list
```

Use `--limit` to control how many results appear per page, and `--page` to paginate:

```bash
fern docs preview list --limit 20 --page 2
```

#### Delete previews

Use `fern docs preview delete` to remove a preview deployment by its full URL or by its `--id`:

```bash
# Delete by full URL
fern docs preview delete https://acme-preview-abc123.docs.buildwithfern.com

# Delete by preview ID (the value passed to --id during generation)
fern docs preview delete --id my-feature
```

For automated cleanup in CI, see [Clean up preview links when PRs merge](#clean-up-preview-links-when-prs-merge).
