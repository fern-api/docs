...
### One domain per docs project

Each docs repository publishes its own site at its own `*.docs.buildwithfern.com` URL, and a custom domain serves whichever site published most recently with that domain declared. Declare a given custom domain in exactly one repository's `docs.yml`, unless the site is a [multi-source site](/learn/docs/preview-publish/multi-source-docs), where each repository claims a distinct basepath on the shared domain. Uniqueness is only enforced across organizations: a publish is blocked when the domain belongs to another organization, but two repositories in the same organization can both claim it.

<Warning>
When two repositories in one organization declare the same `custom-domain`, each publish overwrites the other, and the domain can start serving stale content from the wrong repository. Remove the `custom-domain` from every repository except the one that owns it, then republish that repository to restore the domain.
</Warning>
