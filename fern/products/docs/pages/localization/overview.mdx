...
<Tip>
  You only need to translate the files you want to localize. Any page that is registered in the localized navigation but has no matching `.mdx` falls back to the default-language file automatically.

  Adding a translated `.mdx` alone is not enough to localize a page — the page must also have an entry in the localized navigation YAML (Step 4). Without a localized nav entry, the page falls back to the default language even when a translation file exists.
</Tip>
