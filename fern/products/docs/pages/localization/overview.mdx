...
For each product (or version) file referenced above, create a localized counterpart under the same relative path and mirror its `navigation:` tree. Use `section:` and `page:` for translated display names, and add a `slug:` whenever you want to keep the base file's URL slug stable across languages (e.g., when the page slug is an English term like `markdown` or `api-catalog`):
