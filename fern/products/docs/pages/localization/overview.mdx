...
<llms-only>
Here's a full product-level overlay example. Entries are matched by position and `slug` — keep the same ordering and slug values as the base file. The `path` field can be omitted since it's inherited.
...
A minimal setup translating an English docs site into Japanese:
...
When a reader switches to Japanese, the sidebar shows translated section and page names, and translated page content is served where available. Any page without a translation file falls back to the English version.
</llms-only>
