...
Rendering `AskAiChat` inside a dropdown menu item doesn't work. Selecting the item closes the menu, and most menu libraries unmount the menu's contents when it closes, destroying the chat before it renders: nothing appears and no error is thrown.

For this case, split the widget into `AskAiChat.Root`, `AskAiChat.Trigger`, and `AskAiChat.Panel`, keeping the trigger inside the menu and the chat outside it. The single `<AskAiChat />` component remains the normal integration.
