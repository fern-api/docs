...
`trigger="prompt"` replaces the corner launcher with a single-line input pinned to the bottom center of the viewport, so visitors type their question into the page instead of opening a panel first. It grows on hover and focus, and sending a question opens the chat directly above it with that question already in flight.

```tsx
<AskAiChat domain="https://docs.example.com" trigger="prompt" />
```

Interacting with the page outside the chat closes it, and selecting the prompt again reopens the same conversation. Below 768px the chat is the same bottom drawer the corner launcher opens. `placement` doesn't apply in this mode, and the chat is wider than the corner card: override it with `--fern-ask-ai-prompt-card-width`.
...
<ParamField path="trigger" type="'floating' | 'prompt' | 'inline' | 'none'" default="floating">
  `floating` renders a styled launcher pinned to a corner of the viewport. `inline` renders an unpainted button to place and style yourself, such as in a nav bar. Both carry the sparkle icon and an `Ask AI` label by default. `prompt` renders the composer itself, centered at the bottom of the viewport, and the chat rises from it on send: see [Bottom prompt](#bottom-prompt). `none` renders no trigger at all, for [driving the open state from your own UI](#custom-triggers).
</ParamField>
