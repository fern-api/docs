...
`trigger="prompt"` replaces the corner launcher with a single-line input pinned to the bottom center of the viewport. Sending a question opens the chat directly above the input with that question already in flight.

```tsx
<AskAiChat domain="https://docs.example.com" trigger="prompt" />
```

`placement` doesn't apply in this mode. Set the chat width with `--fern-ask-ai-prompt-card-width`.
