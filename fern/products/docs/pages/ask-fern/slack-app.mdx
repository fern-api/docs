...
Installing the app binds Ask Fern to one docs domain. A channel can override that domain, so a single workspace can answer from a different docs site in each channel. Channels without an override use the install domain.

Overrides are set through an HTTP endpoint rather than a slash command:
...
`team_id` is the Slack workspace the app is installed in, and `channel_id` is the channel to configure. The response reports the stored `domain_override` and the `effective_domain` Ask Fern queries in that channel. Sending `"domain": null` clears the override and restores the install domain.
