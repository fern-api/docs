...
| **show** | Show the current settings, including the docs domain the channel answers from | `/fern show` |
...
### Docs domain per channel

By default, Ask Fern answers from the domain you supplied when you [generated the install link](#setup). If your organization publishes more than one docs site, override that domain in a specific channel to have it answer from a different site. Channels without an override use the install domain.

Set overrides through an HTTP endpoint:

```bash
curl -X POST https://fai.buildwithfern.com/slack/channel-domain-override \
     -H "Authorization: Bearer <YOUR_FERN_TOKEN>" \
     -H "Content-Type: application/json" \
     -d '{
       "team_id": "T0123456789",
       "channel_id": "C0123456789",
       "domain": "plants.docs.buildwithfern.com"
     }'
```

| Field | Description |
|-------|-------------|
| **team_id** | The Slack workspace the app is installed in |
| **channel_id** | The channel to configure |
| **domain** | The docs domain the channel answers from. Send `null` to clear the override and restore the install domain. |

The response reports the stored `domain_override` and the `effective_domain` Ask Fern queries in that channel.

The Fern token must belong to an organization that owns the override domain: requests for a domain in another organization return `403`.
