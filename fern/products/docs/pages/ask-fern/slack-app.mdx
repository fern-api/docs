---
title: Ask AI Slack app
description: Enable your customers to get instant answers to product questions directly in Slack using Ask AI's AI-powered documentation bot.
---

The Ask AI Slack app allows customers to ask questions about your products directly in Slack channels and receive AI-generated answers from your documentation database.
...
Install the Ask AI app in your workspace and add the bot to customer channels.

<Note>
  To install Ask AI in your organization's Slack workspace, you must be a Slack
  admin.
</Note>
...
You'll be redirected to Slack to authorize the Ask AI app. Select the workspace where you want to add Ask AI and click **Allow**.

<Frame>
  <img src="assets/allow-slack.png" alt="Allow Ask AI for Slack workspace" />
</Frame>

</Step>
<Step title="Add to customer channels">

Once you've installed it in your workspace, add the bot to customer Slack channels to give it access. Customers will see that `@Ask AI was added to the channel` and can start asking questions immediately.
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| **respond_to** | Controls whether the Ask AI bot responds to all messages (`all`), reponds only when directly mentioned with `@Ask AI` (`mentions_only`), or determines when to respond to messages depending on context (`auto`). Set to `auto` by default. | `/fern respond_to all`        |
| **roles**      | Specifies which RBAC roles (comma-separated) should be used to filter Ask AI responses (if you have [role-based access control](/learn/docs/authentication/features/rbac) configured)                                                       | `/fern roles developer,admin` |
| **show**       | Show the current settings                                                                                                                                                                                                                   | `/fern show`                  |
| **help**       | Get help with Ask AI slash commands                                                                                                                                                                                                         | `/fern help`                  |
...
1. In Slack, go to **Apps** in the sidebar and click **Ask AI**
...
**User –** _@PlantSupport_ what's the best way to care for succulents
indoors?
...
**Support Lead –** _@PlantSupport_ This is good but you should mention drainage holes and the
...
When a user asks Ask AI a question in Slack, a webhook triggers Fern's servers to search your documentation database and retrieve relevant context. Using that context, Ask AI generates a response.
