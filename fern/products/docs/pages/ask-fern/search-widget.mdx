...
Instead, split the widget with `AskAiChat.Root`, `AskAiChat.Trigger`, and `AskAiChat.Panel`. The trigger lives inside the menu, where unmounting is harmless, and the chat lives outside it:
...
*  `AskAiChat.Panel` is the chat. Render exactly one per root, outside any menu. A root with a trigger but no panel logs a console warning.
...
`open` is controlled in the usual React sense: the widget renders what you give it, so `onOpenChange` must be wired back to your state or the chat's own controls (its close button, `Escape`, and `cmd`/`ctrl` + `/`) will appear to do nothing. Pass neither prop and the widget manages its own state; `onOpenChange` on its own works as a notification hook.
