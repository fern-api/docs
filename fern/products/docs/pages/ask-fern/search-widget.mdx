...
*  `AskAiChat.Root` holds the state and renders no DOM of its own. It accepts the same props as `AskAiChat` except `trigger`. Wrap it around both the menu and the panel.
...
This shape also covers a popover, a command palette, or a nav bar dropdown, and a single root supports multiple triggers.
