...
`SearchModal` renders a trigger button that opens a modal over the page, combining keyword search with AI chat. It's one of the two components in the [`@fern-api/search-widget` package](/learn/docs/ai-features/ask-fern/search-widget/overview); install the package and review the prerequisites there first.
