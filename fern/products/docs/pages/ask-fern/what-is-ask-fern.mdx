...
Ask Fern is Fern's AI Search feature, powered by **Claude 4.7 Sonnet** and **Claude 3.5 Haiku** with **Retrieval Augmented Generation (RAG)**, that indexes your documentation and provides an interface for your end users to ask questions and get answers. Responses include citations that link directly to source pages.
...
3. **Response generation** — Ask Fern sends the retrieved chunks as context to Claude 4.7 Sonnet to generate answers with citations. If the initial context isn't sufficient, it performs an additional keyword search.
