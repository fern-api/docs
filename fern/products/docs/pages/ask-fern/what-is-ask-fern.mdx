...
The end-user queries that power Ask Fern analytics are retained indefinitely by default. To purge them on a schedule, open your site's **Settings** page in the [Fern Dashboard](https://dashboard.buildwithfern.com/), enable **Automatically purge old query logs** on the **Query log retention** card, and set a retention period between 1 and 3650 days. Queries older than that period are purged and no longer appear in analytics.

Retention applies to every docs site in your organization. Changing it requires [Admin permissions](/learn/dashboard/configuration/permissions).
</Accordion>
