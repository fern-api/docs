...
<Steps>
  <Step title="Enable in the Dashboard">
    ...
    Ask Fern is a single switch that turns on every AI surface at once: the sidebar chat panel, the Ask AI trigger, the AI mode in search, and the Ask AI buttons on code snippets. The [`page-actions` options](/learn/docs/configuration/site-level-settings#page-actions-configuration) control only the page action buttons, so they can't hide these surfaces individually.
