...
Fern also supports `Accept` header content negotiation on every documentation page. When an AI tool or agent sends a request with `Accept: text/markdown` or `Accept: text/plain`, Fern returns the page content as clean Markdown instead of HTML. This works on any page URL — no special endpoints needed.

```bash
# Get any docs page as Markdown
curl -H 'Accept: text/markdown' https://your-docs-site.com/getting-started

# Also works with text/plain
curl -H 'Accept: text/plain' https://your-docs-site.com/api-reference/create-plant
```

This is enabled automatically for all Fern docs sites with no configuration required. Combined with `llms.txt`, this means AI tools can access your documentation as Markdown whether they request a specific page or discover content through the `llms.txt` index.
