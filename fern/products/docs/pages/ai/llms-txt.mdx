...
Use the `llms` key in `docs.yml` to automatically prepend or append text to every page's Markdown when served via LLM endpoints. This applies to individual page Markdown (`.md`/`.mdx` URLs) and to each page section within `llms-full.txt`.

```yaml docs.yml
llms:
  page-directive-prepend: "For a complete page index, fetch https://docs.example.com/llms.txt"
  page-directive-append: "End of page content."
```

<ParamField path="page-directive-prepend" type="string">
...
<ParamField path="page-directive-append" type="string">
  Text appended after the page content. Use this to add citations, disclaimers, or navigation hints at the end of every page.
</ParamField>

This replaces the need to manually include a snippet on every page. For example, instead of adding `<Markdown src="/snippets/agent-directive.mdx"/>` to each MDX file, configure a single `page-directive-prepend` in `docs.yml` and the directive is injected automatically.
