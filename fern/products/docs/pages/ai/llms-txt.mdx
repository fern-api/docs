...
### Link to SDK documentation for LLMs

Help users feed your SDK documentation to LLMs by adding a button that links to a filtered `llms-full.txt`. This is useful when users want to use AI tools to write code with your SDK instead of raw HTTP requests.

```jsx Markdown
<Button href="/api-reference/llms-full.txt?lang=python&excludeSpec=true" target="_blank">
  Open Python SDK docs for LLMs
</Button>
```

The `lang` parameter filters SDK code examples to a specific language, and `excludeSpec=true` excludes the raw OpenAPI specification to reduce token usage.
