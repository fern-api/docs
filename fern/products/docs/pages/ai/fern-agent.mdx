...
Once your automation is created, select **Run now** to test the prompt. Manually running an automation still leaves the next scheduled run where it was. Runs take a few minutes, and **Past runs** lists each one with a link to its conversation, where the report and any pull request are.
</Step>
</Steps>

When you [connect Fern Agent to Slack](#setup), every finished run is reported in Slack: the automation's name, whether the run finished or failed, and a link to its conversation. The report is a direct message to whoever created the automation, which requires their Slack account to be linked to `@Fern`. 
