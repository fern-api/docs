...
Fern serves clean Markdown for any documentation page so AI agents and external tooling can consume your content efficiently. Agents fetch the source Markdown (excluding frontmatter) by appending `.md` or `.mdx` to any page URL, or by sending an `Accept: text/markdown` header via [content negotiation](https://developer.mozilla.org/en-US/docs/Web/HTTP/Content_negotiation).

Both methods work for normal and API Reference pages, and combined with [`llms.txt`](/learn/docs/ai-features/llms-txt) reduce token consumption by 90%+ compared to HTML.
