...
When an agent requests a `.md` URL that doesn't exist, the response includes up to three suggested pages under a `## Similar pages` heading, each linking to the closest matching `.md` source. This mirrors the "Were you looking for one of these?" suggestions human readers see on the 404 page, so agents can recover from a broken or outdated link without a separate lookup.
