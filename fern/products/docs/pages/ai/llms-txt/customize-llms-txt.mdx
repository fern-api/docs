...
Fern's own [docs quickstart](https://github.com/fern-api/docs/blob/main/fern/products/docs/pages/getting-started/quickstart.mdx) uses `<llms-only>` to give AI agents an executable CLI equivalent alongside the human-facing UI walkthrough. The snippet below is from the "Initiate your docs" step:

````mdx quickstart.mdx
Use the `fern-api/docs-starter` repository as a template for your site:
  1. Navigate to [fern-api/docs-starter](https://github.com/fern-api/docs-starter) and click **Use this template**.
  2. Choose **Create a new repository** and name it `fern-docs`.
...
Human readers see just the UI walkthrough. AI agents receive the walkthrough plus the executable `gh` command — the same task, expressed in a form an agent can run directly.

`<llms-ignore>` works the opposite way. Wrap marketing callouts, decorative UI, or other human-only content so it renders on your documentation site but is stripped from the Markdown output consumed by agents.
