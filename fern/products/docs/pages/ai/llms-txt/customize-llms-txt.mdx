...
Filter `llms.txt` output with the `lang` and `excludeSpec` query parameters to reduce token usage. Parameters can also be combined. The `lang` parameter also applies to per-page `.md` and `.mdx` URLs.
