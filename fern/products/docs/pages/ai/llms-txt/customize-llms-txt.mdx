...
Filter `llms.txt` output with the `lang`, `excludeSpec`, and `excludeChangelog` query parameters to reduce token usage. Parameters can also be combined. The `lang` parameter also applies to per-page `.md` and `.mdx` URLs.
...
<ParamField path="lang" type="'node' | 'python' | 'java' | 'ruby' | 'go' | 'csharp' | 'swift' | 'php'">
...
<ParamField path="excludeChangelog" type="boolean">
  Exclude changelog sections, including the nested changelog `llms.txt` index. Useful for sites with long release histories, where changelog entries can dominate the index.
</ParamField>
