...
`<llms-only>` content appears only on the LLM-serving endpoints that external agents (like Claude, ChatGPT, or Cursor) fetch directly: `/llms.txt`, `/llms-full.txt`, and each page's [`.md`/`.mdx` source](/learn/docs/developer-tools/view-markdown). It is stripped from every human-facing surface, including the rendered page, [Copy page](/learn/docs/configuration/site-level-settings#page-actions-configuration), the search widget, and [Ask Fern](/learn/docs/ask-fern/overview) — so Ask Fern won't retrieve or cite `<llms-only>` content. `<llms-ignore>` does the reverse: it's shown on human-facing surfaces and stripped from the LLM-serving endpoints.
