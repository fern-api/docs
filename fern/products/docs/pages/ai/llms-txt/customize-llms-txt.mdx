...
**UI-only elements belong to human readers, and their programmatic equivalents belong to AI agents.** Wrap clickable cards and UI walkthroughs in `<llms-ignore>`, and pair each with an `<llms-only>` block giving the CLI or API equivalent. Beyond that pairing, `<llms-only>` suits verbose technical context that would clutter the human UI, such as implementation details, architecture notes, or cross-references that help AI understand relationships between pages. `<llms-ignore>` suits marketing CTAs, navigation hints, and comments meant to stay in source files.
