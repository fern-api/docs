...
To fully disable the MCP server — so the endpoint returns 404, AI agents can't discover it via `llms.txt` or agent prompts, and the connection buttons are hidden — set `mcp` to `false` in your [`page-actions` configuration](/learn/docs/configuration/site-level-settings#page-actionsoptions.mcp):
...
This is useful on [authenticated sites](/learn/docs/authentication/overview) where unauthenticated AI agents discover the endpoint but can't access it, generating support requests from users.
