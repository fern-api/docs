...
The server's `searchDocs` tool queries your documentation through Ask Fern, so it requires Ask Fern to be enabled. With Ask Fern disabled, the server still lists its tools but every `searchDocs` call fails. [Markdown access](/learn/docs/ai-features/markdown) through `llms.txt`, `llms-full.txt`, and per-page `.md` URLs has no such requirement.
...
The server scopes every result to the caller's roles, so an unauthenticated caller only receives content visible to everyone. There is no anonymous access to protected content.
