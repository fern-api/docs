...
The install command is also available from the **Install skills** action in the [page actions toolbar](/learn/docs/configuration/site-level-settings#install-skills-action) on any Fern docs site that configures it. On this site, open the page actions menu on any page to copy the command directly.
...
| `references/snippets.md` | Editing the same content across multiple pages, or adding generic/boilerplate content (e.g., a support footer or prerequisites block). |

This routing keeps context small. An agent writing a changelog entry loads `references/changelog.md` and skips the other references entirely.
