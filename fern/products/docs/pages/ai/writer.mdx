...
Fern Writer accesses Slack messages only in channels where it has been added, and only when directly tagged. Message content is used solely to generate documentation pull requests and isn't stored beyond the duration of a session.

Fern Writer is powered by [Devin](https://docs.devin.ai/admin/security#how-is-your-data-used-to-improve-devin), which doesn't use your data to train AI models. Fern explicitly configures Devin to opt out of any model training. Your channel messages, code, and documentation content are never used for training purposes.
