...
<Note>
Already keeping a hand-built bundle under `fern/.well-known/skills/` or `fern/.well-known/agent-skills/`? Those files are still uploaded as-is when no `path` is set, so nothing breaks. Move the skills into their own directory and declare `path` to have Fern validate them and generate the manifest for you — a declared `path` takes precedence over the hand-built folder.
</Note>
