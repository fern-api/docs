...
Include your basepath if you have one, as in `npx skills add https://example.com/docs`. The `https://` scheme is required: without it the argument is read as a Git source, so a bare domain is cloned and `domain/basepath` is parsed as a GitHub `owner/repo` shorthand.

  </Step>
  <Step title="Customize the Install skills button">

Setting `page-actions.options.skills` already adds an "Install skills" button to the [page action bar](/learn/docs/configuration/site-level-settings#page-actions-configuration). The button opens a modal with a copyable install command and your skill list (read from the generated manifest). Customize the modal with `title`, `description`, `learn-more-url`, and an optional `repository` link to the skill source:
...
By default the modal shows `npx skills add https://<your-docs-domain>`. Override it with `install-command` when that command won't work as-is — for example auth-walled docs, a private registry, a multi-step install, or a site served under a basepath, which the generated command leaves out:
...
## Serve a hand-built bundle

If you maintain the discovery manifest yourself, place the bundle in your `fern/` folder under `.well-known/agent-skills/` (the current v0.2.0 spec) or `.well-known/skills/` (the legacy v0.1.0 layout) and leave `path` unset. Fern uploads the files as-is during `fern generate --docs` and validates each `SKILL.md` against the same name and description rules. When `path` is set, the declared directory wins and a hand-built `.well-known/skills/` folder is ignored with a warning.

The v0.2.0 `index.json` enumerates every skill by `url` and `digest`:
