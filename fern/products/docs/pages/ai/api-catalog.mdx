...
You can test the endpoint on any Fern docs site. For example, this site's catalog is at [buildwithfern.com/learn/.well-known/api-catalog](https://buildwithfern.com/learn/.well-known/api-catalog):

```bash
curl -s https://buildwithfern.com/learn/.well-known/api-catalog | jq .
```

For sites with a basepath like `/docs`, use `/.well-known/api-catalog` under that basepath (e.g. `https://example.com/docs/.well-known/api-catalog`).
