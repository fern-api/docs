...
In self-hosted mode, [on-page feedback](/learn/docs/user-feedback) events are emitted as structured JSON logs to the container's stdout. Each log line is prefixed with `[fern-docs-feedback]` so you can filter for feedback in your logging infrastructure.
...
To find feedback events in your container logs, filter for lines containing the `[fern-docs-feedback]` prefix:
