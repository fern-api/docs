...
`fern check` validates skill bundles: `index.json` must exist and parse, and each `SKILL.md` frontmatter needs a kebab-case `name` (max 64 characters) matching its parent directory and a non-empty `description` (max 1,024 characters).

<Button intent="none" outlined rightIcon="arrow-right" href="/learn/docs/ai-features/agent-skills#host-custom-skills-from-your-docs-site">Read the docs</Button>
