## Build a docs version from a git ref

<ChangelogTags>navigation, docs.yml</ChangelogTags>

You can now point a version entry at a git `ref` (a branch, tag, or commit SHA) instead of a `path` in your working tree. On publish, Fern builds that version's pages, navigation, assets, and API definitions from the ref, composed with the current branch's theme and shell. Use it for snapshot versions that are frozen at a release; keep actively maintained versions on `path`, since `fern docs dev` previews working-tree versions only.

```yaml docs.yml {5}
versions:
  - display-name: Latest
    path: ./versions/latest.yml
  - display-name: v2
    ref: v2.0.0
    availability: deprecated
```

<Button intent="none" outlined rightIcon="arrow-right" href="/learn/docs/configuration/versions#build-a-version-from-a-git-ref">Read the docs</Button>
