...
Use query parameters to customize the output of `llms.txt` and `llms-full.txt` files, reducing token usage by filtering to only relevant content.

### Filter by SDK language

The `lang` parameter filters SDK code examples to show only the specified language:

```
/llms-full.txt?lang=python
```

Supported languages include `node`, `python`, `java`, `ruby`, `go`, `csharp`, and `swift`. Common aliases are also accepted: `javascript`, `typescript`, `js`, `ts`, `py`, and `golang`. The parameter is case-insensitive.

### Exclude API specifications

The `excludeSpec` parameter excludes OpenAPI and AsyncAPI specification sections:
...
Both parameters can be combined to further reduce token usage:
