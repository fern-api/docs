...
Use the `lang` and `excludeSpec` query parameters to customize the output of `llms.txt` and `llms-full.txt` files, reducing token usage by filtering to only relevant content. Parameters can be combined to further reduce token usage.
