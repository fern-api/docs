...
`:slug` and `:slug*` only match whole path segments, so they must follow a `/`. To match mid-segment, give the parameter an explicit pattern in parentheses:

<CodeBlock title="docs.yml">
  ```yml
  redirects:
    - source: "/plants/legacy-:slug(.*)" # Matches /plants/legacy-monstera
      destination: "/plants/:slug(.*)"
  ```
</CodeBlock>

A `(.*)` pattern also captures `/` characters, so it matches across segments. Repeat the same pattern on the parameter in `destination`.
