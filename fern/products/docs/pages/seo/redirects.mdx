...
Instead, [enable automatic homepage redirects](/learn/docs/configuration/site-level-settings#settings-configuration) so broken links land on your homepage rather than a 404 page:
...
Don't redirect from unversioned to versioned URLs. If you have [versions](/learn/docs/configuration/versions) configured, your default version uses unversioned paths (`/docs/getting-started`) while other versions use versioned paths (`/docs/v2/getting-started`), and Fern routes between them automatically by redirecting broken versioned links to the default version and managing canonical URLs.
