...
### Match part of a path segment

A parameter can start mid-segment, but `:slug` stops at the next `/`:

<CodeBlock title="docs.yml">
  ```yml
  redirects:
    - source: "/plants/legacy-:slug" # Matches /plants/legacy-monstera, not /plants/legacy-monstera/care
      destination: "/plants/:slug"
  ```
</CodeBlock>

`:slug*` requires a leading `/` and doesn't work mid-segment. To capture the rest of the path from mid-segment, give the parameter an explicit pattern in parentheses, and repeat that pattern in `destination`:

<CodeBlock title="docs.yml">
  ```yml
  redirects:
    - source: "/plants/legacy-:slug(.*)" # Matches /plants/legacy-monstera/care
      destination: "/plants/:slug(.*)"
  ```
</CodeBlock>
