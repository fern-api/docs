...
| Pattern | Captures | Example `source` | Matches |
|---------|----------|------------------|---------|
| `:slug` | A single path segment. Must follow a `/`. | `/plants/:slug` | `/plants/monstera`, not `/plants/indoor/monstera` |
| `:slug*` | Zero or more path segments, including the separating slashes. Must follow a `/`. | `/plants/:slug*` | `/plants` and `/plants/indoor/monstera` |
| `:slug+` | One or more path segments. Must follow a `/`. | `/plants/:slug+` | `/plants/indoor/monstera`, not `/plants` |
| `:slug(regex)` | Whatever the regular expression matches. Can start mid-segment and span `/`. | `/plants-:slug(.*)` | `/plants-indoor/monstera` |
| `(regex)` | An unnamed group, replayed in `destination` as `:0`. | `/plants/(\d+)` | `/plants/42` |

Three behaviors aren't visible from the syntax itself:

- **A regular expression is the only way to match mid-segment.** `:slug*` and `:slug+` must follow a `/`, and a plain `:slug` never spans one. `/plants-:slug` matches `/plants-monstera` but not `/plants-monstera/care`; `/plants-:slug(.*)` matches both.
- **The regular expression must be repeated in `destination`.** A parameter written as `:slug(.*)` in `source` stays `:slug(.*)` in `destination`, because a bare `:slug` can't hold a value containing `/`. Omitting it resolves the redirect to the literal text `/new-folder/:slug`.
...
A regular expression narrows a parameter as well as widening one: `/v:version(\d+)/plants/:slug*` matches `/v2/plants/monstera/care` and captures `version` as `2`.
