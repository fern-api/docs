...
Fern automatically generates a URL slug for each navigation item and joins them into every page's full URL. You can [rename](#renaming-slugs) a slug, [skip](#skipping-slugs) a level, or [override](#override-with-a-frontmatter-slug) a page's path in its frontmatter.
...
You can override any segment by [renaming](#renaming-slugs) or [skipping](#skipping-slugs) it. Where you set a `slug` determines how much of the URL it replaces: a `slug` in `docs.yml` replaces only that item's own segment, while a `slug` in a page's [frontmatter](#override-with-a-frontmatter-slug) replaces the page's entire section and folder hierarchy. Product and version prefixes are always preserved. When a page sets both, the frontmatter `slug` takes precedence.

This example sets a `slug` in both places for the same page. With slugs only in `docs.yml`, each one replaces its own segment:
...
The **Quickstart** page resolves to `plantstore.docs.buildwithfern.com/start/quick`. Adding a `slug` to the page's frontmatter replaces the section and folder hierarchy:
...
Use a frontmatter `slug` when a page needs a short, memorable URL independent of its sidebar position — a top-level quickstart, a campaign landing page, or a page that moved but must keep its old URL. A frontmatter `slug` [overrides a `docs.yml` slug](#overriding-slugs) and replaces the page's entire section and folder hierarchy, so the page still appears in the sidebar under its section but its URL no longer reflects that structure.
