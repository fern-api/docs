...
- An **article** entry (`TechArticle`) with the page title, description, canonical URL, language, and site name. These come from the `title`, `headline`, `description`, and `canonical-url` fields and can't be overridden separately.
- A **breadcrumb** entry (`BreadcrumbList`) that mirrors the page's position in your navigation. Top-level pages don't get one.

Pages marked `noindex` receive no structured data. Verify a published page's output with the [Schema Markup Validator](https://validator.schema.org).
