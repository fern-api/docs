...
- An **article** entry (`TechArticle`) with the page title, description, canonical URL, language, and site name. These come from the `title`, `headline`, `description`, and `canonical-url` fields and has no override of its own.
