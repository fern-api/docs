...
Every indexable page includes a `<script type="application/ld+json">` tag with [schema.org](https://schema.org) structured data: a `BreadcrumbList` built from the page's position in your navigation and a `TechArticle` carrying the page title, description, canonical URL, language, and site name. Pages marked `noindex` emit no structured data. Verify the output with the [Schema Markup Validator](https://validator.schema.org).

To replace the generated breadcrumb trail, set `jsonld:breadcrumb` in the page frontmatter to a complete `BreadcrumbList` object:
...
The `TechArticle` node is derived from the page's `title`, `headline`, `description`, and `canonical-url` fields and has no separate override.
