...
Every indexable page includes a `<script type="application/ld+json">` tag with [schema.org](https://schema.org) structured data: a `TechArticle` derived from the page's `title`, `headline`, `description`, and `canonical-url` fields plus the site language and name, and, for pages nested in a section, a `BreadcrumbList` built from the page's position in your navigation. Pages marked `noindex` emit no structured data. Verify the output with the [Schema Markup Validator](https://validator.schema.org).

The `TechArticle` node has no override. To replace the generated breadcrumb trail, set `jsonld:breadcrumb` in the page frontmatter to a complete `BreadcrumbList` object:
