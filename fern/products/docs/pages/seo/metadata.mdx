...
Structured data is a machine-readable summary of a page that search engines and AI tools use to display rich results, such as breadcrumb trails under a search listing. Fern adds it to every indexable page automatically in the [JSON-LD](https://json-ld.org) format, using the [schema.org](https://schema.org) vocabulary:

- An **article** entry (`TechArticle`) with the page title, description, canonical URL, language, and site name. Values come from the `title`, `headline`, `description`, and `canonical-url` fields; there is no dedicated override.
- A **breadcrumb** entry (`BreadcrumbList`) that mirrors the page's position in your navigation. Top-level pages don't get one.

Pages marked [`noindex`](#indexing) receive no structured data. Setting `jsonld: false` in the page frontmatter also suppresses all built-in JSON-LD for that page, which is useful when a custom component injects its own structured data:
...
Verify a published page's output with the [Schema Markup Validator](https://validator.schema.org).

To replace the generated breadcrumb trail, set `jsonld:breadcrumb` in the page frontmatter to a complete `BreadcrumbList`:
...
jsonld:breadcrumb:
  ...
  itemListElement:
    ...
    - "@type": ListItem
      position: 2
      name: Authentication
      item: https://docs.example.com/guides/authentication
---
```
