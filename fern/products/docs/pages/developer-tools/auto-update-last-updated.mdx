...
<Steps>
  <Step title="Choose a token">
    The workflow pushes its stamp commit with one of two tokens:

    - **Personal access token (recommended).** The stamp push triggers the PR's other workflows the same way a human push does, so `fern check` and preview deploys run automatically.
    - **`GITHUB_TOKEN`.** No secret to create, but GitHub holds every `pull_request` run the stamp push triggers behind an **Approve workflows to run** banner until someone with write access approves it. No repository or organization setting turns this off. Use it only when the repository runs no other checks on docs PRs.
  </Step>
  <Step title="Create the token">
    Under **Settings > Developer settings > Personal access tokens**, create a fine-grained token scoped to your docs repository with **Contents: Read and write**. Classic tokens with the `repo` scope also work.

    Add it under **Settings > Secrets and variables > Actions** as `DOCS_BOT_TOKEN`. A token your repository already uses for release or sync automation works too, as does a [GitHub App installation token](https://github.com/actions/create-github-app-token), which avoids per-user expiry on team-maintained repositories.

    Skip this step if you chose `GITHUB_TOKEN`.
  </Step>
...
The stamp commit is itself a `synchronize` event, so this workflow runs a second time on its own commit. That run stamps the same date, finds no diff, and exits without committing.
