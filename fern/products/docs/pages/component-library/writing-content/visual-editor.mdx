...
The Editor's AI agent, which drafts and edits content from a prompt, reads `AGENTS.md` from your repository root and from your `fern` folder before it makes any edit, then follows the conventions those files define: style guide, terminology, file layout, frontmatter, and navigation rules. Repositories without an `AGENTS.md` fall back to the agent's defaults. The agent treats `AGENTS.md` as conventions rather than instructions, so it never writes outside the `fern` folder.

```markdown AGENTS.md
## Voice

Write in second person, present tense. Use sentence case for headings.

## Terminology

Say "plant profile", not "plant record".

## Layout

Guides live in `fern/pages/guides/`. Every new page needs a `docs.yml` navigation entry.
```

Beyond your conventions, the agent groups pages by reader task, keeps navigation shallow, and recommends a different structure when a request would make the site harder to navigate.
