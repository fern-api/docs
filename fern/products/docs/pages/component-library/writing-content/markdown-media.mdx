...
| `poster` | URL or path to an image shown while the video is loading. If not set, the browser shows nothing until the first frame is available, then displays it as the poster frame. Accepts the same path formats as `src`. |
