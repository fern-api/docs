...
You can reference external URLs to pull code samples directly from remote sources like GitHub. This is useful for keeping documentation in sync with code that lives in other repositories. The `<Code>` component supports the same props as `<CodeBlock>`, including `title`, `language`, and `maxLines`.
