...
By default, code blocks display line numbers in the gutter. To hide line numbers, set `showLineNumbers` to `false`:
...
For bash, shell, and CLI code blocks, the gutter displays command prompt symbols (`$` for new commands, `>` for continuation lines) instead of line numbers. To hide all line prefixes including both line numbers and these prompt symbols, use `hideLinePrefixes`:
...
This differs from `showLineNumbers={false}`, which only hides line numbers but still displays the `$` and `>` symbols in bash/shell code blocks:
...
<ParamField path="showLineNumbers" type="boolean" required={false} default="true">
  Whether to display line numbers in the gutter. Set to `false` to hide line numbers.
</ParamField>

<ParamField path="hideLinePrefixes" type="boolean" required={false} default="false">
  Whether to hide all line prefixes, including line numbers and command prompt symbols (`$` and `>`) in bash/shell code blocks. Use this for a cleaner look in command examples.
</ParamField>
