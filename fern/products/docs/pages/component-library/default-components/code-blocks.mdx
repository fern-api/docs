...
Use the `lines` prop to extract specific lines from a source file. This is useful when you want to highlight a particular function or section without showing the entire file.
...
The `lines` prop supports single lines, ranges, and combinations:

- `lines={[5]}` - Extract line 5
- `lines={[2-4]}` - Extract lines 2 through 4 (inclusive)
- `lines={[1,3,5]}` - Extract lines 1, 3, and 5
- `lines={[1-3,5,7-10]}` - Extract lines 1-3, line 5, and lines 7-10

Lines are always returned in order, and duplicates from overlapping ranges are automatically removed.
...
<ParamField path="lines" type="string" required={false}>
