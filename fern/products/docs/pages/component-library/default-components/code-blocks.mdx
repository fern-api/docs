...
### Embed external code files

You can reference external URLs to pull code samples directly from remote sources like GitHub. This is useful for keeping documentation in sync with code that lives in other repositories.

<div className="highlight-frame">
<Code src="https://raw.githubusercontent.com/fern-api/docs-starter/refs/heads/main/.github/workflows/check.yml" />
</div>
...
The `<Code>` component supports the same props as `<CodeBlock>`, including `title`, `language`, and `maxLines`. This allows you to customize how embedded code files are displayed.
