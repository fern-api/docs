...
By default, the prompt body wraps long lines and shows up to five lines of content with vertical scrolling beyond that. Use `singleLine` for a compact truncated preview, or `hidePrompt` to show only the title row with copy and open-in actions.
...
<div className="highlight-frame">
  ...
  <Prompt
    ...
    singleLine
  >
  You are a migration assistant. Walk the user through upgrading from v1 to v2 of the Plant Store SDK. Check each breaking change listed in the changelog, update imports, and verify tests pass.
  </Prompt>
  </div>
</div>
...
<div className="highlight-frame">
  ...
  >
  You are a docs setup assistant. Help the user create a new Fern docs site from scratch. Install the Fern CLI, initialize the project with `fern init`, configure navigation in docs.yml, add a quickstart page, and publish to a preview URL.
  </Prompt>
  </div>
</div>
