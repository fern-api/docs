...
When using `toc={true}` with heading-based steps, the heading level determines how steps appear in the table of contents. For example, `##` headings appear at depth 2 in the table of contents (nested under any `#` headings), while `###` headings appear at depth 3 (nested under `##` headings).
...
<ParamField path="toc" type="boolean" required={false}>
  ...
  When using markdown headings inside `<Steps>`, the heading level (e.g., `##` or `###`) determines the depth at which steps appear in the table of contents. This lets you control the nesting hierarchy of steps by choosing the appropriate heading level.
</ParamField>

<ParamField path="tocDepth" type="number" required={false}>
  The depth at which steps appear in the table of contents when `toc` is enabled. Accepts values `1`, `2`, or `3`.

  When using markdown headings inside `<Steps>`, this is automatically set to match the heading level. When using explicit `<Step>` components, you can set this manually to control how steps nest in the table of contents. Defaults to `3` if not specified.
