...
<ParamField path="endpoint" type="string" required={true}>
  The endpoint to display, in the format `METHOD /path` (e.g., `POST /chat/{domain}`). If your API uses [namespaces](/learn/sdks/reference/generators-yml#namespace), prefix with the namespace and `::` (e.g., `myapi::POST /chat/{domain}`).
</ParamField>
