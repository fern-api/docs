...
You can combine `searchable` with `sticky`, `paginate`, and `expandable` on a single table:

```jsx Markdown
<table searchable sticky paginate expandable pageSize={6} className="fern-table">
  ...
</table>
```
...
Add the `paginate` attribute to an HTML table element and set `pageSize` for the number of rows per page (defaults to 10). Add `expandable` to replace the previous/next footer with Show all / Show fewer controls.

<div className="highlight-frame">
  <div className="highlight-frame-content-sticky">
  <table paginate pageSize={6} className="fern-table">
    <thead>
      <tr>
        <th>Plant</th>
        <th>Light Requirements</th>
...
The wrapper components don't include a combined sticky and paginated variant. To make a paginated table sticky, use the raw `<table>` attributes:

```jsx Markdown
<table sticky paginate pageSize={6} className="fern-table">
  ...
</table>
```
