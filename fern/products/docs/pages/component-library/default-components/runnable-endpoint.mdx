...
<ParamField path="collapsed" type="boolean" required={false} default="false">
  When set to `true` (or just `collapsed`), the component renders collapsed by default with the form section hidden. Users can expand it by clicking the component.
</ParamField>
