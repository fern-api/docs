...
<ParamField path="readonly" type="string[]" required={false}>
  List of fields to lock by removing their dropdown selectors. For example, `readonly={["environment"]}` removes the environment dropdown, locking users to the selected environment (set via `defaultEnvironment` or the first environment by default).
</ParamField>
