...
<ParamField path="collapsed" type="boolean" required={false}>
  When set to `true`, the component renders collapsed by default with the form section hidden. Users can expand it by clicking the component. Defaults to `false` (expanded).
</ParamField>
