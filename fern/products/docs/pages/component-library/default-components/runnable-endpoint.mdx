...
<ParamField path="endpoint" type="string" required={false}>
  The endpoint locator in the format "METHOD /path". For example: `"POST /api/users"` or `"GET /api/users/{id}"`. If your API uses [namespaces](/learn/sdks/reference/generators-yml#namespace), prefix with the namespace and `::` (e.g., `"myapi::POST /api/users"`).
</ParamField>
