...
The walkthrough splits the content area between prose and the code panel, so a table of contents leaves both columns cramped. Fern recommends the [`reference` layout](/learn/docs/configuration/page-level-settings#layout) on pages built around a walkthrough: it hides the table of contents and gives the walkthrough the full content width.

```mdx
---
title: Plant Store quickstart
layout: reference
---
```

Setting [`hide-toc`](/learn/docs/configuration/page-level-settings#hide-table-of-contents) instead removes the table of contents but keeps the narrower `guide` width.
