...
The `<ScrollWalkthrough>` component pairs a column of prose steps with a code panel pinned beside them. As each step or substep scrolls past the reading line, the panel switches to that target's file and focuses the lines it describes, so the reader never scrolls back to find the code being discussed. Use it for quickstarts and integration guides that build up one codebase from top to bottom.

## Usage

Declare each file once as a fenced code block inside the component, then point steps and substeps at it with `file` and a line range in `focus` or `highlight`. A step or substep that omits `file` keeps the previously targeted file.
