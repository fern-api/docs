...
The `<SchemaSnippet>` component displays type definitions from your API Reference as a JSON code block. Use it on its own to display the JSON representation, or pair it with the [`<Schema>`](/learn/docs/writing-content/components/schema) component to show both the field-by-field breakdown and JSON together.
