...
Use the `highlighted` property to highlight specific files or folders with an accent background color, and the `comment` property to add explanatory text that appears next to an item. This is useful for drawing attention to important files or providing additional context, similar to how code blocks support [line highlighting](/learn/docs/writing-content/components/code-blocks#line-highlighting) and inline comments.
