...
Colors set in your [`docs.yml` colors configuration](/learn/docs/configuration/site-level-settings#colors-configuration) are automatically available as CSS custom properties in your stylesheets. You don't need to redefine them.

Fern converts your configured colors to [oklch](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/oklch) for consistent color management, with hex fallbacks for unsupported browsers. From your `accent-primary` color, Fern generates a full 12-step color scale following [Radix UI's color scale conventions](https://www.radix-ui.com/colors/docs/palette-composition/understanding-the-scale):

- `--accent-1` through `--accent-12` — solid accent color scale
- `--accent-a1` through `--accent-a12` — accent scale with alpha transparency

Similarly, Fern generates scales for `--grayscale-1` through `--grayscale-12` (and their alpha variants `--grayscale-a1` through `--grayscale-a12`).

Theme-level variables from your `docs.yml` `colors` configuration are also available:

- `--background` — page background color
- `--card-background` — card and code block background color
- `--border` — border and divider color
- `--sidebar-background` — sidebar background color
- `--header-background` — header background color
- `--accent` — primary accent color (maps to `accent-primary`)

Use these variables in your custom CSS to stay consistent with your site's theme and ensure styles adapt between light and dark modes:
...
For the full list of available CSS variables and theme customization techniques, see the [CSS selectors reference](/learn/docs/customization/css-selectors-reference#light-and-dark-mode-customization).
