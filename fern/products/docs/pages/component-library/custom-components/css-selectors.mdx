...
Container for product selection dropdown. Custom styling can be applied for specific layouts; see the docs repo `styles.css` for advanced customization examples.

</Accordion>

<Accordion title=".fern-product-selector-radio-group">

Radio button group for selecting products. Can be customized with grid layouts for multi-column display; see the docs repo `styles.css` for grid layout examples.
