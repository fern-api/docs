...
<Steps>
  ...
  Once you've written the component, you can start leveraging it in your Markdown guides. Use the `@/` prefix to import components with an absolute path from your fern folder root:
...
/>
  ...
  The `@/` prefix resolves to the root of your fern folder, so `@/components/CustomCard` refers to `fern/components/CustomCard`. This is useful for nested MDX files where relative paths would be cumbersome (e.g., `../../../components/CustomCard`). The imports are automatically transformed to relative paths at publish time.
