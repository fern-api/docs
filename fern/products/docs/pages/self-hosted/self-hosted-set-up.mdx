...
Self-hosted containers support [`global-theme`](/learn/docs/customization/global-themes) when the theme is vendored into the image. Vendor the theme by copying its directory (the output of `fern docs theme download --name <name>`) into the image at `/fern-themes/<name>/`, where `<name>` matches the value of `global-theme` in `docs.yml`:
...
Use `fern docs theme download --name <name>` to obtain the theme directory from Fern's cloud registry and commit it to your image source.
