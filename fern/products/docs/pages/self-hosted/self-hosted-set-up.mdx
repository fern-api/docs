...
### Global themes in self-hosted

Self-hosted containers support [`global-theme`](/learn/docs/customization/global-themes) when the theme is vendored into the image. First, [download](/learn/cli-api-reference/cli-reference/docs-commands#fern-docs-theme-download) the theme from Fern's registry into a local directory:

```bash
fern docs theme download --name my-org-theme --output ./theme
```

The command writes `theme.yml` and its assets (logos, fonts, CSS, JS) into `./theme/`. Then copy that directory into the image at `/fern-themes/<name>/`, where `<name>` matches the value of `global-theme` in `docs.yml`:

```dockerfile Dockerfile
ARG BASE_IMAGE=fernenterprise/fern-self-hosted:latest
FROM ${BASE_IMAGE}

# Vendor the theme outside /fern so a later COPY fern/ /fern/ cannot overwrite it.
COPY theme/ /fern-themes/my-org-theme/
...
During `fern-generate`, the container finds the vendored theme, uploads it into its local registry, and publishes with the theme applied — exactly as a Fern-hosted build would. If the theme directory is absent, the container logs a warning, strips `global-theme` from `docs.yml`, and builds unthemed instead of failing.

Re-run `fern docs theme download` and rebuild the image whenever the theme changes in Fern's registry.
