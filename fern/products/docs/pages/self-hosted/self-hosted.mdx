...
| [Fern Agent](/learn/docs/ai-features/fern-agent) | No |
| [Fern Editor](/learn/docs/writing-content/fern-editor) | No |
| [Global themes](/learn/docs/customization/global-themes) | Yes (vendored) |
...
| [Multi-source docs](/learn/docs/preview-publish/multi-source-docs) | No |
...
<Note title="Using a hosted docs.yml with self-hosted">
A single `docs.yml` can serve both Fern-hosted and self-hosted publishes. The self-hosted container automatically handles two cloud-only fields at build time:

- **`global-theme`** — If the image vendors the theme at `/fern-themes/<name>/` (see [Global themes in self-hosted](/learn/docs/self-hosted/set-up#global-themes-in-self-hosted)), the container seeds it into its local registry and the theme renders. If no vendored copy is found, the field is stripped with a warning and the site builds unthemed.
- **`instances[].multi-source: true`** — Stripped with a warning. Multi-source publishing is only supported on Fern-hosted multi-tenant domains; self-hosted serves a single site and ignores this flag.

Neither omission fails the build.
</Note>
