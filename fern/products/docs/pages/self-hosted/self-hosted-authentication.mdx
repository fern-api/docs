...
This includes pages gated with [`viewers`](/learn/docs/authentication/features/rbac). When `FERN_AUTH_TYPE` is not set, the container has no login flow, so no visitor can hold a role and `viewers` cannot be enforced. The container serves `viewers`-gated pages to every visitor instead of returning a 404. To restrict that content on a container without auth, control access at the network or ingress level. Once `FERN_AUTH_TYPE` is set, `viewers` is enforced normally: unauthenticated visitors are redirected to login, and authenticated visitors without a matching role receive a 404.
