---
title: Postman integration
description: Generate Postman collections from API definitions with Fern. Create example requests and responses for testing and exploring APIs.
---

<Warning>The Postman collection generator is no longer actively maintained. We recommend importing your OpenAPI specification directly into Postman instead. See [Postman's OpenAPI import docs](https://learning.postman.com/docs/integrations/available-integrations/working-with-openAPI/).</Warning>

[Postman](https://www.postman.com/) is an API platform for testing and exploring APIs.

Fern can generate Postman collections from your API definitions with example requests and responses. Unlike [other integrations](/docs/integrations/overview), Postman is configured in [`generators.yml`](/sdks/overview/project-structure#generatorsyml) rather than `docs.yml`.

To generate a Postman collection, see the [Postman quickstart](/learn/sdks/generators/postman/quickstart). To publish to a specific collection or directly to Postman, see [Publishing to Postman](/learn/sdks/generators/postman/publishing).

## Customer showcase

<CardGroup cols={2}>

<Card
  title="Square"
  href="https://app.getpostman.com/run-collection/36406701-2ccfeb38-520e-4353-9d2e-386295e72165"
  horizontal
  icon={<img src="https://cdn.simpleicons.org/square" />}
/>

<Card
  title="Webflow"
  href="https://www.postman.com/webflow/webflow-apis/overview"
  horizontal
  icon={<img src="https://dhygzobemt712.cloudfront.net/Mark/Mark_Logo_Blue.svg" />}
/>
</CardGroup>
