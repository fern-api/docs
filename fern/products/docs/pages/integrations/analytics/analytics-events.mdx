...
Fern emits analytics events on every documentation site. When you [configure analytics providers](/learn/docs/integrations/overview) in `docs.yml`, these events are forwarded to your providers. Not every provider receives every event — the **Forwarded to** column in the tables below shows which configured providers receive each event.
