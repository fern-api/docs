...
Fern's [built-in web analytics dashboard](/learn/dashboard/metrics/web-analytics) reports aggregated site usage automatically. This page lists every analytics event Fern emits from your documentation site and whether Fern forwards it to a third-party provider. The **Forwarded to** column in each table shows which [configured providers](/learn/docs/integrations/overview) receive that event — not every provider receives every event.
