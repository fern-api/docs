...
<Steps>
	...
	<Step title="(Optional) Provide the ID at publish time">
		...
		If you reference the measurement ID as an environment variable, `GA4_MEASUREMENT_ID` must be present in whatever environment runs `fern generate --docs`. When you publish with a GitHub Actions workflow, map it into the publish step's `env`:
...
<Steps>
	...
	<Step title="(Optional) Provide the ID at publish time">
		...
		If you reference the container ID as an environment variable, `GTM_CONTAINER_ID` must be present in whatever environment runs `fern generate --docs`. When you publish with a GitHub Actions workflow, map it into the publish step's `env`:
