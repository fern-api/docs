...
<Steps>
	...
	<Step title="Add PostHog to `docs.yml`">
		...
		<Note>
		When you publish with a GitHub Actions workflow, adding `POSTHOG_API_KEY` as a repository or organization secret isn't enough on its own. GitHub doesn't inject secrets into the run automatically, so you must also map the secret into the publish step's `env`. Otherwise the run fails because it can't find `POSTHOG_API_KEY`.
