...
<AccordionGroup>
  ...
  <Accordion title="fern upgrade">
    ...
    When your organization has a [CLI version policy](/learn/cli-api-reference/cli-reference/version-policy#interaction-with-fern-upgrade), the version written to `fern.config.json` is brought into range: raised to the minimum, and never above the maximum.

    ### version

    Use `--version` to upgrade to a specific published version of the [`fern-api` package](https://www.npmjs.com/package/fern-api?activeTab=versions) instead of the latest release. The version must be ahead of the version in `fern.config.json`; use `fern downgrade <version>` to move to an older version.
  ...
  <Accordion title="fern org set cli-version">
    ...
    Use `fern org set cli-version` to constrain the Fern CLI version used by every project in your organization. Requires an organization admin. The [CLI version policy](/learn/cli-api-reference/cli-reference/version-policy) page covers how each bound is enforced.
