...
<AccordionGroup>
  ...
  <Accordion title="fern check">
    ...
    `fern check` also errors when a path parameter shares a name with another request property — an inlined request body property, query parameter, or header — including collisions that only appear after camelCase normalization. Rename one of the parameters using [`x-fern-parameter-name`](/learn/api-definitions/openapi/extensions/parameter-names#resolve-naming-conflicts) (OpenAPI) or the `name` property (Fern Definition) to resolve the conflict.
