...
<AccordionGroup>
  ...
  <Accordion title="fern org set cli-version">
    ...
    Each bound is an exact published version of the [`fern-api` package](https://www.npmjs.com/package/fern-api?activeTab=versions). Pinning an exact version can't be combined with `--min` or `--max`.
