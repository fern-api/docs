...
<AccordionGroup>
  ...
  <Accordion title="fern generate">
    ...
    Use `--fernignore` to specify a custom `.fernignore` file to use instead of the one on the main branch during remote generation. This is useful when you want to temporarily override which files are ignored during SDK generation without modifying the `.fernignore` file in your repository.
    ...
    <Note>
    This flag only works with remote generation. It cannot be used with `--local` or `--runner` flags.
    </Note>
