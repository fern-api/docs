...
<AccordionGroup>
  ...
  <Accordion title="fern docs diff">
    ...
    The `<preview-url>` is the preview deployment URL to compare against production, typically generated by `fern generate --docs --preview`. The `<files..>` argument accepts one or more file paths to generate diffs for. The command resolves each file path to its corresponding page slug and captures screenshots of both the production and preview versions. Diff images are saved to `.fern/diff` by default.
