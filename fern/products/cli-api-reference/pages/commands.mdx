...
<AccordionGroup>
  ...
  <Accordion title="fern check">
    ...
    ### strict-broken-links (deprecated)

    <Warning>
    The `--broken-links` and `--strict-broken-links` flags are deprecated. Use `check.rules` in `docs.yml` instead. See [check configuration](/learn/docs/configuration/site-level-settings#check-configuration).
    </Warning>
    ...
    You can configure the severity of validation rules run by `fern check` in your `docs.yml` file using the `check.rules` property. Each rule can be set to `"warn"` (non-blocking) or `"error"` (blocking).
    ...
    See [check configuration](/learn/docs/configuration/site-level-settings#check-configuration) for the full list of available rules and their defaults.
