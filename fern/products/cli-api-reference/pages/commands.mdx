...
<AccordionGroup>
  ...
  <Accordion title="fern api enrich">
    ...
    Use `fern api enrich` to merge [`x-fern-examples`](/learn/api-definitions/openapi/extensions/request-response-examples) from an overrides file into native OpenAPI `example` fields. The command reads each endpoint's `x-fern-examples` entries and maps them to standard OpenAPI locations:
    ...
    When an endpoint has multiple examples, each is stored as a named entry under the plural `examples` field instead.

    After merging, all `x-fern-examples` keys are stripped from the output.
    ...
    ```bash
    fern api enrich openapi.yml -f ai_examples_override.yml -o enriched-openapi.yml
    ```

    This is useful when you want to convert [AI-generated examples](/learn/docs/ai-features/ai-examples) or manually authored `x-fern-examples` into portable OpenAPI examples that any OpenAPI-compatible tool can consume.

    ### file

    Use `-f` (or `--file`) to specify the overrides file containing `x-fern-examples`. This flag is required.

    ```bash
    fern api enrich openapi.yml -f ai_examples_override.yml -o output.yml
    ```

    ### output

    Use `-o` (or `--output`) to specify the path for the enriched output file. Supports `.yml` and `.json` extensions. This flag is required.
