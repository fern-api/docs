...
<AccordionGroup>
  ...
  <Accordion title="fern docs link check">
    ...
    This is different from the [`broken-links` rule](/learn/docs/configuration/site-level-settings#check-configuration) in `fern check`:
    ...
    When broken links are found, the output includes the broken URL, the HTTP status code, and the source page where the link was found. If a Fern workspace is available, source pages are resolved to local file paths.

    ```plaintext
    Checking links on https://your-org.docs.buildwithfern.com...

    ✗ 3 broken links found

    https://your-org.docs.buildwithfern.com/learn/docs/overview
      404 Not Found
      Found on: fern/pages/getting-started/quickstart.mdx

    https://external-site.com/removed-page
      404 Not Found
      Found on: fern/pages/integrations/overview.mdx
    ```
