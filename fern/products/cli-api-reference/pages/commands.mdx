...
<AccordionGroup>
  ...
  <Accordion title="fern check">
    ...
    `fern check` validates against the navigation tree built from your **local** config — it does not crawl your live deployed site or follow external URLs. To check links on a published site (including external URLs), use the link checker in the [Fern Dashboard](https://dashboard.buildwithfern.com/) instead.
