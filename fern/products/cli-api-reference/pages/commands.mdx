...
<AccordionGroup>
  ...
  <Accordion title="fern generate">
    ...
    Use `--group <group>` to filter to a specific group within `generators.yml`. Required unless you have a `default-group` declared. You can also pass an [alias](/learn/sdks/reference/generators-yml#aliases) to run multiple groups in parallel.
    ...
    Aliases let you run multiple generator groups with a single command. Define aliases in your `generators.yml`:
    ...
    Then run all groups in the alias in parallel:

    ```bash
    # Runs php-sdk, ts-sdk, and go-sdk in parallel
