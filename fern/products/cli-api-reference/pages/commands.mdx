...
<AccordionGroup>
  ...
  <Accordion title="fern generate">
    Use `fern generate` to run the Fern compiler and create SDKs for your API. Remote generation (the default) requires authentication — the CLI prompts you to log in if needed, or you can set `FERN_TOKEN`. Self-hosted generation (`--local`) does not require login.
