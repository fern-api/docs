...
<AccordionGroup>
  ...
  <Accordion title="fern mcp install">
    ...
    Use `fern mcp install` to connect a coding agent to [your organization's Fern MCP server](/learn/docs/ai-features/fern-mcp-servers), which exposes the tools [Fern Agent](/learn/docs/ai-features/fern-agent) works from: searching your published documentation and reading your site configuration, analytics, and reader feedback. The command writes the token from [`fern login`](#fern-login) into the agent's config, so the agent doesn't go through a second OAuth flow:
