...
| [`fern login`](#fern-login) | Login to Fern CLI via GitHub, Google, or enterprise SSO |
...
<AccordionGroup>
  ...
  <Accordion title="fern login">
    ...
    By default, `fern login` opens a browser for GitHub or Google authentication. Two alternative flows are available:
    ...
    ```bash
    fern login --email alice@acme.com
    ```
