...
<AccordionGroup>
  ...
  <Accordion title="fern upgrade">
    ...
    When your organization has a [CLI version policy](/learn/cli-api-reference/cli-reference/version-policy#interaction-with-fern-upgrade), the version written to `fern.config.json` is clamped into it: raised to the floor, and never above the ceiling.
  ...
  <Accordion title="fern org set cli-version">
    ...
    # Set a floor, a ceiling, or both
    fern org set cli-version --min 5.40.0
    fern org set cli-version --max 5.50.0
    fern org set cli-version --min 5.40.0 --max 5.50.0
    ```

    Each bound must be an exact published version of the [`fern-api` package](https://www.npmjs.com/package/fern-api?activeTab=versions). Passing a positional version together with `--min` or `--max` is rejected, as is a floor above the ceiling.
