...
<AccordionGroup>
  ...
  <Accordion title="fern generate --docs">
    ...
    Use `--id` with `--preview` to create a stable, named preview link. Instead of generating a new random URL each time, the same `--id` value produces the same preview URL. Subsequent runs with the same `--id` overwrite the previous preview.
