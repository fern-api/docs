...
| [`fern docs diff`](#fern-docs-diff) | Generate visual diffs between preview and production docs |
...
<AccordionGroup>
  ...
  <Accordion title="fern docs diff">
    ...
    Use `fern docs diff` to generate visual diffs between your preview deployment and production docs. This command captures screenshots of specified pages from both environments and creates side-by-side comparison images highlighting any visual differences.
    ...
    The preview deployment URL to compare against production. This is typically the URL generated by `fern generate --docs --preview`.

    ```bash
    fern docs diff acme-preview-abc123.docs.buildwithfern.com fern/pages/intro.mdx
    ```

    ### files

    One or more file paths to generate diffs for. The command resolves each file path to its corresponding page slug and captures screenshots of both the production and preview versions.
    ...
    Use `--output` to specify the directory where diff images will be saved. Defaults to `.fern/diff`.

    ```bash
    fern docs diff acme-preview-abc123.docs.buildwithfern.com fern/pages/intro.mdx --output ./my-diffs
    ```

    The command outputs JSON with the diff results, including the file path, page slug, comparison image path, change percentage, and whether the page is new.
