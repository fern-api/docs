...
<AccordionGroup>
  ...
  <Accordion title="fern generate">
    ...
    <CodeBlock title="terminal">
    ```bash
    fern generate [--group <group>] [--api <api>] [--version <version>] [--preview] [--fernignore <path>] [--local] [--force] [--no-replay] [--package] [--package-mode <local|docker>] [--package-only]
    ```
    </CodeBlock>
    ...
    Use `--package-mode <local|docker>` to choose where packaging runs. The default, `local`, uses the language toolchains installed on your machine. `docker` runs each toolchain inside the generator's official Docker image, so no toolchain is required on the host.
