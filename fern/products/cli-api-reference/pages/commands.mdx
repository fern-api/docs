...
<AccordionGroup>
  ...
  <Accordion title="fern generate">
    ...
    <CodeBlock title="terminal">
    ```bash
    fern generate [<alias>] [--group <group>] [--api <api>] [--version <version>] [--preview] [--fernignore <path>]
    ```
    </CodeBlock>

    ### alias

    Use `fern generate <alias>` to run multiple generator groups in parallel. Aliases are [defined in your `generators.yml`](/learn/sdks/reference/generators-yml#aliases) and map to a list of groups.
