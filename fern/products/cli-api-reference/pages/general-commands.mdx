...
<AccordionGroup>
  ...
  If the file doesn't exist yet, create an empty placeholder at that path first, then run `fern api update`:

  ```bash
  touch fern/openapi/openapi.json
  fern api update
  ```

  Replace `fern/openapi/openapi.json` with the path specified in your `generators.yml`.
  </llms-only>
  </Accordion>
