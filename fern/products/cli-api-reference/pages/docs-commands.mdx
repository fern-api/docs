...
| [`fern docs theme download`](#fern-docs-theme-download) | Download a theme from Fern's registry into a local theme directory |
...
<AccordionGroup>
  ...
  <Accordion title="fern docs theme download">

    Use `fern docs theme download` to pull a [theme](/learn/docs/customization/global-themes) from Fern's registry into a local directory. It's the inverse of `fern docs theme upload`: the command downloads every referenced asset (logos, fonts, CSS, JS), rewrites asset paths to be relative to the output directory, and writes a `theme.yml` alongside them. The result is the same directory structure `fern docs theme upload` reads, so a downloaded theme can be edited and re-uploaded.

    <CodeBlock title="terminal">
    ```bash
    fern docs theme download [--name <name>] [--org <org>] [--output <directory>]
    ```
    </CodeBlock>

    Downloading a theme is useful for [vendoring it into a self-hosted image](/learn/docs/self-hosted/set-up#global-themes-in-self-hosted) or for recovering a theme whose source repository is no longer available.

    ### name

    Use `--name` to select the theme to download. Defaults to `default`.

    ```bash
    fern docs theme download --name my-theme
    ```
    ...
    ```bash
    fern docs theme download --name my-theme --org my-org
    ```

    ### output

    Use `--output` to specify the directory to write the theme into. By default, files are written to `./fern/theme/`.

    ```bash
    fern docs theme download --name my-theme --output ./themes/my-theme
    ```
