...
<AccordionGroup>
  ...
  <Accordion title="fern docs md generate">
    ...
    Use `--local` to parse library source locally rather than on Fern's servers. Required when the library uses a `path` input, and supported for `git` inputs, which are cloned at the configured `ref` before parsing. Local generation does not require authentication.
