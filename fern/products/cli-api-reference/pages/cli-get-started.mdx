...
<AccordionGroup>
  <Accordion title="Docs workflow">
    ...
    |---------|-------------|---------------|
    | `fern init --docs` | Create a new docs project | No |
    | `fern docs dev` | Preview docs locally at `localhost:3000` | No |
    | `fern generate --docs` | Publish documentation to production | Yes |
  </Accordion>
  ...
  <Accordion title="SDK workflow">
    ...
    | Command | Description | Requires auth |
    |---------|-------------|---------------|
    | `fern init` | Create a new SDK project | No |
    | `fern check` | Validate your API definition | No |
    | `fern generate --preview` | Write a local SDK preview to `.preview/` without publishing to package managers or GitHub. This is not a hosted [docs preview link](/learn/docs/preview-publish/preview-changes#preview-links) | Yes |
    | `fern generate` | Publish SDKs to production | Yes |
    | `fern generate --group <name>` | Generate a specific SDK group | Yes |
