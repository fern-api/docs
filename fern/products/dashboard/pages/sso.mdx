...
<Tabs>
  <Tab title="SAML">
    <Steps>
      ...
      <Step title="Send Fern your IdP metadata">
        Copy the SSO URL, Entity ID, and X.509 certificate from Google. Send them to Fern.
      </Step>

      <Step title="Receive configuration values from Fern">
        Fern will send you the ACS URL and Entity ID through the secure channel.
      </Step>

      <Step title="Complete configuration in Google">
        On **Service provider details**, enter these values:
        ...
        Assign users to the app, then notify Fern. Fern will enable SSO and run a test login with you.
      </Step>
    </Steps>
  </Tab>
</Tabs>
  ...
  <Tab title="SAML">
    <Steps>
      ...
      <Step title="Configure attributes & claims">
        Set **Name ID** to `user.primaryauthoritativeemail` (email) and add these claims:
