...
Select **Add member**, then choose one of the following to invite members to your organization:

- **One-time link**: Assign a [role](#role-types) and select **Generate invite link** to create a one-time-use link to share with anyone.
- **Email**: Enter an email address, assign a [role](#role-types), and select **Send invitation**.

For Editors, you can also configure whether they have [CLI access](#editor).
</Step>
