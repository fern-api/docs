...
<Note>
  Fern only supports gRPC SDK generation for .NET/C#. For other languages, protobuf specs can be used for documentation generation.
</Note>
...
api: 
  specs: 
    - proto:
        root: ../user-service/proto # Path up to where package starts (e.g., for package userservice.v1)
        target: ../user-service/proto/data/v1/user_service.proto # Omit to generate docs for entire root folder
groups:
  external:
    generators:
