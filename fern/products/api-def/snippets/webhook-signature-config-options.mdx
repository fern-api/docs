...
<ParamField path="payload-format.components" type="list of strings" required={false} toc={true}>
  ...
  <Warning>Without `timestamp` in the components list, the `timestamp.tolerance` check cannot protect against replay attacks because the timestamp is unauthenticated. Include `timestamp` to ensure replay protection.</Warning>
</ParamField>
