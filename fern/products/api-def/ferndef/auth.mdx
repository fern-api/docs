...
Set `omit: true` on `username` or `password` to remove it from the generated SDK. This is useful for APIs that send credentials over basic auth but only require one of the two fields — for example, a publishable API key sent as the username with no password.
...
The omitted field is removed from the SDK constructor, type hints, and getters. Internally it is treated as an empty string when encoding the `Authorization: Basic` header (e.g., `Basic base64("apiKey:")`). When both `username` and `password` are omitted, the SDK skips the `Authorization` header entirely.
