...
If your API's basic auth uses only one of the two fields (for example, a token in the username and an empty password), set `omit: true` on the field that isn't used. The omitted field is removed from the SDK's public API and treated as an empty string when encoding the `Authorization: Basic` header.
...
The generated SDK accepts only the `apiKey` parameter, and the `Authorization` header encodes `apiKey:` (with a trailing colon):
...
If both `username.omit` and `password.omit` are set to `true`, the `Authorization` header is skipped entirely.
