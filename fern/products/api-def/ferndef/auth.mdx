...
`token-header` and `token-prefix` customize the authorization header sent with API requests in both generated SDKs and the [API Explorer](/learn/docs/api-references/api-explorer#oauth-client-credentials). By default, the token is sent as `Authorization: Bearer <token>`.

For example, the following produces a header `Fern-Authorization: Fern-Bearer <token>`:
