...
Set `base-url-env` to the name of an environment variable that the generated SDK reads for its base URL, the same way auth credentials can default from environment variables. This lets each deployment (staging, production, self-hosted) point the SDK at a different host without passing a base URL in code.
