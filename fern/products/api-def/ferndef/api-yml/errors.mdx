...
endpoint, every error that's listed must have a different HTTP status code. The
[wildcard status codes](/learn/api-definitions/ferndef/errors#wildcard-status-codes)
`4XX` and `5XX` count as distinct from concrete codes such as `400` and `500`.
