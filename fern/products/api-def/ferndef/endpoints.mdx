...
Set `optional: true` on a `body` to declare that the endpoint can be called with no request body. [Endpoint examples](/learn/api-definitions/ferndef/examples#examples-for-endpoints) can then omit `request`, and generated SDK methods take the body as an optional argument.
...
An OpenAPI request body with `required: false` imports as optional only when the [`respect-optional-request-body`](/learn/sdks/reference/generators-yml#specification-types) setting is enabled for that spec.
