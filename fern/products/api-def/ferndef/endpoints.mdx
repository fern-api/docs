...
A path parameter can't share a name with another request property (a body property, query parameter, or header), including after camelCase normalization. `fern check` errors on such collisions. Set the `name` property on the path parameter to rename it and deconflict:
