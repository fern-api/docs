...
Omit `request` for endpoints with an [optional request body](/learn/api-definitions/ferndef/endpoints/overview#request-body) to document a call that sends no body. `request: {}` documents an empty JSON object instead, which renders as `-d '{}'` in cURL and the API Explorer.
