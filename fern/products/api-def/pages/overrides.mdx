...
For OpenAPI, AsyncAPI, and OpenRPC, you can use [Fern's extensions](/api-definitions/overrides#definition-specific-extensions) to apply customizations. 
...
The `overrides` field accepts a single path or a list of paths. When multiple paths are provided, overrides are applied sequentially in order, with later files taking precedence over earlier ones for conflicting keys. This is supported for all spec types: OpenAPI, AsyncAPI, OpenRPC, and Protobuf.

Below are common scenarios where multiple override files help keep your configuration organized.

### Shared overrides across multiple specs
