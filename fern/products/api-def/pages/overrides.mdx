...
<Warning>
  ...
  However, overrides are also fully supported. If your team already uses overrides, there's no need to switch unless you need advanced functionality like bulk modifications using JSONPath wildcards or modifying elements within arrays. You can also use both together (overrides are applied first, then overlays).
</Warning>
