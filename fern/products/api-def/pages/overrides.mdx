...
This is useful when multiple specs need the same base set of overrides. For example, if you have separate specs for different API versions or services, each spec can reference a shared overrides file plus its own spec-specific overrides:
...
Configure different overrides for public versus internal APIs. If your internal API is a superset of the public API, use multiple override files to share the public overrides as a base and layer internal-specific additions on top:
