...
Fern doesn't combine OpenAPI files based on their location in a repository. Files listed together under one API's `specs:` key in a single `generators.yml` merge into one API definition, one API Reference, and one combined raw specification URL. APIs configured in separate `fern/apis/<api>/generators.yml` files remain separate. Each API has its own API Reference and raw specification URL. Raw specification URLs are keyed by the API definition's slug, title, or UUID: a project with one API uses `/openapi.json`, while a multi-API project uses `/openapi/<id>.json`.
