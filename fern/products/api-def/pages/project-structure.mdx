...
Fern doesn't combine API definition files based on their location in a repository. Files listed together under one API's `specs:` key in a single `generators.yml` merge into one API definition and one API Reference. APIs configured in separate `fern/apis/<api>/generators.yml` files remain separate, each with its own API Reference.

Raw specification URLs follow the specification format. A project with one API uses `/<format>.json`; a multi-API project uses `/<format>/<id>.json`, where `<id>` is the API definition's slug, title, or UUID. For example, OpenAPI definitions use `/openapi.json` or `/openapi/<id>.json`, and AsyncAPI definitions use `/asyncapi.json` or `/asyncapi/<id>.json`.

For SDK generation, Fern supports two approaches for working with multiple API definitions. Both require an `apis` folder — this folder must use that exact name.
