...
```yaml title="Fern Definition" {3,8}
types:
  Person:
    properties:
      name: string
...
```yaml title="Fern Definition" {3-5}
types:
  GoldenRetriever:
    extends:
      - Dog
...
```yaml title="Fern Definition" {4-6, 11-13, 16-19}
types:
  Word:
    type: string
    validation:
...
```yaml title="Fern Definition" {4-6, 12-15, 18-20}
types:
  Age:
    type: integer
    validation:
