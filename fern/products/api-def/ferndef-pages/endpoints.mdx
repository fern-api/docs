...
Endpoints can specify pagination configuration to enable auto-pagination in generated SDKs. Fern supports both offset-based and cursor-based pagination schemes.

<CodeBlocks>
```yaml title="Offset pagination" {5-8}
service:
  base-path: /users
  auth: false
  endpoints:
...
| `cursor` | Path to the cursor parameter in the request (e.g., `$request.starting_after`) |
...
Mark an endpoint as idempotent to allow SDK users to specify idempotency headers for safe request retries. You must also configure [idempotency headers](/api-definitions/ferndef/api-yml/global-headers#idempotency-headers) in your `api.yml`.

```yaml title="service.yml" {7}
service:
