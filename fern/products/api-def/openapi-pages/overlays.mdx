...
<Note>
  ...
  [Overrides](/api-definitions/overrides) are also fully supported. If overrides are working for your team, there's no need to switch. You can also use both together (overrides are applied first, then overlays).
</Note>
...
## Reusing overlays across APIs

Each spec in `generators.yml` accepts one overlay file. You can't specify multiple overlays per spec, but you can reference the same overlay file across different specs and configurations.

### Shared overlays across multiple specs
