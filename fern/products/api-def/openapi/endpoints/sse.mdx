...
<Note>
For SDK-side usage of server-sent event (SSE) metadata (event ID, event type, retry interval), see [Accessing server-sent event metadata](/learn/sdks/deep-dives/sdk-user-features#accessing-server-sent-event-metadata).
</Note>
