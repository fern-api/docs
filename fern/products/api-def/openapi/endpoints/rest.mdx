...
A `requestBody` with `required: false` imports as a required body unless the [`respect-optional-request-body`](/learn/sdks/reference/generators-yml#specification-types) setting is enabled for the spec, which imports it as an optional body and drops the argument from generated SDK methods and snippets.
