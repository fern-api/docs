...
Fern imports a `requestBody` as required, even when it's marked `required: false`. To import `required: false` as an [optional request body](/learn/api-definitions/ferndef/endpoints/overview#optional-request-body), enable the [`respect-optional-request-body`](/learn/sdks/reference/generators-yml#specification-types) setting for the spec. The generated SDK method then takes the body as an optional argument, and snippets omit it when it isn't provided.
