...
Configuration can be set at the **document level** (applies to all webhook endpoints) or **per-endpoint** (overrides the document-level default). Both levels accept the same configuration options. To declare one scheme for the whole API without touching the spec, or when the spec models no webhooks, set [`api.settings.webhook-signature` in `generators.yml`](/learn/sdks/deep-dives/webhook-signature-verification#configure-api-wide-in-generatorsyml) instead.
