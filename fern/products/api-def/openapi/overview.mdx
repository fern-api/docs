...
Fern reads the `maxLength`, `minLength`, `pattern`, and `format` keywords on string schemas and surfaces them in your documentation: the API Reference lists them alongside a property's type (for example, `<=64 characters`), the [API Explorer](/learn/docs/api-references/api-explorer) caps input at `maxLength`, and `fern check` validates the examples in your specification against them. Generated SDKs don't enforce these constraints at runtime or in their types; rejecting invalid values remains the responsibility of your API server.
