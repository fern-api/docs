...
To remove `username` or `password` from the generated SDK — for example, when an API sends credentials over basic auth but only uses one of the two fields — set `omit: true` on the field:

```yaml title="openapi.yml" {9-10}
components:
  securitySchemes:
    BasicAuth:
      type: http
...
The omitted field is removed from the SDK constructor and treated as an empty string when encoding the `Authorization: Basic` header.
