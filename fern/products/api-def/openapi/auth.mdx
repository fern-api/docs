...
To remove either credential from the generated SDK, set `omit: true` on the field your API does not accept. The omitted field is encoded as an empty string in the `Authorization` header (e.g. `omit: true` on `password` produces `base64("clientId:")`). If both fields are omitted, no `Authorization` header is sent.

```yaml title="openapi.yml" {10}
components:
  securitySchemes:
    BasicAuth:
      type: http
...
To remove a credential from the generated SDK, set `omit: true` on the field your API does not accept. See the [`auth-schemes` reference](/learn/sdks/reference/generators-yml#auth-schemes) for details.
