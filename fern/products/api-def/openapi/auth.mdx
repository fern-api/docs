...
To remove either credential from the generated SDK, set `omit: true` on the field your API doesn't accept. The omitted field is encoded as an empty string in the `Authorization` header (e.g. `omit: true` on `password` produces `base64("clientId:")`). If both fields are omitted, no `Authorization` header is sent.
...
To remove a credential from the generated SDK, set `omit: true` on the field your API doesn't accept. See the [`auth-schemes` reference](/learn/sdks/reference/generators-yml#auth-schemes) for details.
