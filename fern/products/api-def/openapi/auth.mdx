...
Fern uses per-endpoint `security` definitions to generate SDKs that route the correct credentials to each endpoint automatically. For Fern Definition format, this is configured with [endpoint-level authentication](/learn/api-definitions/ferndef/authentication#endpoint-level-authentication).
