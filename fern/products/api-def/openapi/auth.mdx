...
To customize the authorization header sent with API requests, set `token-header` and `token-prefix`:

```yaml title="generators.yml" {5-6}
auth-schemes:
  OAuth:
    scheme: oauth
    type: client-credentials
    token-header: X-Auth-Token    # default: Authorization
...
This configuration applies to both generated SDKs and the [API Explorer](/learn/docs/api-references/api-explorer#oauth-client-credentials), where users can enter credentials and fetch tokens interactively.
