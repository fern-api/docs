...
Instead of a default scheme, set [`api.auth: endpoint-security`](/learn/sdks/reference/generators-yml#auth) to have generated SDKs honor each endpoint's own `security` requirements.
...
To enable OAuth client credentials in the [API Explorer](/learn/docs/api-references/api-explorer#authentication), set `playground.oauth` on your API Reference entry in `docs.yml`. With it enabled, an `x-fern-playground-description` on the OAuth security scheme appears beneath the client credentials input.
