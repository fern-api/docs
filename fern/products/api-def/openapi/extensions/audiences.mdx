...
Both are build-time filters, not per-viewer access control. Fern prunes the API definition when you publish and serves that single output to every reader, so filtered-out endpoints don't exist on the published site and can't be revealed to one logged-in user. To vary what an authenticated reader sees by role, gate navigation entries with [`viewers`](/learn/docs/authentication/features/rbac).
...
Servers are the exception to build-time filtering: a tagged server is [shown or hidden per viewer in the API Explorer](/learn/docs/api-references/api-explorer#multiple-environments) based on the roles of the authenticated reader.
