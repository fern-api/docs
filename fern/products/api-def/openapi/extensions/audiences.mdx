...
Specify which audiences to include in your SDKs or API Reference docs. Audience filtering is fail-open: filtering applies only when a generator group or docs navigation entry sets `audiences`. An entry with no `audiences` receives the full spec, and any element left untagged is included for every audience.
...
<Warning title="Audiences are not a security boundary">
Because filtering is fail-open, an SDK group or docs entry that omits `audiences` exposes the entire spec, and an untagged internal endpoint reaches every consumer. To guarantee that internal endpoints never appear in partner-facing SDKs or docs, keep them out of the partner output entirely: split internal and partner endpoints into separate API definitions, or exclude internal paths with [`settings.filter`](/learn/api-definitions/openapi/generators-yml-reference#settingsfilter). Physical separation makes leakage structurally impossible, whereas audience tags rely on every consumer being configured correctly. Use audiences for product segmentation, not as the only control enforcing a security boundary.
</Warning>
