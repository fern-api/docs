...
In generated TypeScript SDKs, availability values become JSDoc tags on client methods. IDEs surface these as warnings and strikethrough styling.

<Frame caption="Deprecated method shown with strikethrough and @deprecated tag in VS Code (Square SDK)">
![Deprecated method with strikethrough and @deprecated JSDoc tag in VS Code](./images/availability-deprecated-ide.png)
</Frame>
...
This generates `@deprecated Use PATCH /plants/{plantId} instead.` in the JSDoc.

For a real-world example, see the [Square TypeScript SDK](https://github.com/square/square-nodejs-sdk/blob/master/src/api/resources/employees/client/Client.ts) where deprecated endpoints are marked with `@deprecated`.
