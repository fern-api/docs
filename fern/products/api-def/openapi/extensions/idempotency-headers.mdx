...
Add the `x-fern-idempotent` extension to individual endpoints and set it to `true` to mark them as idempotent. A marked operation also exposes an `--idempotency-key` flag in a generated CLI and becomes eligible for [automatic retries](/learn/cli-generator/get-started/features#retries-with-backoff):
