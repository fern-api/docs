...
For endpoints that accept a request body, place the body properties directly under `request` (flat format):

```yaml title="openapi.yml" {7-9}
paths:
  /widgets:
    post:
      x-fern-examples:
...
<Note>
The `request` field contains the body properties directly. Do not nest them under `request.body` — that format is not supported. The `response` field, by contrast, requires a `body` key because responses can also include an `error` field.
</Note>
