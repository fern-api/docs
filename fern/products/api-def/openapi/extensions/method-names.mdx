...
To ignore operation `tags` entirely and derive SDK structure from each `operationId`, enable [`ignore-tags`](/learn/sdks/reference/generators-yml#settingsignore-tags) in `generators.yml`.
