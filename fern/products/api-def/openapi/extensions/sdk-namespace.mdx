...
Add `x-fern-sdk-namespace` to a response object in `components.responses` to declare a shared error in a namespace. Fern only reads the extension on error responses when the spec enables [`settings.namespaced-errors`](/learn/sdks/reference/generators-yml#settingsnamespaced-errors) in `generators.yml`. Without that setting, error responses are compared across the whole API by status code.
