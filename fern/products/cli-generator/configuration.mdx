...
GitHub App the Homebrew and Scoop jobs mint a short-lived installation token from at release time, instead of reading a long-lived personal access token. Declared once for the whole `distribution` block, since one App normally serves both channels; each job passes its own tap or bucket owner, so a tap and a bucket under different accounts mint from the right installation. A channel's `tokenEnvironmentVariable` takes precedence over `githubApp`.
</ParamField>
