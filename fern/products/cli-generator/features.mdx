...
Idempotent methods (GET, HEAD, OPTIONS, DELETE, PUT) retry by default; POST and PATCH retry only when the endpoint is marked idempotent in the spec. Pass `--no-retry` to disable retries for a single invocation.
