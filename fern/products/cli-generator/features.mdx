...
Run `contoso <command> --help` to list a command's flags, or [`--schema`](#machine-readable-schema) for the same surface as a machine-readable JSON contract. When a flag name differs from the spec, `--help` shows the original name as `(api: <name>)`, which is the key to use in `--params`.
...
| Command | Output |
| --- | --- |
| `contoso --schema` | `globalFlags` plus every operation's name and description. |
...
`input.properties` mirrors the CLI's flag surface: query, path, header, and body fields are siblings, each tagged with a `location`. Keys are the spec's names, so read `flag` to get the exact flag to pass; it accounts for kebab-casing, `x-fern-parameter-name` renames, and the `-param` suffix added when a name collides with a global flag. A property without `flag` is passed through `--params` or `--json`. Properties also carry JSON Schema constraints (`enum`, `default`, `minimum`, `nullable`, `deprecated`) from the spec.

Operations also surface CLI capabilities that go beyond the spec, so agents can discover them without trial and error. Each field appears only when it applies:

- `paginable`: supports [`--page-all`](#pagination), with the pagination `kind` and the parameters and response paths the CLI walks.
- `binaryResponse`: the response is a binary body, so `--output <PATH>` writes it to a file.
- `streaming`: the response streams, with `format` of `sse`, `json`, or `text`.
