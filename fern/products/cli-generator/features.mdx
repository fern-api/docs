...
Run `contoso <command> --help` to list a command's flags, or [`--schema`](/learn/cli-generator/get-started/schema) for the same surface as a machine-readable JSON contract. When a flag name differs from the spec, `--help` shows the original name as `(api: <name>)`, which is the key to use in `--params`.
