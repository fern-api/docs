...
Each generated CLI sends a `User-Agent` header identifying it by binary name and version, `<binaryName>-cli/<version>` (for example, `contoso-cli/1.4.0`), so its traffic is distinguishable on the API backend. The `-cli` suffix is added automatically; a binary already named `contoso-cli` is left unchanged.
