...
When an API accepts more than one type of credential (for example, both `MY_API_KEY` and `MY_TOKEN` environment variables), the CLI can accept either one. This requires both the OpenAPI spec and `generators.yml` to declare all schemes. If a scheme is missing from the spec, the CLI ignores it.

### Define schemes in the OpenAPI spec

Each scheme must appear in `securitySchemes`, and the top-level `security` array must list them as alternatives. If the spec already defines one scheme, add the second through an [override](/learn/api-definitions/openapi/overrides):

```yaml title="openapi-overrides.yml"
components:
  securitySchemes:
    TokenAuth:
      type: http
...
Multiple top-level entries in `security` are OR alternatives — the API accepts any one of them. See [multiple auth schemes](/learn/api-definitions/openapi/authentication#multiple-auth-schemes) in the OpenAPI authentication docs for details.

### Configure schemes in `generators.yml`

Define both schemes under [`auth-schemes`](/learn/sdks/reference/generators-yml#auth-schemes) and compose them with `api.auth.any`:
...
The `any` strategy uses the first scheme that has a credential. With this configuration, the CLI authenticates using whichever environment variable is set:
