...
| OAuth 2 (client credentials) | Reads the client ID and secret from `<BIN>_CLIENT_ID` and `<BIN>_CLIENT_SECRET` (or the names in the API definition) and fetches the token automatically. |
