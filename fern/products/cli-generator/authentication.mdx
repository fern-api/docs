...
| Authorization code with PKCE | `authorization-code` | Interactive login. The CLI opens a browser and receives the callback on a loopback listener. |
| Device code | `device-code` | Interactive login on machines without a browser, such as SSH sessions and containers. |

The interactive flows are public-client only: they use PKCE (RFC 7636) rather than a client secret. They require CLI generator `0.29.0` or later.
...
Omitting `redirect-uri` binds an OS-assigned loopback port at login time, which is the recommended setup (RFC 8252 §7.3) and needs no port registration beyond whatever the authorization server allows. To pin a port, set `redirect-uri` to a loopback URL, and list `ports` to add fallbacks tried in order when the primary port is busy:
...
The host must be `127.0.0.1` or `localhost` over `http`, the port is required, and the path is arbitrary. The CLI binds and advertises this URI exactly, so every URL it can produce, including each backup port, must be registered with the authorization server. PKCE is always applied, and `S256` is the only supported challenge method.
...
Authorization servers that require additional literal parameters, such as an Auth0 `audience`, accept them through per-request maps. Parameters reserved by the OAuth protocol itself are ignored.
