...
Omitting `redirect-uri` binds an OS-assigned loopback port at login time, which is the recommended setup and needs no port registration beyond whatever the authorization server allows. To pin a port, set `redirect-uri` to a loopback URL, and list `ports` to add fallbacks tried in order when the primary port is busy:
