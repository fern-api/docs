...
The loopback listener answers the browser once the login finishes, rendering a built-in page for the success and failure states. `success-redirect-url` and `error-redirect-url` hand that last screen off to pages you host:
...
Both take an absolute `http` or `https` URL, validated at generation time, and the listener responds with a `302` to it. The success redirect forwards no query parameters, since the credential is already captured on the loopback. The error redirect carries `error` and, when the authorization server sent one, `error_description`, so the hosted page can explain the failure:

```
https://example.com/app/oauth/cli/error?error=access_denied&error_description=User+denied+the+request
```

A callback that arrives without a usable `code` or `state` reports `error=invalid_request`, and a failure after the callback reports `error=server_error`. Query parameters already on the configured URL are preserved, and the appended parameters are inserted before any fragment.

Neither URL is the OAuth `redirect_uri`, so neither needs registering with the authorization server. Omitting both keeps the built-in pages. These fields require Fern CLI `5.96.0` and CLI generator `0.34.0` or later.
