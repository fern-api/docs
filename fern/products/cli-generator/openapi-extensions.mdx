...
Headers declared API-wide with [`x-fern-global-headers`](/learn/api-definitions/openapi/extensions/global-headers) are sent on every request the same way, and an `x-fern-global-parameters` entry targeting the same header overrides them.
