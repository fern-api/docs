...
Fern reads the OpenAPI spec, runs the CLI generator, and writes a complete Rust project to the output path. To run generation on your own machine instead of Fern's cloud, pass `--local` (requires Docker). See [Local generation](/learn/cli-generator/get-started/local-generation) for setup details.
</Step>
