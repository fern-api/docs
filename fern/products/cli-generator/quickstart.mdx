...
The `--help` output shows the full command tree: top-level commands derived from OpenAPI tags, subcommands from individual operations, and built-in utilities like `completion` and `man`. Each group's [help text](/learn/cli-generator/get-started/openapi-extensions#command-group-help) comes from its tag description.
</Step>
