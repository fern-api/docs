...
The `--help` output shows the full command tree: top-level commands derived from OpenAPI tags with [help text](/learn/cli-generator/get-started/openapi-extensions#command-group-help) from their tag descriptions, subcommands from individual operations, and built-in utilities like `completion` and `man`.
</Step>
