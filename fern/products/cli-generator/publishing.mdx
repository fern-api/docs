...
The CI workflow then builds binaries for all platforms, attaches the archives and the `curl | bash` and PowerShell installers to the GitHub Release, and publishes to npm.

## Distribution channels

Homebrew and Scoop install from the archives already attached to each GitHub Release, so both require a `github` block in `generators.yml`. Configure them under [`distribution`](/learn/cli-generator/get-started/configuration#config-options):
...
Each channel needs a repository and a token before the first release:
...
<Step title="Regenerate and tag a release">

```bash
fern generate --group cli
```

Regeneration adds a `publish-homebrew-formula` job, a `publish-scoop` job, or both to the release workflow, and adds the matching install commands to the generated README. Prereleases are skipped: a tap and a bucket have no prerelease channel, so an RC never becomes the version users install.

</Step>

</Steps>

The Scoop manifest declares a `64bit` architecture only, matching the single Windows target the workflow builds. Users on ARM64 Windows install through npm or the PowerShell installer instead. `self-update` replaces the binary in place for every channel; it doesn't defer to `brew upgrade` or `scoop update`, so a package manager can later report an older version than the one installed.
