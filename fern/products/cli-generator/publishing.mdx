...
OIDC can't perform a package's first publish; npm requires a package to exist before you can add a trusted publisher. Create every package with a placeholder version before your first publish run.

The CI workflow publishes one launcher package and one package per platform. For a package named `@myorg/my-cli`, that is:
...
Use a placeholder version below the one CI will release (for example `0.0.1`) so the first OIDC run publishes cleanly over it. For each package name, create a directory with a minimal `package.json` and publish it:
...
<llms-only>
Publish all of the packages listed above, not just the launcher — an OIDC run fails on any that does not yet exist. Set the placeholder version to something below the first CI release (e.g. `0.0.1`).

If the account has two-factor authentication enabled, `npm login` (a browser session) cannot publish. Instead, have the user create a [granular access token](https://docs.npmjs.com/creating-and-viewing-access-tokens) with **Read and Write** access to all packages, then set it locally with `npm config set "//registry.npmjs.org/:_authToken" <token>` and publish. This token is only for the local bootstrap; CI uses OIDC.
</llms-only>
...
The CI workflow then builds binaries for all platforms and publishes to npm, Homebrew, and GitHub Releases.
