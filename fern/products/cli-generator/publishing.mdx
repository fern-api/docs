...
Homebrew and Scoop are optional channels that install from the archives attached to each GitHub Release, so both require a `github` block in `generators.yml`. Configure them under [`distribution`](/learn/cli-generator/get-started/configuration#config-options):
...
The next generation adds a `publish-homebrew-formula` job, a `publish-scoop` job, or both to the release workflow, along with the matching install commands in the generated README. Prereleases are skipped: a tap and a bucket have no prerelease channel, so an RC never becomes the version users install.
