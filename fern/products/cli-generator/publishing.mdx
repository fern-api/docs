...
The workflow's built-in `GITHUB_TOKEN` is scoped to the CLI repository and can't push to another one, so each channel authenticates with credentials you store as Actions secrets in the CLI repository, under **Settings** > **Secrets and variables** > **Actions**:
...
One App serves both channels: each publish job mints an installation token at release time, scoped to a single repository and valid for an hour, and revoking access becomes uninstalling the App.

1. In the account that owns the tap or bucket, go to **Settings** > **Developer settings** > **GitHub Apps** > **New GitHub App**. Grant the **Contents: Read and write** repository permission and clear **Webhook** > **Active**. Record the App ID, then generate a private key and download the `.pem` file.
2. From the App's page, select **Install App** and limit the installation to the tap and bucket repositories. A tap and a bucket under different accounts each need the App installed on their own account: every job requests a token for the owner of the repository it pushes to.
3. Add two Actions secrets to the CLI repository, one holding the App ID and one holding the entire contents of the `.pem` file. An Actions *variable* isn't readable as a secret, and a private key pasted as a single line still carries its `BEGIN` header while failing to sign, which surfaces as `secretOrPrivateKey must be an asymmetric key`, so keep the key's line breaks intact.
4. Reference both secret names from [`distribution.githubApp`](/learn/cli-generator/get-started/configuration#config-options):
...
A channel's `tokenEnvironmentVariable` takes precedence over `githubApp`, so channels can migrate one at a time: an App declared alongside `scoop.tokenEnvironmentVariable` means Homebrew publishes through the App while Scoop keeps its token.
