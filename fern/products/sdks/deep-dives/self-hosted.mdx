...
`fern generate --local` runs in GitHub Actions. GitHub-hosted `ubuntu-latest` runners ship with Docker preinstalled, which satisfies the [Docker runtime requirement](#infrastructure-requirements). Self-hosted runners need Docker-in-Docker or a mounted Docker socket.

Store the Fern API key as a repository secret and pass it to the generate step as `FERN_TOKEN`:
...
The generator image is pulled fresh on each run because it isn't cached between jobs, so the first generation in a job is slower. Because `--local` computes SDK version numbers in your pipeline rather than in Fern's cloud, configure versioning as described in [Self-hosted SDK versioning](/learn/sdks/deep-dives/self-hosted-versioning).
