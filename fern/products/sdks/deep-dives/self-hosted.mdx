...
Generate a Fern token, which is required for local generation to verify your organization. Create one from the [API tokens](/learn/dashboard/configuration/fern-tokens) page in the Dashboard, or run `fern token` in your terminal:
