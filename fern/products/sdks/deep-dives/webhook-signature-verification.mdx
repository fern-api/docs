...
Configure signature verification in your API definition with the [`x-fern-webhook-signature` extension](/learn/api-definitions/openapi/endpoints/webhooks#sdk-signature-verification), at the document level (inherited by all webhooks) or per-webhook. That page covers the signature methods, payload format, and [every configuration option](/learn/api-definitions/openapi/endpoints/webhooks#configuration-options).
