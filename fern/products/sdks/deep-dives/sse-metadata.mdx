...
<Note>
Dedicated metadata-aware iteration methods are available for TypeScript and Go. Python does not yet expose an equivalent method.
</Note>

Metadata-aware iteration reads SSE protocol fields — event ID, event type, and retry interval — alongside the parsed data. Default iteration is unchanged, so opting in is fully backward compatible.
