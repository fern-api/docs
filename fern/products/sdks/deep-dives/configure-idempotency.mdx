...
By default the SDK sends an `Idempotency-Key` header on `POST` and `PUT` requests. Pass an object with `header-name` and `methods` to change the header or the eligible methods, and use a generator's `config.auto-generate-idempotency-key` to override the API-level value for a single SDK. The TypeScript, Python, Java, Go, C#, PHP, and Ruby SDKs support auto-generation.
