...
By default, the caller supplies the key. To have the SDK generate one automatically on retry-unsafe requests (`POST`, `PUT`) when the caller omits it, enable [`auto-generate-idempotency-key`](/learn/sdks/reference/generators-yml#settingsauto-generate-idempotency-key) in your `generators.yml`:
...
Setting it under `api.settings` applies to every generator in the API. Set it on an individual generator's [`config`](/learn/sdks/reference/generators-yml#configauto-generate-idempotency-key) to enable or override it per SDK.
