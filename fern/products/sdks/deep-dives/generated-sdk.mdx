...
SDK users can opt into metadata-aware iteration to read the SSE protocol fields — event ID, event type, and retry interval — alongside the parsed data: `stream.withMetadata()` in TypeScript, or `RecvEvent()` with `LastEventID()` in Go. This is useful for resuming a stream via the standard `Last-Event-ID` header: store the last received ID as you iterate, then pass it back to the server on reconnection. Default iteration is unchanged, so opting in is fully backward compatible.
