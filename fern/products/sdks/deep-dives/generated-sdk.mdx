...
TypeScript SDKs type the `body` property on each generated error class that has a declared body schema, so narrowing an error with `instanceof` gives type-checked access to the body fields. This requires TypeScript SDK generator version 3.84.0+.
...
try {
    ...
    if (err instanceof PlantStore.BadRequestError) {
        console.log(err.body.code, err.body.message);
    }
}
```

Errors without a declared body schema expose `body` as `unknown`, so you must narrow or cast it before reading fields.
