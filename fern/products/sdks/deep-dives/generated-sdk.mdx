...
### Accessing server-sent event metadata
...
<CodeBlocks>
  ...
  <CodeBlock title="Python">
    ...
    # Use default iteration (shown above) to consume parsed data.
    response = client.chat.stream(query="Hello")
...
The event ID is useful for resuming a stream via the standard `Last-Event-ID` header. Store the last received ID as you iterate, then pass it back to the server on reconnection.
