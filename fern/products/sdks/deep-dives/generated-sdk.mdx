...
When you mark an endpoint with an [availability](/learn/api-definitions/openapi/extensions/availability) status like `deprecated` or `beta`, the generated TypeScript SDK includes the corresponding JSDoc tag (`@deprecated`, `@beta`) on the client method. IDEs display these as warnings and strikethrough styling so your users know which endpoints are stable and which to avoid.
