...
TypeScript SDKs type the `body` property on each generated error class that has a declared body schema, so narrowing an error with `instanceof` gives type-checked access to the body fields:

```typescript
try {
    await client.plants.water("plantId");
} catch (err) {
    if (err instanceof PlantStore.PlantNotFoundError) {
        console.log(err.body.code, err.body.message);
    }
}
```

Errors with an `unknown` or undeclared body type expose `body` as `unknown`.
