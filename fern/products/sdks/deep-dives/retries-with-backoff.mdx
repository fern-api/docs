...
The Java SDK exposes client builder options to tune the exponential backoff applied between retries, alongside `maxRetries`. These are runtime options on the generated client, available on both the root client builder and `ClientOptions.Builder`.

```java {3-5}
PlantStoreClient client = new PlantStoreClient.Builder()
    .maxRetries(3)
...
- `initialRetryDelayMillis`: initial delay in milliseconds used for exponential backoff. Defaults to `1000`.
- `maxRetryDelayMillis`: maximum delay in milliseconds between retries. Defaults to `60000`.
- `retryJitterFactor`: jitter factor between 0 and 1 applied to retry delays. Defaults to `0.2`.
