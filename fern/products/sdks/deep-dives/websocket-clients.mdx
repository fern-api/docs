...
Fern generates the following client code. Send methods are named `send` + the operation name from the spec — for example, the `sendReading` operation produces `sendSendReading` in TypeScript/Python/Java/Rust (C# uses overloaded `Send()` methods instead).
...
The C# SDK generates a `PlantUpdatesApi` class implementing `IAsyncDisposable` with `Event<T>` subscriptions for typed message handling. The WebSocket client is created through the root client's factory method.
