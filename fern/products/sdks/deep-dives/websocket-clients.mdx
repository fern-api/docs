...
Fern generates the following client code. Send methods are named `send` + the operation name from the spec — for example, the `sendReading` operation produces `sendSendReading` in TypeScript/Python/Java/Rust (C# uses overloaded `Send()` methods instead). Receive handling varies by language: TypeScript uses a generic `socket.on("message", handler)` callback, Python returns typed responses from `ws.recv()`, Java generates per-message-type handlers like `onPlantStatus(handler)`, C# exposes typed `Event<T>` properties like `PlantStatus.Subscribe(handler)`, and Rust returns a typed enum from `ws.recv()`.
