...
When no extensions are present, Fern derives group names from operation `tags`. To ignore tags and structure the SDK from each operation's `operationId` instead, enable [`ignore-tags`](/learn/sdks/reference/generators-yml#settingsignore-tags) in `generators.yml`.
