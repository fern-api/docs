...
For APIs that combine this scheme with basic auth under `auth: any`, generated clients select OAuth whenever `client-id-env` and `client-secret-env` resolve, even when basic auth credentials are passed to the constructor. The [Python](/learn/sdks/generators/python/configuration#prefer_explicit_auth), [Ruby](/learn/sdks/generators/ruby/configuration#preferexplicitauth), [C#](/learn/sdks/generators/csharp/configuration#prefer-explicit-auth), and [PHP](/learn/sdks/generators/php/configuration#preferexplicitauth) generators support a prefer-explicit-auth option that makes constructor credentials authoritative.
