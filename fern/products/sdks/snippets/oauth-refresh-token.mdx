...
<ParamField path="endpoint" type="string" required={true}>
  The endpoint that refreshes access tokens (e.g., `"POST /oauth/refresh"` or `"auth.refreshToken"`). If your API uses [namespaces](/learn/sdks/reference/generators-yml#namespace), prefix with the namespace and `::` (e.g., `"iam::POST /oauth/refresh"`).
</ParamField>
