...
<ParamField path="endpoint" type="string" required={true}>
  The endpoint that issues access tokens, such as `'auth.get_token'` or `'POST /oauth/token'`. If your API uses [namespaces](/learn/api-definitions/overview/project-structure#combined-sdks-from-multiple-apis), prefix with the namespace and `::` (e.g., `'iam::POST /oauth/token'`).
</ParamField>
