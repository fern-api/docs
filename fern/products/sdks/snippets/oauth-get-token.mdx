...
<ParamField path="request-properties" type="object" required={false}>
  Maps OAuth parameter names to your API's request field names. Use this when your token endpoint expects different field names than the OAuth standard (e.g., your API uses `clientId` instead of `client_id`). These mappings also determine the field names shown in the [API Explorer](/learn/docs/api-references/api-explorer#oauth-client-credentials) credentials form.
</ParamField>
