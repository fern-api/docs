...
<ParamField path="username.omit, password.omit" type="boolean" required={false} default="false">
  When `true`, removes the corresponding credential from the generated SDK's public API (constructor parameters, type hints, and code samples). The omitted field is encoded as an empty string in the `Authorization: Basic` header — for example, `password.omit: true` produces `base64("username:")` and `username.omit: true` produces `base64(":password")`. If both fields are omitted, no `Authorization` header is sent. Use this when your API only requires one of the two credentials (such as a username-only token or a password-only secret).
</ParamField>
