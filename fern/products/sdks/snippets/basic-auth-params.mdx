...
<ParamField path="username.omit, password.omit" type="boolean" required={false} default="false">
  When `true`, the field is removed from the generated SDK's public API (constructor parameters, type hints, and getters). The omitted field is treated as an empty string when encoding the `Authorization: Basic` header — for example, omitting the password produces `Basic base64("username:")`. When both `username.omit` and `password.omit` are `true`, the SDK skips the `Authorization` header entirely.
</ParamField>