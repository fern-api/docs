...
<ParamField path="username.omit, password.omit" type="boolean" required={false} default="false">
  Omit the username or password from the generated SDK's public API. The omitted field is removed from constructor parameters and treated as an empty string when encoding the `Authorization: Basic` header — for example, setting `password.omit: true` encodes `username:` (trailing colon). Use this when the API's basic auth uses only one of the two fields (e.g., a token in the username and an empty password). If both are omitted, the `Authorization` header is skipped entirely.
