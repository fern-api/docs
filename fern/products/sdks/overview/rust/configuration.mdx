...
Path to a custom license file (e.g., `LICENSE.md`). When set, uses `license-file` instead of `license` in `Cargo.toml`. This is useful when your license isn't a standard SPDX identifier.
</ParamField>
...
Configure custom [Cargo features](https://doc.rust-lang.org/cargo/reference/features.html) for the generated crate:

```yaml title="generators.yml"
config:
  features:
    experimental: ["dep-a", "dep-b"]
    advanced: ["sse"]
  defaultFeatures: ["sse"]
```

<ParamField path="features" type="object" required={false} toc={true}>
Custom Cargo features to include in the generated `Cargo.toml`. Specify as a map of feature names to lists of dependencies or features each one enables. The generator also auto-detects features for file uploads (`multipart`), WebSocket channels (`websocket`), and streaming endpoints (`sse`).
...
Override which features are enabled by default. If not set, the generator automatically includes auto-detected features (such as `multipart`, `websocket`, and `sse`) in the default set.

```yaml title="generators.yml"
config:
  defaultFeatures: ["sse", "multipart"]
```
</ParamField>
...
config:
  customReadmeSections:
    - title: "Custom integration"
      content: "Instructions for integrating with your plant database..."
...
Add custom sections to the generated README file. Each section requires a `title` and `content`. The `content` field supports the `{{ packageName }}` template variable, which is replaced with the crate name during generation.
</ParamField>
