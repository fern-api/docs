...
<ParamField path="sdk_variables" type="object" default="{}" required={false} toc={true}>
  Variables that can be injected into the client constructor to be used as path parameters. Defined using the `x-fern-sdk-variables` extension in OpenAPI specs.
  ...
  This will allow the client to be constructed with the variable that can be used in paths:
  ```python
