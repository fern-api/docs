...
<ParamField path="exclude_types_from_init_exports" type="bool" default="false" required={false} toc={true}>
  When true, types won't be exported in the package's __init__.py file.
</ParamField>
...
<ParamField path="extras" type="object" default="{}" required={false} toc={true}>
  Optional extras/feature flags for the package.
</ParamField>

<ParamField path="flat_layout" type="bool" default="false" required={false} toc={true}>
  When true, generates a flatter package structure without nested directories.
</ParamField>
...
<ParamField path="skip_formatting" type="bool" default="false" required={false} toc={true}>
  When true, skips code formatting of the generated Python code.
</ParamField>
...
<ParamField path="x-fern-sdk-variables" type="object" default="{}" required={false} toc={true}>
  Allows defining reusable variables that can be referenced in path parameters and substituted by the SDK. For example:
  ...
  When defined in your OpenAPI spec, this will allow the generated SDK to automatically handle path parameter substitution for the variable.
</ParamField>
