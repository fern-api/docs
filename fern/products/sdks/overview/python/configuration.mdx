...
<ParamField path="exclude_types_from_init_exports" type="bool" default="false" required={false} toc={true}>
  If true, types will not be exported in the package's __init__.py file.
</ParamField>
...
<ParamField path="extras" type="object" default="{}" required={false} toc={true}>
  Optional dependencies that can be installed with your package.
</ParamField>
...
<ParamField path="include_union_utils" type="bool" default="false" required={false} toc={true}>
  If true, includes utility functions for working with union types.
</ParamField>
...
<ParamField path="skip_formatting" type="bool" default="false" required={false} toc={true}>
  If true, skips code formatting with Black.
</ParamField>
...
<ParamField path="use_api_name_in_package" type="bool" default="false" required={false} toc={true}>
  If true, includes the API name in the generated package name.
</ParamField>
...
The Python generator (v4.24.0+) supports setting extension headers through the `x-fern-sdk-variables` field in your OpenAPI spec:
...
This allows you to define variables that will be injected into path parameters automatically when making requests. The variables are set when initializing the client:

```python
from my_sdk import Client

client = Client(
    project_id="proj_123" # Will be injected into paths automatically
)
```