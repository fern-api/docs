...
The Python generator supports injecting additional path parameters via extension headers. This feature is configured using the `x-fern-sdk-variables` property in your OpenAPI spec:
...
These variables are then passed through the client constructor and automatically injected into relevant path parameters:

```python
client = YourClient(
    project_id="proj_abc123"  # Will be injected into paths with {project_id}
)
```
