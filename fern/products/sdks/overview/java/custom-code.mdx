...
```java title="src/main/java/com/example/CustomClientBuilder.java" {5}
...
### For authentication, override `build()` instead

```java
@Override
public CustomClient build() {
    if (customAuth != null) {
        this.addHeader("Authorization", customAuth);
    }
    return new CustomClient(buildClientOptions());
}
```
