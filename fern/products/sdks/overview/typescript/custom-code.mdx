...
<Steps>
    ...
    ### Add a package export entry

    Use the [`packageJson.exports`](/learn/sdks/generators/typescript/configuration#packagejson) config in `generators.yml` to add your custom export entry:
    ...
    The export key (e.g., `"./helper"`) determines the import path your users will use and doesn't need to match the source file path. The dist paths point to the compiled output.
    
    <Note>
      The TypeScript generator auto-generates the `exports` field in `package.json` based on your API definition. Since Node.js [enforces the `exports` map](https://nodejs.org/api/packages.html#package-entry-points), custom files aren't importable until you add them to `packageJson.exports`. Custom files in `packageJson.exports` are merged into the generated `exports` (not overwritten), so they persist across regenerations. 
    </Note>
    ...
    ```typescript
    import { myHelper } from "@acme/sdk/helper";
