...
<Steps>
    ...
    The TypeScript generator auto-generates the `exports` field in `package.json` based on your API definition. Custom files aren't included in this map, and since Node.js [enforces the `exports` map](https://nodejs.org/api/packages.html#package-entry-points), your custom functions won't be importable without this step.

    Use the [`packageJson.exports`](/sdks/generators/typescript/configuration#packageJson) config in `generators.yml` to add your custom export entry:
    ...
    These entries are merged into the generated `exports` (not overwritten), so they persist across regenerations.

    <Tip>Use a clean export path like `"./helper"` rather than `"./src/helper.ts"`. This gives your users a cleaner import path and avoids exposing internal source structure.</Tip>
