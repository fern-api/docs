...
The TypeScript generator auto-generates the `exports` field in `package.json` based on your API definition. Custom files aren't included in this map, and since Node.js [enforces the `exports` map](https://nodejs.org/api/packages.html#package-entry-points), your custom functions won't be importable by default.
...
<Warning>Make sure the custom source files are also listed in `.fernignore` so they're not deleted on regeneration.</Warning>
