...
<Steps>
    ...
    These entries are merged into the generated `exports` (not overwritten), so they persist across regenerations. The export key (e.g., `"./helper"`) determines the import path your users will use, while the dist paths point to the compiled output.
