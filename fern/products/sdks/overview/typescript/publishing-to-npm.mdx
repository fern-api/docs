...
<Info>
	...
	* A GitHub repository for your TypeScript SDK. See [Set up GitHub repositories](/sdks/overview/quickstart#set-up-github-repositories).
...
2. **SDK repo on GitHub**: Publishing requires a separate GitHub repo for the TypeScript SDK (e.g., `<org>/<api>-typescript-sdk`). If it doesn't exist yet, create it:
   ```bash
   gh repo create <org>/<api>-typescript-sdk --private
   ```
   Then install the [Fern GitHub App](https://github.com/apps/fern-api) on both repos (browser step — provide the link).
3. **TypeScript generator group**: If they skipped the [TypeScript quickstart](quickstart), they need to run `fern add fern-typescript-sdk --group ts-sdk` first.
