...
<ParamField path="sdkClientVariables" type="Record<string, { type: string; description?: string }>" toc={true}>
Specify variables to be included in the client constructor that can be used to inject common parameters. For example:
...
This will generate a client that accepts `projectId` in the constructor:
...
The variable will be available internally to inject into requests. You can define this behavior using OpenAPI extensions:
...
Now the client will automatically inject the `projectId` into the path parameter.
</ParamField>
