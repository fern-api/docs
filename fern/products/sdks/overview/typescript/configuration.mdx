...
<ParamField path="includeSdkClientHeaders" type="boolean" default="false" toc={true}>
When enabled, adds standard SDK client headers to requests including:
- x-fern-sdk-name
- x-fern-sdk-version
- x-fern-sdk-id
</ParamField>
...
<ParamField path="sdkVariables" type="Record<string, { type: string; description?: string }>" toc={true}>
Define variables that will be passed to the client constructor and injected into paths and headers. This is useful for injecting values like project IDs, API versions, etc.
...
The variable will be automatically injected into paths that contain `{project_id}`:
```yaml
paths:
...
In the generated TypeScript SDK:
```typescript
const client = new Client({ 
  projectId: "proj_123"  // Required constructor param
})
