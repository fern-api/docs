...
<ParamField path="sdkVariables" type="object" toc={true}>
Configure variables that can be injected when constructing the client. This allows you to specify request parameters that are common across multiple endpoints but don't want to repeat in each request.

```yaml
# OpenAPI schema with x-fern-sdk-variables extension
...
When generating the SDK, these variables will be made available in the client constructor:

```typescript
const client = new Client({
  projectId: "proj_123" // Will be injected into all requests that use project_id
});

// No need to pass project_id in each request
...
<Warning>Response validation only occurs