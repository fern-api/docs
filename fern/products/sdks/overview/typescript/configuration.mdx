...
You can also use `packageJson.exports` to add custom export entries. The generator only auto-generates export entries for your API definition, so custom files need to be added manually—otherwise Node.js won't resolve imports for them. See [Adding custom code](/sdks/generators/typescript/custom-code) for details.
</ParamField>
