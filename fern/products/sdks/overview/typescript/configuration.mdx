...
<ParamField path="extensionHeaders" type="Record<string, string>" toc={true}>
Add custom extension headers to every request made by the client. For example, to inject a project ID into the path:

```yaml
# generators.yml
config:
  extensionHeaders:
    "x-fern-sdk-variables": '{"project_id": "proj_abc123"}'
```

This would automatically inject the project_id variable into any path that uses it as a parameter.
</ParamField>
