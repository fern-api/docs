...
Bundle the generated SDK code into a single file.
</ParamField>
...
Configure SDK variables that should be passed as headers on every request:
...
This allows users to configure these variables once when initializing the client:
...
The header will be automatically included on all requests.
</ParamField>
...
Include an "other" catch-all type in union types for unknown values.
</ParamField>

<ParamField path="includeUtilsOnUnionMembers" type="boolean" toc={true}>
Include utility functions on union type members for type checking.
</ParamField>
...
<Warning>Response validation only occurs when the Serde layer is