...
<ParamField path="fernHeaders" type="array" toc={true}>
Add custom Fern headers to the generated client's requests. Useful for injecting variables or dynamic values into paths and requests:

```yaml
# generators.yml 
config:
  fernHeaders:
    - name: x-fern-sdk-project-id
      description: "The project_id variable that will be injected into paths"
```

The header value can be provided when instantiating the client and will be used in request paths marked with `x-fern-sdk-variable`:

```typescript
const client = new ApiClient({
  headers: {
    "x-fern-sdk-project-id": "proj_123" 
  }
});
```
