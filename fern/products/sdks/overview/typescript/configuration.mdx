...
You can also use `packageJson` to add custom `exports` entries. This is required when you [add custom code](/sdks/generators/typescript/custom-code) to your SDK, because the generator only auto-generates export entries for your API definition. Without this, Node.js won't resolve imports for your custom files since it [enforces the `exports` map](https://nodejs.org/api/packages.html#package-entry-points).
...
<Note>Use the `packageJson` config option instead of `.fernignore` to prevent package.json from being overwritten.</Note>
</ParamField>
