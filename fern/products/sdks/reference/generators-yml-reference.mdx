...
<ParamField path="audiences" type="list of strings" required={false}>
  Filter which API elements are included in the generated SDK based on audience tags. Only endpoints, schemas, and properties tagged with the specified audiences in your API spec will be included. Without this filter, all endpoints are included regardless of their audience tags.

  To use audiences, first tag your API elements with `x-fern-audiences` in OpenAPI or `audiences` in Fern Definition, then specify which audiences to include here. For a complete guide, see [Filter your endpoints (audiences)](/learn/sdks/guides/filter-your-endpoints-audiences).
