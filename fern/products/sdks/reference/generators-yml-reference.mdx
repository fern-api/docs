...
<ParamField path="auth" type="string or object" toc={true}>
  ...
  To delegate authentication to individual endpoints (per-endpoint security), use `endpoint-security`:
  ...
  With this configuration, each endpoint uses the `security` requirements defined in your OpenAPI spec. The generated SDK accepts credentials for all defined schemes and routes them to the correct endpoints automatically. See [endpoint-level authentication](/learn/api-definitions/ferndef/authentication#endpoint-level-authentication) for details.
