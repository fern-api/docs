...
<ParamField path="settings" type="object" toc={true}>
  Global API settings that apply to all specifications. These settings can be overridden at the spec level or generator level. The precedence hierarchy is: generator-level settings > spec-level settings > global settings.
  ...
  api:
    ...
    specs:
      - openapi: "./openapi.yml"
        settings:
          # Spec-level settings override global settings
    ...
    production:
      generators:
        - name: fernapi/fern-typescript-node-sdk
          ...
          api:
            settings:
              # Generator-level settings override both spec and global settings
              coerce-enums-to-literals: false
  ```

  Global settings are useful for configuration flags that apply to the IR generator and should be consistent across all specifications and generators. For available settings, see the specification type documentation below.
</ParamField>
