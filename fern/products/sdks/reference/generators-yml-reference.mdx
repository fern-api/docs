...
Define shortcuts that map to multiple generator groups, allowing you to [run several groups with a single command](/learn/cli-api-reference/cli-reference/commands#group). When you run `fern generate <alias>`, all groups in the alias run in parallel.
