...
<ParamField path="asyncapi" type="string | git source" required={true} toc={true}>
  Path to the AsyncAPI specification file. Accepts either a local file path (string) or a [remote git reference](#git-remote-sources).
  ...
  asyncapi:
    git:
      repo: https://github.com/org/api-specs.git
      ref: v2.0.0        # optional — defaults to the repo's default branch
      path: asyncapi/events.yml
  ```
...
Instead of referencing a local file path, the `openapi`, `asyncapi`, and proto `root` fields accept a `git` object that points to a file or directory in a remote git repository. At generation time, the CLI shallow-clones the repository at the specified ref and resolves the path locally.
...
<ParamField path="git.repo" type="string" required={true} toc={true}>
  The git repository URL (HTTPS or SSH).
</ParamField>

<ParamField path="git.ref" type="string" toc={true}>
  Branch, tag, or commit SHA to check out. Defaults to the repository's default branch when omitted.
</ParamField>

<ParamField path="git.path" type="string" required={true} toc={true}>
  Path to the spec file or directory within the repository.
</ParamField>

**Authentication** relies on your environment's native git credential resolution — credential helpers, SSH keys, or `GIT_ASKPASS`. No tokens are stored in `generators.yml`.

**Deduplication**: if multiple specs reference the same repository and ref, the CLI clones it only once.
