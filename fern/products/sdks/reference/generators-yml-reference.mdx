...
Define shortcuts that map to multiple generator groups, allowing you to run several groups with a single command. When you run `fern generate <alias>`, all groups in the alias run in parallel.
...
With this configuration, running `fern generate all` generates all three SDKs in parallel, while `fern generate backend` generates only the PHP and Go SDKs.
