...
<ParamField path="settings.title-as-schema-name" type="boolean" default="false" toc={true}>
  ...
  <Callout intent="info">
    The default changed from `true` to `false` in CLI v1.0.0 (2025-11-13). For existing workspaces, the CLI automatically upgrades `generators.yml` to explicitly set the pre-v1.0.0 default so behavior remains unchanged.
  </Callout>
</ParamField>
...
<ParamField path="settings.respect-nullable-schemas" type="boolean" default="true" toc={true}>
  ...
  <Callout intent="info">
    The default changed from `false` to `true` in CLI v1.0.0 (2025-11-13). For existing workspaces, the CLI automatically upgrades `generators.yml` to explicitly set the pre-v1.0.0 default so behavior remains unchanged.
  </Callout>
</ParamField>
