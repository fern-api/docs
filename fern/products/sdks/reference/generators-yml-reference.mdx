...
<ParamField path="smart-casing-digit-word-boundary" type="boolean" default="false">
Adds a word boundary after a digit run in snake_case output (e.g., `int32value` becomes `int32_value`). Off by default to preserve existing identifier casing. Only takes effect when `smart-casing` is enabled. Supported by the TypeScript, Python, Go, and Java generators.
</ParamField>
