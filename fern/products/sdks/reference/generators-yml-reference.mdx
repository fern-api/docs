...
<ParamField path="auth" type="string | object" toc={true}>
  ...
  Both schemes must also be declared in the OpenAPI spec's `securitySchemes` and listed in its `security` array. See [multiple auth schemes for CLIs](/learn/cli-generator/get-started/authentication#multiple-auth-schemes) for a full walkthrough.
