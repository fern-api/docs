...
<ParamField path="audiences" type="list of strings" required={false} toc={true}>
  ...
  To use audiences, first tag your API elements with [`x-fern-audiences` in OpenAPI](/learn/api-definitions/openapi/extensions/audiences) or [`audiences` in Fern Definition](/learn/api-definitions/ferndef/audiences), then specify which audiences to include here. For a complete guide on configuring audiences, see [Filter your endpoints](/learn/sdks/deep-dives/audiences).

  For path-based filtering instead of tag-based filtering, use [`settings.filter`](#settingsfilter) at the spec level (OpenAPI).
</ParamField>
