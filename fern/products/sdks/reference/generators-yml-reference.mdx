...
<ParamField path="settings.auto-generate-idempotency-key" type="boolean or object" default={false} toc={true}>
  Auto-generates an idempotency-key header on retry-unsafe requests (`POST`, `PUT`) when the caller doesn't supply one. Applies to every generator in the API, so it doesn't have to be set on each generator's `config`.

  Set to `true` to enable with defaults, or use the object form to set a custom header name and the eligible methods:
  ...
  A generator's own [`config.auto-generate-idempotency-key`](#configauto-generate-idempotency-key) overrides this value for that generator, including an explicit `false` to opt out.
</ParamField>
...
<ParamField path="config.auto-generate-idempotency-key" type="boolean or object" default={false} toc={true}>
  Enables idempotency-key auto-generation for this generator, overriding the API-level [`api.settings.auto-generate-idempotency-key`](#settingsauto-generate-idempotency-key). Accepts the same boolean shorthand or object form (`header-name`, `methods`). Set it to `false` to opt this generator out of an API-level default.
