...
<ParamField path="smart-casing" type="boolean" default="false" toc={true}>
...
Pair this with [`smart-casing-digit-word-boundary`](/learn/sdks/reference/generators-yml#smart-casing-digit-word-boundary) to also break snake_case identifiers after a digit run.
</ParamField>

<ParamField path="smart-casing-digit-word-boundary" type="boolean" default="false" toc={true}>
Adds a word boundary after a digit run in snake_case output (e.g., `int32value` becomes `int32_value`). Off by default to preserve existing identifier casing. Only takes effect when [`smart-casing`](/learn/sdks/reference/generators-yml#smart-casing) is enabled. Supported by the TypeScript, Python, Go, and Java generators.
</ParamField>
