...
The default network request timeout, in seconds, baked into the generated SDK's `ClientOptions.Timeout`. Set to `infinity` to disable timeouts (the generated client uses `Timeout.InfiniteTimeSpan`). Defaults to 30 seconds when unset. SDK users can still override this per-request via `RequestOptions.Timeout`.
</ParamField>
