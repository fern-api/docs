...
Automatically attaches an idempotency-key header to eligible requests (`POST` and `PUT` by default) unless the caller provides one. Set `false` to opt out of the API-wide [`api.settings.auto-generate-idempotency-key`](/learn/sdks/reference/generators-yml#settings), or pass an object to customize `header-name` and `methods`. See [Auto-generate idempotency keys](/learn/sdks/deep-dives/idempotency#auto-generate-idempotency-keys).
</ParamField>
