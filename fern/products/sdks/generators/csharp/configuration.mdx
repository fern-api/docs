...
Deprecated. Use [`default-timeout-in-milliseconds`](#default-timeout-in-milliseconds) instead. The default timeout for network requests, in seconds, converted to milliseconds internally. When both keys are set, `default-timeout-in-milliseconds` takes precedence. Set to `infinity` to disable the default timeout. SDK users can still [override this per request](/learn/sdks/deep-dives/sdk-user-features#customization-options) by passing `RequestOptions.Timeout`.
</ParamField>
