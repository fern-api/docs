...
<ParamField path="prefer-explicit-auth" type="boolean" default="false" required={false} toc={true}>
When enabled, credentials passed explicitly to the client constructor determine the auth scheme, taking precedence over credentials read from environment variables.

Enable this for APIs that combine [OAuth client credentials](/learn/sdks/reference/generators-yml#auth-schemes) with basic auth under `auth: any`, where the client otherwise selects OAuth whenever the OAuth environment variables are set, ignoring basic auth credentials passed to the constructor.
</ParamField>
