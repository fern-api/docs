...
Custom interceptors run in registration order, after the retry, logging, and response decompression interceptors Fern installs. The retry interceptor wraps them, so every retried attempt passes through them again: a signing interceptor recomputes its signature per attempt rather than replaying the signature from the first attempt.

For OAuth APIs the builder is staged, and `addInterceptor` is available at each stage. `builder()` returns an intermediate builder, `token(...)` or `credentials(...)` returns the auth-specific builder, and interceptors registered before the auth call are forwarded to it:
...
- **Fern Java SDK version**: 3.41.0 or later, or 3.42.5 or later for OAuth staged builders
- **Configuration**: `custom-interceptors: true` in `generators.yml`
