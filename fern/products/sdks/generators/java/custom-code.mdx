...
SDK users register interceptors when they construct the client, and each one is applied to the underlying `OkHttpClient` at build time:
...
YourApiClient client = YourApiClient.builder()
    ...
    .addInterceptor(chain -> {
        ...
        return chain.proceed(request.newBuilder()
            .header("X-Signature", sign(request))
            .build());
    })
    .build();
```

Interceptor registration code belongs in a file listed in `.fernignore` so regeneration doesn't overwrite it. The builder also exposes a protected `setInterceptors(ClientOptions.Builder)` method, so a [custom builder](#adding-custom-client-configuration) can override it to attach interceptors on behalf of its users rather than requiring each caller to register them.
