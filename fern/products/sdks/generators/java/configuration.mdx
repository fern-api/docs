...
The Java generator formats its output with `./gradlew :spotlessApply`, which downloads Gradle and the Spotless plugin from `services.gradle.org` and `plugins.gradle.org`. On networks that block those hosts, set `FERN_JAVA_SKIP_FORMATTING=true` (generator 4.18.0 or later) to skip the formatting pass:

```bash
FERN_JAVA_SKIP_FORMATTING=true fern generate --group java-sdk
```

The CLI forwards the variable into the generator container. Generated code is left unformatted; run `./gradlew spotlessApply` in the SDK repository to format it.
