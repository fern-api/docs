...
<ParamField path="respect-optional-request-body" type="boolean" default="false" required={false} toc={true}>
When enabled, endpoints whose API definition doesn't mark the request body as required get an extra overload without the body parameter, avoiding an `Optional<T>` parameter and the staged builder it would require. Calling the overload sends no body and no `Content-Type` header instead of an empty JSON object.

```java
waterPlant(String plantId);
waterPlant(String plantId, WaterRequest request);
```

This applies to referenced request bodies: a body declared as a single named type. Required bodies and inlined request properties are unaffected.
</ParamField>
