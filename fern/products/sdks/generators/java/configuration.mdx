...
When enabled, the generated client builder exposes an `addInterceptor(Interceptor)` method for registering [custom OkHttp interceptors](/learn/sdks/generators/java/custom-code#adding-custom-interceptors). Interceptors are applied to the `OkHttpClient` when the client is built, which covers HTTP-level customizations such as request signing and public key client validation. Requires generator version 3.41.0 or later.
</ParamField>
