...
<ParamField path="respectAuthSchemeNames" type="boolean" default="false" required={false} toc={true}>
Names the client's credential keyword arguments after the auth schemes configured in your API definition, snake-cased, matching the TypeScript and Python generators. For example, `token: { name: apiKey }` produces `Client.new(api_key:)` instead of `Client.new(token:)`. A name that collides with a built-in client keyword such as `base_url` or `environment` gets an `_auth` suffix, and a global header that collides with a credential gets a `header_` prefix.

Off by default because enabling it renames the keyword argument for existing callers of a published gem.
</ParamField>
