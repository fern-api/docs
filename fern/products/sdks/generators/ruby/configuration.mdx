...
Names the client's credential keyword arguments after the auth schemes in your API definition. For example, `token: { name: apiKey }` produces `Client.new(api_key:)` instead of `Client.new(token:)`. Enabling it renames the keyword argument for existing callers of a published gem.
</ParamField>
