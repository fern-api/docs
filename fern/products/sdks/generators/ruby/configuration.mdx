...
When enabled, the caller may omit the request body on endpoints whose API definition doesn't mark it as required, and a bodyless call sends `nil` with no `Content-Type` header instead of an empty JSON object. Form-urlencoded bodies are excluded.
