...
<ScrollWalkthrough>
  <Step title="Configure `output` location" file="generators.yml" focus="6-7">
  ...
  <Step title="Add a unique package name" file="generators.yml" focus="8">
    Your package name must be unique in the npmjs registry, otherwise publishing your SDK will fail.
  </Step>

  <Step title="Configure `namespaceExport`" file="generators.yml" focus="9-10">
...
<Steps>
	...
	OIDC can't perform a package's first publish: npm requires the package to exist before you can add a trusted publisher. Publish a placeholder version to create it:
