...
When enabled, the request argument becomes optional on endpoints whose API definition doesn't mark the request body as required, and a call that omits it sends no body and no `Content-Type` header instead of an empty JSON object. The body keeps its own type rather than being widened to an optional one.
