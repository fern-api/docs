...
Takes precedence over the deprecated `defaultTimeoutInSeconds` and `timeoutInSeconds`.
</ParamField>

<ParamField path="defaultTimeoutInSeconds" type="number | 'infinity'" toc={true}>
<Warning>Deprecated. Use `defaultTimeout` instead.</Warning>
The default timeout for network requests, in seconds. In the generated client, this can be overridden at the request level. The value is converted to milliseconds, and `defaultTimeout` takes precedence when set.
</ParamField>
