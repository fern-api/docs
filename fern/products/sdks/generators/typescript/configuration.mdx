...
Wraps environment variable reads for credentials in the generated auth providers (API key, bearer, basic, and OAuth) and for the [base URL](/learn/api-definitions/ferndef/api-yml/environments#base-url-from-an-environment-variable) (`base-url-env`) in a `typeof process !== "undefined"` check. Enable this when the SDK runs in browsers, Cloudflare Workers, Deno, or other runtimes without a Node `process` global; otherwise `process.env` reads throw a `ReferenceError` when a credential isn't passed to the client. Node.js behavior is unchanged.
...
When `baseUrl` is optional, the client (and any sub-client constructed standalone) resolves the base URL in this order: `baseUrl`, `environment`, the environment variable named by [`base-url-env`](/learn/api-definitions/ferndef/api-yml/environments#base-url-from-an-environment-variable) in your API definition, then the default environment.
</ParamField>
...
<ParamField path="websocketHandlerMode" type="'replace' | 'accumulate'" default="replace" toc={true}>
  Controls what happens when you register more than one handler for the same event on a generated WebSocket socket. Requires `generateWebSocketClients: true`.

  * `replace`: registering a second handler for an event replaces the first.
  * `accumulate`: handlers for the same event accumulate and fire in registration order, like an `EventEmitter`.

  In both modes, `socket.off(event, handler)` detaches a handler.
