...
<ParamField path="timeout" type="number | 'infinity'" default="60" required={false} toc={true}>
  Alias for `timeout_in_seconds`. Sets the client timeout in seconds, or `infinity` to disable timeouts. Takes precedence over `timeout_in_seconds` when both are set.
</ParamField>

<ParamField path="timeout_in_seconds" type="number | 'infinity'" default="60" required={false} toc={true}>
  By default, the generator generates a client that times out after 60 seconds. You can customize this value by providing a different number or setting to `infinity` to get rid of timeouts. Aliased by `timeout`, which takes precedence when both are set.
</ParamField>
