...
When enabled, the body parameter defaults to the `OMIT` sentinel on endpoints whose API definition doesn't mark the request body as required, so an explicit `None` stays distinguishable from a body that was never passed. A call that omits the body sends no body and no `Content-Type` header instead of an empty JSON object.

```python
def water_plant(self, plant_id: str, *, request: WaterRequest = OMIT, ...)
```

This applies to referenced request bodies: a body declared as a single named type. Required bodies and inlined request parameters are unaffected.
</ParamField>
