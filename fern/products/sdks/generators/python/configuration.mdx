...
<ParamField path="prefer_explicit_auth" type="bool" default="false" required={false} toc={true}>
When enabled, credentials passed explicitly to the client constructor determine the auth scheme, taking precedence over credentials read from environment variables. Also accepted as `prefer-explicit-auth` or `preferExplicitAuth`.
