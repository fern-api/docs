## 5.14.8
**`(fix):`** Fix `construct_type` crash on bare unparameterized `dict`, `list`, and `set` types.
...
## 5.14.7
**`(fix):`** Fix offset pagination so generated `list` endpoints emit a real `_has_next` instead of the
