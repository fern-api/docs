...
When enabled, a request wrapper with a nil `Body` sends no body and no `Content-Type` header on endpoints whose API definition doesn't mark the request body as required, instead of the JSON literal `null`.
