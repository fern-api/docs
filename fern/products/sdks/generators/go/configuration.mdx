...
<ParamField path="applyQueryDefaultsOnNilRequest" type="boolean" default="false" required={false} toc={true}>
When enabled, passing `nil` as the request argument encodes query parameters as if a zero-value request struct were passed, so query parameter defaults are still sent. By default, `nil` requests drop those defaults.
</ParamField>
