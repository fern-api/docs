...
Percent-encodes path parameter values as the generated SDK substitutes them into the request path, so a value containing `/` or `..` can't change which endpoint the request resolves to. A `plantId` of `../plants` is sent as `..%2Fplants`, and values that intentionally contain slashes, such as file-path-like IDs, are sent with `%2F`.
</ParamField>
