...
When enabled, the caller may omit the request body argument on endpoints whose API definition doesn't mark the request body as required, and the request is then sent with no body and no `Content-Type` header instead of an empty JSON object.
