...
<Note>
Custom container registries are only supported with local generation (`--local`). Remote generation through Fern's cloud infrastructure doesn't support custom registries.
</Note>

To use a custom registry, replace the `name` field in your generator configuration with an [`image`](/learn/sdks/reference/generators-yml#image) object containing `name` and `registry`:
...
- **Remote generation**: Custom images are blocked for remote generation (`fern generate` without `--local`). Only Fern's trusted images can run on Fern's cloud infrastructure.
- **Auto-upgrade**: Generators configured with a custom `image` are skipped during `fern generator upgrade`. You must manually update the version for these generators.
