...
## Using auto versioning with self-hosted mode

When running SDK generation in self-hosted (`--local`) mode, you can use auto versioning to automatically generate version numbers for your SDKs. This is particularly useful when combined with AI-powered versioning through Bedrock.

### Configuration

To enable auto versioning with self-hosted mode, you need to configure both your `generators.yml` file and set the required environment variables.

#### Step 1: Configure AI provider in generators.yml

Add the AI configuration at the top level of your `generators.yml` file:

```yaml title="generators.yml"
ai:
  provider: bedrock
  model: <your_favorite_bedrock_model>

# Rest of your generators.yml configuration
api:
  specs:
    - openapi: "./openapi.yml"
...
Before running `fern generate`, export the required AWS credentials as environment variables:
...
#### Step 3: Run fern generate with --version AUTO

When running the `fern generate` command, specify `--version AUTO` to enable automatic version generation:

```bash
fern generate --version AUTO
```

The AI provider will analyze your changes and automatically determine an appropriate version number based on the semantic versioning conventions.
