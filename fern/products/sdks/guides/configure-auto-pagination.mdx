...
To set up auto pagination for OpenAPI, annotate the desired paginated endpoint with the [`x-fern-pagination`](/api-definitions/openapi/extensions/pagination) extension, specify the pagination scheme (`offset` or `cursor`), and specify where your `results` are located using dot-access notation.
