...
Your API may use certain headers for every endpoint or most endpoints. These are called **global headers**. When configured, Fern generates an SDK that accepts global headers as constructor parameters and automatically includes them in all API calls.
...
Add global headers using the `x-fern-global-headers` extension:
...
Specify headers meant to be included on every request in `api.yml`:
