...
<ParamField path="settings.type-dates-as-strings" type="boolean" default="false" toc={true}>
  控制 `format: date` 或 `format: date-time` 的 `string` 模式如何被类型化。当为 `false` 时，它们会成为 Fern 专用的 `date` 和 `datetime` 类型。当为 `true` 时，它们保持为 `string`，并将 format 作为校验保留，因此 API 参考文档会将它们渲染为带有 `format: "date"` 或 `format: "date-time"` 标签的 `string`，并且 SDK 会将它们类型化为字符串。
</ParamField>
