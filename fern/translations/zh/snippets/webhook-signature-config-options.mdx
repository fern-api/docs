...
<ParamField path="payload-format.components" type="list of strings" required={false} toc={true}>
  签名前要连接的负载组件的有序列表。省略时，仅对原始请求正文进行签名。可用组件：
  ...
  <Warning>如果组件列表中不包含 `timestamp`，`timestamp.tolerance` 检查将无法防止重放攻击，因为时间戳未经过身份验证。请包含 `timestamp` 以确保重放保护。</Warning>
</ParamField>

<ParamField path="payload-format.delimiter" type="string" required={false} toc={true}>
  用于连接组件的字符串。对于带时间戳前缀的负载使用 `"."`，对于直接连接使用 `""`。设置 `payload-format.components` 时必需。
</ParamField>
