...
<Note title="Fern 会自动解析 `operationId`">
  如果没有扩展存在，Fern 使用您的 operation ID 来生成 SDK 方法名称。将 operation ID 格式化为 `{tag_name}_{operation_name}`（例如：`users_get`）来自动生成像 `users.get()` 这样的方法。如果 operation ID 不以标签开头，Fern 会直接使用它作为方法名称。
</Note>
...
如果您添加多个 `x-fern-sdk-group-name` 扩展，那么生成的 SDK 将嵌套组名称。生成的 SDK 方法保持组名称的顺序。
