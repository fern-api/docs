...
>
  Ask AI
  应该索引和搜索的其他内容源。有关更多详细信息，请参阅[其他内容源](/learn/docs/ai-features/ask-fern/content-sources)。
</ParamField>

<ParamField path="datasources.url" type="string" required={true} toc={true}>
  要索引的网站 URL。Ask AI 将爬取并索引此 URL 的内容。
</ParamField>

<ParamField path="datasources.title" type="string" toc={true}>
  此数据源的可选显示名称。当 Ask AI
  引用来自此源的内容时，这有助于用户了解信息来源。
</ParamField>

<ParamField path="ai-search.system-prompt" type="string" toc={true}>
  默认情况下，Ask AI 使用系统提示词来优化 AI
  搜索结果。添加自定义提示词以覆盖默认设置。 请参阅 Anthropic
  的[提示词指南](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview)获取想法和示例。
</ParamField>
