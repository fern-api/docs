...
在您的 fern 目录中创建一个 `generators.yml` 文件，并添加对您的 gRPC proto 文件的引用。请参见 [gRPC generators.yml 参考](/learn/zh/api-definitions/grpc/generators-yml-reference)以获取完整的配置选项。
...
<Note>
如果您想要[本地编译 `.proto` 文件 (`local-generation: true`)](/learn/zh/api-definitions/grpc/generators-yml-reference#local-generation)，您必须在您的机器上或[在您的 CI/CD 环境中](/learn/zh/api-definitions/grpc/sync-your-g-rpc-specification)安装 [buf](https://buf.build/docs/installation)。
</Note>
