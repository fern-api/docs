...
<Steps>
  ...
  <Step title="添加 GitHub Action">
    在您的仓库中创建 `.github/workflows/sync-openapi.yml`。此操作使用 [`fern api update`](/learn/cli-api-reference/cli-reference/commands#fern-api-update) 从您的 `generators.yml` 文件中的 `origin` 字段拉取最新版本的 OpenAPI 规范。 
