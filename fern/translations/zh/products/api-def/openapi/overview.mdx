...
- **使用合适的项目结构进行组织。** 按照[项目结构](/learn/api-definitions/overview/project-structure)中的说明，清晰地组织包含您的定义和其他相关文件的目录。
- **为端点添加 `operationId`。** 为每个端点包含清晰的 `operationId`，以控制在 SDK 中生成的函数名称。（或使用[扩展来自定义组和方法名称](/learn/api-definitions/openapi/extensions/method-names)。）
...
- **使用覆盖层和 Fern 扩展进行自定义。** 使用 Fern [扩展](/learn/api-definitions/openapi/extensions/overview)来自定义您的规范，这些扩展保存在[覆盖层文件](/learn/api-definitions/openapi/overlays)中。这使您能够修改生成行为，而不更改核心 OpenAPI 定义。
