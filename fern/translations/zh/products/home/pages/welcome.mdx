---
title: 使用 Fern 构建
slug: /
description: 构建更好的开发者体验。生成 TypeScript、Python、Go、Java、C#、PHP、Ruby、Swift 和 Rust 的 SDK。创建带有 AI 搜索的交互式 API 文档。
hide-toc: true
layout: custom
---


<llms-ignore>
<div className="lp-page-container">
  {/* Main Content */}
  <div className="main-content">
    {/* Dashed Pattern */}
    <div className="dashed-pattern dashed-pattern-left"></div>
    <div className="dashed-pattern dashed-pattern-right"></div>
    {/* Hero Section */}
    <div className="hero-section">
      <div className="hero-title-container">
        <h1 className="hero-title" data-state="closed">
          使用 Fern 构建
        </h1>
        <p className="hero-description">
          从 SDK、文档或两者开始。
        </p>
      </div>
    </div>

    {/* Feature Grid */}
    <div className="feature-grid">
      {/* SDKs Card */}
      <div className="panel-card sdks-card">
        <div className="card-header">
          <a className="card-title" href="/learn/zh/sdks/overview/introduction">
            SDK
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden m-0" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block m-0" noZoom />
          </a>
          <p className="card-description">
            生成多种语言的客户端库。
          </p>
        </div>
        
        {/* Rive Animation */}
        <div className="sdk-rive">
          <div className="rive-container">
            <canvas id="sdk-rive-canvas"></canvas>
          </div>
        </div>

        {/* Language Icons */}
        <div className="language-icons">
          <span className="language-icons-label">开始使用：</span>
          {/* TypeScript */}
          <a className="language-icon" href="/learn/zh/sdks/generators/typescript/quickstart">
            <img src="./images/ts-logo.svg" alt="TypeScript" className="language-icon-img" noZoom />
          </a>
          {/* Python */}
          <a className="language-icon" href="/learn/zh/sdks/generators/python/quickstart">
            <img src="./images/python-logo.svg" alt="Python" className="language-icon-img" noZoom />
          </a> 
          {/* Go */}
          <a className="language-icon" href="/learn/zh/sdks/generators/go/quickstart">
            <img src="./images/go-logo.svg" alt="Go" className="language-icon-img" noZoom />
          </a>
          {/* Java */}
          <a className="language-icon" href="/learn/zh/sdks/generators/java/quickstart">
            <img src="./images/java-logo.svg" alt="Java" className="language-icon-img" noZoom />
          </a>
          {/* C# */}
          <a className="language-icon" href="/learn/zh/sdks/generators/csharp/quickstart">
            <img src="./images/csharp-logo.svg" alt="C#" className="language-icon-img" noZoom />
          </a>
          {/* PHP */}
          <a className="language-icon" href="/learn/zh/sdks/generators/php/quickstart">
            <img src="./images/php-logo.svg" alt="PHP" className="language-icon-img" noZoom />
          </a>
          {/* Ruby */}
          <a className="language-icon" href="/learn/zh/sdks/generators/ruby/quickstart">
            <img src="./images/ruby-logo.svg" alt="Ruby" className="language-icon-img" noZoom />
          </a>
          <a className="language-icon" href="/learn/zh/sdks/generators/swift/quickstart">
              <img src="./images/swift-logo.svg" alt="Swift" className="language-icon-img" noZoom />
          </a>
          <a className="language-icon" href="/learn/zh/sdks/generators/rust/quickstart">
              <img src="./images/rust-light-mode.svg" alt="Rust" className="language-icon-img dark:hidden" noZoom />
              <img src="./images/rust-dark-mode.svg" alt="Rust" className="language-icon-img hidden dark:block" noZoom />
            </a>
        </div>

        {/* Action Buttons */}
        <div className="action-buttons">
          <a className="fern-button filled normal primary gap-1 a-btn" href="/learn/zh/sdks/overview/introduction">
            介绍
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal gap-1 a-btn" href="/learn/zh/sdks/overview/quickstart">
            快速开始
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal gap-1 a-btn" href="https://buildwithfern.com/showcase">
            客户案例
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
        </div>
      </div>

      {/* Docs Card */}
      <div className="panel-card with-left-border docs-card">
        <div className="card-header">
          <a className="card-title" href="/learn/zh/docs/getting-started/capabilities">
            文档
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden m-0" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block m-0" noZoom />
          </a>
          <p className="card-description">
            美观的交互式文档网站。
          </p>
        </div>
        
        {/* Rive Animation */}
        <a href="/learn/zh/docs/getting-started/capabilities" style="cursor: pointer; text-decoration: none;">
          <div class="docs-rive rive-container fade-bottom">
            <canvas id="docs-rive-canvas"></canvas>
          </div>
        </a>

        <div className="action-buttons-vertical">
          <a className="fern-button filled normal primary gap-1 w-fit a-btn" href="/learn/zh/docs/getting-started/capabilities">
            介绍
            <img src="./images/arrow-right-white.svg" alt="Arrow right" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-black.svg" alt="Arrow right" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal w-fit gap-1 a-btn" href="/learn/zh/docs/getting-started/quickstart">
            快速开始
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal w-fit gap-1 a-btn" href="/learn/zh/docs/ai-features/overview">
            AI 功能
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal w-fit gap-1 a-btn" href="/learn/zh/docs/writing-content/fern-editor">
            Fern 编辑器
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal w-fit gap-1 a-btn" href="/learn/zh/docs/api-references/generate-api-ref">
            使用您的 API 规范
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal w-fit gap-1 a-btn" href="/learn/zh/docs/authentication/features/rbac">
            控制基于角色的访问
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal w-fit gap-1 a-btn" href="/learn/zh/docs/self-hosted/overview"> 
            自托管您的文档
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal w-fit gap-1 a-btn" href="https://buildwithfern.com/showcase#docs-customers.alldocs-features">
            客户案例
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
        </div>
      </div>

      {/* AI Search Card */}
      <div className="panel-card ai-search-card">
        <div className="card-header">
          <a className="card-title" href="/learn/zh/docs/ai-features/ask-fern/overview">
            Ask Fern
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden m-0" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block m-0" noZoom />
          </a>
          <p className="card-description">
            AI 搜索，即时在文档中找到答案。
          </p>
        </div>

        {/* Rive Animation */}
        <a href="/learn/zh/docs/ai-features/ask-fern/overview" style="cursor: pointer; text-decoration: none;">
          <div class="ask-fern-rive rive-container fade-bottom">
            <canvas id="ai-rive-canvas"></canvas>
          </div>
        </a>

        <div className="action-buttons">
          <a className="fern-button filled normal primary gap-1 a-btn" href="/learn/zh/docs/ai-features/ask-fern/overview">
            介绍
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal gap-1 a-btn" href="/learn/zh/docs/ai-features/ask-fern/overview">
            配置
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
          <a className="fern-button minimal normal gap-1 a-btn" href="https://buildwithfern.com/showcase#ask-fern-customers">
            客户案例
            <img src="./images/arrow-right-black.svg" alt="Arrow right light" className="dark:hidden" noZoom />
            <img src="./images/arrow-right-white.svg" alt="Arrow right light" className="hidden dark:block" noZoom />
          </a>
        </div>
      </div>
    </div>


    {/* Supported Specs Section */}
    <div className="community-section specs">
      <div className="community-title-container specs">
        <p className="card-title">支持的规范</p>
        <p className="card-description">
          选择一个或多个规范来生成 SDK 和文档。
        </p>
      </div>
      
      <div className="specs-grid">
        <a className="specs-card" href="/learn/zh/api-definitions/openapi/overview">
          <img className="m-0 dark:hidden" src="./images/openapi-light.svg" alt="Github Preview" noZoom/>
          <img className="m-0 hidden dark:block" src="./images/openapi-dark.svg" alt="Github Preview" noZoom />

          <p className="community-card-title">OpenAPI</p>
        </a>
        <a className="specs-card" href="/learn/zh/api-definitions/asyncapi/overview">
          <img className="m-0 dark:hidden" src="./images/asyncapi-light.svg" alt="Github Preview" noZoom/>
          <img className="m-0 hidden dark:block" src="./images/asyncapi-dark.svg" alt="Github Preview" noZoom />

          <h3 className="community-card-title">AsyncAPI</h3>
        </a>
        <a className="specs-card" href="/learn/zh/api-definitions/openrpc/overview">
          <img className="m-0 dark:hidden" src="./images/openrpc-light.svg" alt="Github Preview" noZoom/>
          <img className="m-0 hidden dark:block" src="./images/openrpc-dark.svg" alt="Github Preview" noZoom />

          <h3 className="community-card-title">OpenRPC</h3>
        </a>
        <a className="specs-card" href="/learn/zh/api-definitions/grpc/overview">
          <img className="m-0 dark:hidden" src="./images/grpc-light.svg" alt="Github Preview" noZoom/>
          <img className="m-0 hidden dark:block" src="./images/grpc-dark.svg" alt="Github Preview" noZoom />

          <h3 className="community-card-title">gRPC</h3>
        </a>
      </div>
    </div>
    {/* Community Section */}
    <div className="community-section">
      <div className="community-title-container">
        <p className="card-title">社区</p>
      </div>
      
      <div className="community-grid">
        <div className="community-card">
          <div className="community-card-header">
            <img className="m-0 dark:hidden" src="./images/changelog-light.svg" alt="Changelog Preview" noZoom />
            <img className="m-0 hidden dark:block" src="./images/changelog-dark.svg" alt="Changelog Preview" noZoom />

            <h3 className="community-card-title">变更日志</h3>
          </div>
          <p className="community-card-description">
            查看我们最新的产品更新。
          </p>
          <div className="flex flex-row flex-wrap items-center">
            <a className="language-icon" href="/learn/zh/docs/changelog">
              <img src="./images/fern-logo.svg" alt="Docs" className="language-icon-img" noZoom />
            </a>
            <a className="language-icon" href="/learn/zh/sdks/generators/typescript/changelog">
              <img src="./images/ts-logo.svg" alt="TypeScript" className="language-icon-img" noZoom />
            </a>
            {/* Python */}
            <a className="language-icon" href="/learn/zh/sdks/generators/python/changelog">
              <img src="./images/python-logo.svg" alt="Python" className="language-icon-img" noZoom />
            </a> 
            {/* Go */}
            <a className="language-icon" href="/learn/zh/sdks/generators/go/changelog">
              <img src="./images/go-logo.svg" alt="Go" className="language-icon-img" noZoom />
            </a>
            {/* Java */}
            <a className="language-icon" href="/learn/zh/sdks/generators/java/changelog">
              <img src="./images/java-logo.svg" alt="Java" className="language-icon-img" noZoom />
            </a>
            {/* C# */}
            <a className="language-icon" href="/learn/zh/sdks/generators/csharp/changelog">
              <img src="./images/csharp-logo.svg" alt="C#" className="language-icon-img" noZoom />
            </a>
            {/* PHP */}
            <a className="language-icon" href="/learn/zh/sdks/generators/php/changelog">
              <img src="./images/php-logo.svg" alt="PHP" className="language-icon-img" noZoom />
            </a>
            {/* Ruby */}
            <a className="language-icon" href="/learn/zh/sdks/generators/ruby/changelog">
              <img src="./images/ruby-logo.svg" alt="Ruby" className="language-icon-img" noZoom />
            </a>
            {/* Swift */}
            <a className="language-icon" href="/learn/zh/sdks/generators/swift/changelog">
              <img src="./images/swift-logo.svg" alt="Swift" className="language-icon-img" noZoom />
            </a>
            {/* Rust */}
            <a className="language-icon" href="/learn/zh/sdks/generators/rust/changelog">
              <img src="./images/rust-light-mode.svg" alt="Rust" className="language-icon-img dark:hidden" noZoom />
              <img src="./images/rust-dark-mode.svg" alt="Rust" className="language-icon-img hidden dark:block" noZoom />
            </a>
          </div>
        </div>

        <div className="community-card">
          <div className="community-card-header">
            <img className="m-0 dark:hidden" src="./images/github-light.svg" alt="Github Preview" noZoom/>
            <img className="m-0 hidden dark:block" src="./images/github-dark.svg" alt="Github Preview" noZoom />

            <h3 className="community-card-title">Github</h3>
          </div>
          <p className="community-card-description">
            跟踪进展并为代码库做贡献。
          </p>
          <a className="fern-button outlined normal gap-1 a-btn" href="https://github.com/fern-api/fern">
            查看
          </a>
        </div>

        <div className="community-card">
          <div className="community-card-header">
            <img className="m-0 dark:hidden" src="./images/x-light.svg" alt="Twitter Preview" noZoom />
            <img className="m-0 hidden dark:block" src="./images/x-dark.svg" alt="Twitter Preview" noZoom />

            <h3 className="community-card-title">
              <span className="twitter-strikethrough">Twitter</span> X
            </h3>
          </div>
          <p className="community-card-description">
            获取 Fern 平台的更新。
          </p>
          <a className="fern-button outlined normal gap-1 a-btn" href="https://x.com/buildwithfern">
            查看
          </a>
        </div>
      </div>
    </div>

    {/* Help Section */}
    <div id="get-support" className="help-section">
      <div className="card-header">
        <p className="card-title">帮助</p>
        <p className="card-description">
          我们提供闪电般快速的支持！如果您是客户，请通过您的专用 Slack 频道联系我们。
        </p>
      </div>
      
      <div className="help-buttons">
        <a className="fern-button outlined normal gap-2 a-btn" href="https://github.com/fern-api/fern/issues">
          <img src="./images/github-light.svg" alt="Github" className="h-4 w-4 dark:hidden" noZoom />
          <img src="./images/github-dark.svg" alt="Github" className="h-4 w-4 hidden dark:block" noZoom />

          提交 Github issue
        </a>

        <a className="fern-button outlined normal gap-2 a-btn" href="mailto:support@buildwithfern.com">
          <img src="./images/email-light.svg" alt="Email" className="h-4 w-4 dark:hidden" noZoom />
          <img src="./images/email-dark.svg" alt="Email" className="h-4 w-4 hidden dark:block" noZoom />

          发邮件给我们
        </a>
        <a className="fern-button outlined normal gap-2 a-btn" href="https://buildwithfern.com/book-demo">
          <img src="./images/email-light.svg" alt="Email" className="h-4 w-4 dark:hidden" noZoom />
          <img src="./images/email-dark.svg" alt="Email" className="h-4 w-4 hidden dark:block" noZoom />

          预约演示
        </a>
      </div>
    </div>

  </div>
</div>
</llms-ignore>

<llms-only>
## SDKs

Generate client libraries in multiple languages.

Get started with: [TypeScript](/learn/zh/sdks/generators/typescript/quickstart), [Python](/learn/zh/sdks/generators/python/quickstart), [Go](/learn/zh/sdks/generators/go/quickstart), [Java](/learn/zh/sdks/generators/java/quickstart), [C#](/learn/zh/sdks/generators/csharp/quickstart), [PHP](/learn/zh/sdks/generators/php/quickstart), [Ruby](/learn/zh/sdks/generators/ruby/quickstart), [Swift](/learn/zh/sdks/generators/swift/quickstart), [Rust](/learn/zh/sdks/generators/rust/quickstart)

- [Introduction](/learn/zh/sdks/overview/introduction)
- [Quickstart](/learn/zh/sdks/overview/quickstart)
- [Customers](https://buildwithfern.com/showcase)

## Docs

A beautiful, interactive documentation website.

- [Introduction](/learn/zh/docs/getting-started/capabilities)
- [Quickstart](/learn/zh/docs/getting-started/quickstart)
- [AI features](/learn/zh/docs/ai-features/overview)
- [Fern Editor](/learn/zh/docs/writing-content/fern-editor)
- [Bring your own API spec](/learn/zh/docs/api-references/generate-api-ref)
- [Control role-based access](/learn/zh/docs/authentication/features/rbac)
- [Self-host your docs](/learn/zh/docs/self-hosted/overview)
- [Customers](https://buildwithfern.com/showcase#docs-customers.alldocs-features)

## Ask Fern

AI search to find answers in your documentation instantly.

- [Introduction](/learn/zh/docs/ai-features/ask-fern/overview)
- [Configure](/learn/zh/docs/ai-features/ask-fern/overview)
- [Customers](https://buildwithfern.com/showcase#ask-fern-customers)

## Supported specs

Select one or more specs to generate SDKs and Docs.

- [OpenAPI](/learn/zh/api-definitions/openapi/overview)
- [AsyncAPI](/learn/zh/api-definitions/asyncapi/overview)
- [OpenRPC](/learn/zh/api-definitions/openrpc/overview)
- [gRPC](/learn/zh/api-definitions/grpc/overview)

## Community

- [Changelog](/learn/zh/docs/changelog): See the most recent product updates.
- [GitHub](https://github.com/fern-api/fern): Follow progress and contribute to the codebase.
- [X](https://x.com/buildwithfern): Get updates on the Fern platform.

## Help

Fern offers lightning-fast support. If you're a customer, reach out via your dedicated Slack channel.

- [File a GitHub issue](https://github.com/fern-api/fern/issues)
- [Email us](mailto:support@buildwithfern.com)
- [Book a demo](https://buildwithfern.com/book-demo)
</llms-only>
