...
`<llms-only>` 内容仅出现在外部代理（如 Claude、ChatGPT 或 Cursor）直接获取的 LLM 服务端点上：`/llms.txt` 以及每个页面的 [`.md`/`.mdx` 源文件](/learn/zh/docs/ai-features/markdown)。它在所有面向人类的界面中都是隐藏的，包括渲染页面、[复制页面](/learn/zh/docs/configuration/site-level-settings#page-actions-configuration)、搜索小部件和 [Ask Fern](/learn/zh/docs/ai-features/ask-fern/overview)。
...
`<llms-ignore>` 内容出现在所有面向人类的界面上，包括渲染页面、[复制页面](/learn/zh/docs/configuration/site-level-settings#page-actions-configuration)、搜索小部件和 [Ask Fern](/learn/zh/docs/ai-features/ask-fern/overview)（它会索引并可以像任何其他页面内容一样引用它）。它会从 LLM 服务端点（`/llms.txt` 和每个页面的 `.md`/`.mdx` 源文件）中被去除。
...
要预览和调试 AI 为任何页面看到的内容，您可以在页面 URL 后附加 `.md` 或 `.mdx` 来[查看其 Markdown 源文件](/learn/zh/docs/ai-features/markdown)。
...
您可以通过[在页面的 frontmatter 中添加 `noindex: true`](/learn/zh/docs/seo/setting-seo-metadata#noindex) 来从 LLM 端点（`llms.txt`）排除整个页面。标记为 `noindex` 的页面不会被搜索引擎索引，但在您的侧边栏导航中仍然可见，并且仍可以通过 URL 直接访问。要同时从导航中隐藏页面，请参阅[隐藏内容](/learn/zh/docs/customization/hiding-content)。
...
要提供您自己的 `llms.txt` 而不是自动生成的版本，请在 [`docs.yml` 中的 `agents` 键下](/learn/zh/docs/configuration/site-level-settings#agents-configuration)指向您的文件：
...
要控制*哪些*爬虫访问您的站点而不是*它们接收什么*，请改为提供[自定义 `robots.txt`](/learn/zh/docs/seo/robots-txt)。
