...
要完全禁用服务器，在 [`page-actions` 配置](/learn/docs/configuration/site-level-settings#page-actionsoptions.mcp)中将 `mcp` 设置为 `false`：
...
这对于[需要身份验证的站点](/learn/docs/authentication/overview)尤其有用，因为未经身份验证的 AI 代理可以发现 MCP 端点，但无法读取其内容。禁用服务器后，端点会返回 404，连接按钮会被隐藏，代理也无法再通过 `llms.txt` 或代理提示发现它。
