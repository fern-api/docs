...
要完全禁用 MCP 服务器 — 使端点返回 404、AI 代理无法通过 `llms.txt` 或代理提示发现它、连接按钮也会隐藏 — 在 [`page-actions` 配置](/learn/docs/configuration/site-level-settings#page-actionsoptions.mcp)中将 `mcp` 设置为 `false`：
...
这对于未经身份验证的 AI 代理会发现端点但无法访问的[需要身份验证的站点](/learn/docs/authentication/overview)很有用。
