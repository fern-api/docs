...
| 组件 | 描述 |
| --- | --- |
| `<Fern.Logo />` | 您在 `docs.yml` 中配置的网站[标志](/learn/docs/configuration/site-level-settings#logo-configuration)。链接到主页。您可以使用 `document.querySelector('#fern-header [data-fern-logo]')` 来定位此组件。 |
| `<Fern.Search />` | [搜索](/learn/docs/customization/search)栏，如果启用的话包括 AI 搜索触发器。 |
| `<Fern.ProductSwitcher />` | 用于在[产品](/learn/docs/configuration/products)之间切换的下拉菜单。 |
| `<Fern.VersionSwitcher />` | 用于在[版本](/learn/docs/configuration/versions)之间切换的下拉菜单。 |
| `<Fern.LanguageSwitcher />` | 用于切换活动 [SDK 语言](/learn/docs/configuration/site-level-settings#default-language)的下拉菜单。 |
| `<Fern.NavbarLinks />` | 在 `docs.yml` 的 `navbar-links` 下配置的[导航链接](/learn/docs/configuration/site-level-settings#navbar-links-configuration)。 |
| `<Fern.LoginButton />` | [身份验证文档](/learn/docs/authentication/overview)的登录/注册按钮。 |
| `<Fern.ThemeSwitch />` | 在[明暗模式](/learn/docs/configuration/site-level-settings#theme-configuration)之间切换。 |
