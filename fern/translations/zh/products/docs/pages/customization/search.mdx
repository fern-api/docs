...
如果您正在使用 [Ask AI](/learn/docs/ai-features/ask-ai/overview)（AI 搜索），搜索框同时也充当您站点的聊天窗口。

<Note>
  带有 `nofollow` 或 `noindex`
  [frontmatter](/learn/docs/configuration/page-level-settings#indexing-properties)
  的页面将从 Algolia DocSearch 索引中排除，不会出现在搜索结果中。
</Note>
...
如果您需要将 Fern 的文档搜索集成到自己的应用程序或仪表板中，您可以使用[独立搜索小部件](/learn/docs/ai-features/ask-ai/search-widget)来嵌入现成的 React 组件，或直接从 Fern 团队请求 Algolia 凭据以构建自定义集成。
...
<Note>
  **注意：** 请保护您的 Algolia
  凭据安全，避免在客户端代码中暴露它们。考虑实现后端代理来发出 Algolia 请求。
</Note>
