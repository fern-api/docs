...
通过编写 Markdown 文件创建新的变更日志条目。您可以使用 `.md` 或 `.mdx` 文件。使用 `.mdx` 的好处是您可以在条目中利用 Fern 内置的[组件库](/learn/zh/docs/writing-content/components/overview)。
...
<Note>
使用[变更日志过滤器 CSS 选择器](/learn/zh/docs/customization/css-selectors-reference#changelog-filter-components)自定义过滤器 UI。这些选择器仅在配置了标签时适用。
</Note>
