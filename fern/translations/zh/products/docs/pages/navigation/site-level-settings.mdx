...
`docs.yml` 文件是您在文档网站中自定义颜色、字体、布局、分析等的主要工具。对于大多数自定义需求，请从这里开始，然后再考虑[自定义 CSS 和 JavaScript](/learn/docs/customization/custom-css-js) 来满足高级使用场景。
...
<ParamField path="instances.audiences" type="list of strings" required={false} toc={true}>
  ...
  您可以使用受众来控制每个文档实例中显示哪些版本和产品，从而为不同用户群体创建单独的网站。当内容的受众标签与实例受众匹配时，内容会被包含。没有受众标签的内容默认包含。了解更多关于为[产品和/或版本](/learn/docs/configuration/products#add-instance-audiences)配置实例受众的信息。
</ParamField>
...
<ParamField path="settings.http-snippets" type="boolean | list of objects" required={false} default="true" toc={true}>
  控制 [API 参考中 HTTP 片段](/learn/docs/api-references/http-snippets)的显示。默认情况下为所有语言启用 HTTP 片段。
...
<ParamField path="page-actions.options.copy-page" type="boolean" required={false} default="true" toc={true}>
  启用时，显示允许用户将整个页面内容复制到剪贴板的按钮，便于共享或参考。要控制复制输出中出现的内容，请使用 [`<llms-ignore>`](/learn/docs/ai-features/customize-llm-output#filter-within-a-page) 标签。
</ParamField>

<ParamField path="page-actions.options.view-as-markdown" type="boolean" required={false} default="true" toc={true}>
  启用时，显示允许用户查看当前页面原始 Markdown 源代码的按钮。用户也可以[通过在页面 URL 后附加 `.md` 查看 Markdown](/learn/docs/ai-features/markdown)。要控制 Markdown 输出中出现的内容，请使用 [`<llms-ignore>`](/learn/docs/ai-features/customize-llm-output#filter-within-a-page) 标签。
</ParamField>
...
<ParamField path="page-actions.options.mcp" type="boolean" required={false} default="true" toc={true}>
  控制站点的 [MCP 服务器](/learn/docs/ai-features/mcp-server)端点。设置为 `false` 时，`/_mcp/server` 端点返回 404，其 URL 从 [`llms.txt`](/learn/docs/ai-features/llms-txt) 和代理提示中删除，**连接到 Claude Code** / **连接到 Cursor** 按钮也会隐藏。适用于未经身份验证的 AI 代理会发现端点但无法访问的[需要身份验证的站点](/learn/docs/authentication/overview)。
</ParamField>
...
<Note title="想要为单个页面设置元数据？">
 [在页面的 frontmatter 中使用 `keywords` 属性](/learn/docs/configuration/page-level-settings#seo-metadata)。
</Note>
