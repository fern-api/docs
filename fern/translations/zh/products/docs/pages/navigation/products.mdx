...
<Note>关于不使用产品的独立版本控制，请参阅我们的[版本控制指南](/learn/docs/configuration/versions)。</Note>
...
通过为版本和/或产品标记受众，控制哪些版本和/或产品出现在每个[文档实例](/learn/docs/configuration/site-level-settings#instances-configuration)中。这使得可以为不同用户群体（例如，内部开发者、beta 测试者、公共客户）提供单独的站点。
...
实例受众与 [API 参考受众](/learn/docs/api-references/audiences)一起工作，后者过滤 API 文档中的端点和模式。您可以同时使用这两个功能：
