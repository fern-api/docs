...
| 模式 | 捕获内容 | 示例 `source` | 匹配 |
|------|----------|---------------|------|
| `:slug` | 单个路径段。必须紧跟在 `/` 之后。 | `/plants/:slug` | 匹配 `/plants/monstera`，不匹配 `/plants/indoor/monstera` |
| `:slug*` | 零个或多个路径段，包括分隔用的斜杠。必须紧跟在 `/` 之后。 | `/plants/:slug*` | 匹配 `/plants` 和 `/plants/indoor/monstera` |
| `:slug+` | 一个或多个路径段。必须紧跟在 `/` 之后。 | `/plants/:slug+` | 匹配 `/plants/indoor/monstera`，不匹配 `/plants` |
| `:slug(regex)` | 正则表达式匹配到的任何内容。可以从段中间开始，也可以跨越 `/`。 | `/plants-:slug(.*)` | 匹配 `/plants-indoor/monstera` |
| `(regex)` | 未命名的分组，在 `destination` 中以 `:0` 回填。 | `/plants/(\d+)` | 匹配 `/plants/42` |

有三种行为无法从语法本身看出：

- **正则表达式是从段中间开始匹配的唯一方式。** `:slug*` 和 `:slug+` 必须紧跟在 `/` 之后，普通的 `:slug` 也不能跨越 `/`。`/plants-:slug` 匹配 `/plants-monstera`，但不匹配 `/plants-monstera/care`；`/plants-:slug(.*)` 两者都匹配。
...
正则表达式既能放宽参数，也能收窄参数：`/v:version(\d+)/plants/:slug*` 匹配 `/v2/plants/monstera/care`，并将 `version` 捕获为 `2`。
