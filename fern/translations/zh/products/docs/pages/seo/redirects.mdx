...
|------|----------|
| `:slug` | 单个路径段，紧跟在 `/` 之后开始。 |
| `:slug*` | 零个或多个路径段，包括分隔用的斜杠。必须紧跟在 `/` 之后。 |
| `:slug+` | 一个或多个路径段。必须紧跟在 `/` 之后。 |
| `:slug(regex)` | 正则表达式匹配到的任何内容。与上述形式不同，它可以从段中间开始，也可以跨越 `/`。 |
| `(regex)` | 未命名的分组，在 `destination` 中以 `:0` 回填。 |

普通的 `:slug` 只能在 `/` 之后开始，且不能跨越 `/`，而 `:slug*` 和 `:slug+` 根本无法从段中间开始，因此当 URL 在段中间发生变化时，请使用正则表达式。`/plants-:slug` 匹配 `/plants-monstera`，但不匹配 `/plants-monstera/care`，而 `/plants-:slug(.*)` 两者都匹配。同样的语法也可以收窄参数而不是放宽它：`/v:version(\d+)/plants/:slug*` 匹配 `/v2/plants/monstera/care`，并将 `version` 捕获为 `2`。
