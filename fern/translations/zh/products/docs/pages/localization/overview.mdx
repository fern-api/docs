...
<Note>
  本地化功能正在积极开发中。自动翻译、Ask AI、`fern check` 错误和 API 参考页面仍在开发中。
...
<Warning>
  字段级回退仅适用于条目内。如果某个页面在基础导航中但在本地化 YAML
  中缺失，本地化 URL 将提供默认语言页面——即使存在翻译的 `.mdx`
  文件。在这里也要注册每个你[翻译](#translate-page-content)的页面。
</Warning>
