...
[`@fern-api/search-widget`](https://www.npmjs.com/package/@fern-api/search-widget) 包提供了一个独立的 React 组件，可以将 Ask AI 的 AI 驱动搜索功能带到您的 Fern Docs 网站之外的任何 React 应用程序中。在您的仪表板、营销网站或内部工具中嵌入搜索模态框，让用户无需离开其工作流程即可找到相关文档。
...
- **启用了 [Ask AI](/learn/docs/ai-features/ask-fern/overview) 的实时云托管 Fern Docs 站点** — 该组件在运行时连接到您已发布的文档站点。不支持自托管和本地预览环境。
...
<ParamField path="domain" type="string" required={true}>
  您已发布的 Fern Docs 站点的
  URL（例如，`https://docs.example.com`）。如果您的文档不在根目录，请包含完整路径（例如，`https://buildwithfern.com/learn`）。
</ParamField>
