...
Fern 从您的 [AsyncAPI 通道定义](/learn/zh/api-definitions/asyncapi/channels/pubsub)生成类型化的 WebSocket 客户端。生成的客户端处理连接管理、类型化的发送/接收消息和自动重连。
...
WebSocket 客户端从 [AsyncAPI 规范](/learn/zh/api-definitions/asyncapi/overview)中的通道定义生成。有关定义通道、消息和操作的信息，请参阅[发布/订阅操作](/learn/zh/api-definitions/asyncapi/channels/pubsub)。
