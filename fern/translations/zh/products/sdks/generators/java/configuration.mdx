...
启用时，生成支持通过继承进行自定义的可扩展构建器类。这允许您[添加自定义代码](/learn/zh/sdks/generators/java/custom-code#adding-custom-client-configuration)，通过额外功能扩展生成的构建器。
</ParamField>
...
启用时，生成[模拟服务器（wire）测试](/learn/zh/sdks/deep-dives/testing#mock-server-tests)以验证 SDK 是否按预期发送和接收 HTTP 请求。
</ParamField>
