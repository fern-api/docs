...
默认情况下，TypeScript 生成器只为您的 API 定义在 `package.json` 中自动生成 `exports` 条目。如果您希望消费者通过专用子路径导入您的自定义文件（例如 `import { myHelper } from "@acme/sdk/helper"`），您必须在 `generators.yml` 中使用 [`packageJson.exports`](/learn/zh/sdks/generators/typescript/configuration#packagejson) 手动注册：
...
要添加您的自定义代码所需的包，请在您的 `generators.yml` 中使用 [`extraDependencies`](/learn/zh/sdks/generators/typescript/configuration#extraDependencies) 和 [`extraDevDependencies`](/learn/zh/sdks/generators/typescript/configuration#extraDevDependencies) 选项。
