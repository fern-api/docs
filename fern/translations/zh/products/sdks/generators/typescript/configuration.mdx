...
查看 [TypeScript 序列化层](/learn/zh/sdks/generators/typescript/serde-layer) 了解更多信息。
</ParamField>
...
查看 [TypeScript 序列化层](/learn/zh/sdks/generators/typescript/serde-layer) 了解何时启用此选项的详细指导。
</ParamField>

<ParamField path="offsetSemantics" type="'item-index' | 'page-index'" default="item-index" toc={true}>
控制如何解释[自动分页](/learn/zh/sdks/deep-dives/auto-pagination)端点的偏移参数。
...
您也可以使用 `packageJson.exports` 注册自定义子路径导出（例如 `import { myHelper } from "@acme/sdk/helper"`）。生成器只会为您的 API 定义自动生成导出条目，因此需要手动添加自定义文件—否则 Node.js 不会解析它们的子路径导入。查看[添加自定义代码](/learn/zh/sdks/generators/typescript/custom-code)了解详情。
</ParamField>
...
但是，当禁用序列化层时（`noSerdeLayer: true`），它们将被类型化为 `number | bigint`。查看 [TypeScript 序列化层](/learn/zh/sdks/generators/typescript/serde-layer) 了解更多信息。
