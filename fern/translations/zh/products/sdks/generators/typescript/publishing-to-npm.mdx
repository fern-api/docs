...
你需要更新 `generators.yml` 文件来配置包名、输出位置和客户端命名，以便进行 npm 发布。你的 `generators.yml` [应该位于你的源仓库中](/learn/sdks/overview/project-structure)（或在你的本地机器上），而不是包含你的 TypeScript SDK 代码的仓库中。
