...
对于标注了 [`x-fern-pagination`](/learn/api-definitions/openapi/extensions/pagination) 的端点，CLI 在设置 `--page-all` 标志时自动分页。
...
对于具有[模板化服务器 URL](/learn/api-definitions/openapi/extensions/server-names-and-url-templating)（如 `https://api.example.com/stores/{store_hash}/v3`）的 API，CLI 自动将每个模板变量暴露为 CLI 标志和环境变量。
