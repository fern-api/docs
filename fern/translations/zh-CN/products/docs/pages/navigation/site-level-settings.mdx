---
title: 站点级设置
subtitle: 在 `docs.yml` 中自定义颜色、字体、logo、布局、导航栏链接和分析
description: "了解如何使用 docs.yml 文件配置您的 Fern 文档站点。自定义颜色、字体、布局、分析等内容。"
sidebar-title: 站点级设置
max-toc-depth: 2
---

`docs.yml` 文件是您自定义文档站点颜色、字体、布局、分析等内容的主要工具。在考虑使用[自定义 CSS 和 JavaScript](/docs/customization/custom-css-js)进行高级定制之前，大多数自定义需求都可以从这里开始。

### YAML 模式验证

要在编辑器中启用智能 YAML 验证和自动补全，请在 `docs.yml` 文件顶部添加以下行：

```yaml docs.yml
# yaml-language-server: $schema=https://schema.buildwithfern.dev/docs-yml.json
```

这将基于 Fern 的[完整模式](https://github.com/fern-api/fern/blob/09555d587294fd3dc77ceb35f21e8976a5a2b7a2/fern/apis/docs-yml/definition/docs.yml#L110)启用实时模式验证和自动补全。

## 核心配置

每个 Fern 文档网站都需要一个包含核心配置设置的 `docs.yml` 文件。以下是您可以配置的基本顶级属性：
...
colors:                              
  accent-primary:
    light: "#635BFF"                 # Stripe 的主要紫色
    dark: "#9B90FF"                  # 深色模式下的浅紫色
  background:
    light: "#FFFFFF" 
    dark: "#0A2540"                  
...
<ParamField path="logo" type="object" required={false} toc={true}>
  为您的站点设置自定义 logo。了解更多关于 [`logo` 配置](/learn/docs/getting-started/global-configuration#logo-configuration)的信息。
</ParamField>

<ParamField path="favicon" type="string" required={false} toc={true}>
  favicon 的相对文件路径。路径相对于设置该属性的 YAML 文件（例如 `docs.yml`）。
</ParamField>
...
<ParamField path="redirects" type="list of objects" required={false} toc={true}>
  您希望配置为永久重定向到另一个路径的路径数组。了解更多关于
  [`redirects` 配置](/learn/docs/getting-started/global-configuration#redirects-configuration)的信息。
</ParamField>

<ParamField path="navbar-links" type="list of objects" required={false} toc={true}>
  您希望作为行动号召包含的名称和链接的数组。了解更多关于
  [`navbar-links` 配置](/learn/docs/getting-started/global-configuration#navbar-links-configuration)的信息。
</ParamField>

<ParamField path="background-image" type="object" required={false} toc={true}>
  设置在每个页面后面显示的自定义背景图像。了解更多关于
  [`background-image` 配置](/learn/docs/getting-started/global-configuration#background-image-configuration)的信息。
</ParamField>
...
<ParamField path="layout" type="object" required={false} toc={true}>
  自定义文档网站的布局。了解更多关于
  [`layout` 配置](/learn/docs/getting-started/global-configuration#layout-configuration)的信息。
</ParamField>

<ParamField path="settings" type="object" required={false} toc={true}>
  自定义文档网站的设置。了解更多关于
  [`settings` 配置](/learn/docs/getting-started/global-configuration#settings-configuration)的信息。
</ParamField>

<ParamField path="landing-page" type="object" required={false} toc={true}>
  为您的文档网站创建着陆页。了解更多关于
  [`landing-page` 配置](/learn/docs/getting-started/global-configuration#landing-page-configuration)的信息。
</ParamField>

<ParamField path="default-language" type="string" required={false} toc={true}>
  设置 API Reference 中代码片段显示的默认语言。

  选项包括：`typescript`、`python`、`java`、`go`、`ruby`、`csharp`、`php`、`swift`、`curl`
</ParamField>

<ParamField path="metadata" type="object" required={false} toc={true}>
  为您的文档网站配置 SEO 元数据。了解更多关于
  [`metadata` 配置](/learn/docs/getting-started/global-configuration#seo-metadata-configuration)的信息。
</ParamField>

<ParamField path="global-theme" type="string" required={false} toc={true}>
  应用到此站点的[全局主题](/learn/docs/customization/global-themes)名称。CLI 在发布时从 Fern 注册表中获取命名主题，并将其品牌字段（logo、颜色、字体、布局、CSS、JS 等）合并到本地 `docs.yml` 中。使用此功能可在多个文档站点间共享一致的视觉标识。
</ParamField>

<ParamField path="header" type="string" required={false} toc={true}>
  用于替换 Fern 默认头部的自定义 React 组件文件（TSX 或 JSX）的路径。该组件必须有默认导出。了解更多关于[自定义头部和底部组件](/learn/docs/customization/header-and-footer)的信息。
</ParamField>

<ParamField path="footer" type="string" required={false} toc={true}>
  用于替换 Fern 默认底部的自定义 React 组件文件（TSX 或 JSX）的路径。该组件必须有默认导出。了解更多关于[自定义头部和底部组件](/learn/docs/customization/header-and-footer)的信息。
</ParamField>

## 实例配置

`instance` 是独特文档网站的后端。每个实例使用 `--instance` 标志发布到唯一域名。最常见的做法是使用实例来配置发布到不同 URL 的预发布和生产文档。
...
<ParamField path="instances.edit-this-page" type="object" required={false} toc={true}>
  如果指定，将在每个页面底部添加"编辑此页面"链接，链接到指定的公共 GitHub 仓库。您可以选择设置 `launch` 目标来控制链接将用户引导到何处。了解更多关于 [`edit-this-page` 配置](#github-configuration)的信息。
</ParamField>

<ParamField path="instances.audiences" type="list of strings" required={false} toc={true}>
  指定此实例服务的受众（例如，内部开发者、测试版测试者、公共客户）。
  
  您可以使用受众来控制哪些版本和产品出现在每个文档实例中，从而为不同用户群体创建单独的站点。当内容的受众标签与实例受众匹配时，内容会被包含在内。没有受众标签的内容默认被包含。了解更多关于为[产品和/或版本](/docs/configuration/products#add-instance-audiences)配置实例受众的信息。
</ParamField>

## 颜色配置

为您的文档配置明暗模式的颜色调色板。只有 `accent-primary` 是必需的——所有其他颜色都有合理的默认值。这些颜色也会自动作为[CSS 自定义属性](/learn/docs/customization/custom-css-js#built-in-css-color-variables)在您的自定义样式表中可用。

```yaml docs.yml
colors:
  accent-primary:
    light: "#418326"  # 浅色模式的主要品牌颜色
    dark: "#ADFF8C"   # 深色模式的主要品牌颜色
...
<ParamField path="logo.href" type="string" required={false} toc={true}>
  用户点击 logo 时将跳转到的 URL。通常是您公司的主页或应用。
</ParamField>

<ParamField path="logo.dark" type="string" required={false} toc={true}>
  深色模式 logo 文件的路径，相对于设置它的 YAML 文件（例如 `docs.yml`）。推荐使用 SVG 格式以获得最佳质量。示例：`assets/images/logo-dark.svg`
</ParamField>

<ParamField path="logo.light" type="string" required={false} toc={true}>
  浅色模式 logo 文件的路径，相对于设置它的 YAML 文件（例如 `docs.yml`）。推荐使用 SVG 格式以获得最佳质量。示例：`assets/images/logo-light.svg`
</ParamField>

<ParamField path="logo.right-text" type="string" required={false} toc={true}>
  在 logo 图片右侧显示的文本。这对于在 logo 旁边添加"Docs"或"API"等标签很有用。
</ParamField>

<ParamField path="logo.height" type="number" required={false} toc={true}>
  logo 的自定义高度（像素）。如果默认高度不符合您的设计，请使用此选项调整 logo 大小。
</ParamField>
...
<ParamField path="type" type="enum" required={false} toc={true}>
  `outlined`、`minimal`、`filled`、`github` 或 `dropdown` 之一。此值控制按钮的样式。
</ParamField>
...
<ParamField path="value" type="string" required={false} toc={true}>
  GitHub 仓库的 URL。与 `href` 类似，但专门用于 GitHub 仓库链接。当 `type` 设置为 `github` 时使用此字段。示例：https://github.com/example-company/fern
</ParamField>
...
<ParamField path="target" type="string" required={false} toc={true} default="_self">
  指定在何处打开链接的 URL。对于典型的文档站点，链接可以在同一标签页（`_self`）或新标签页（`_blank`）中打开。对于嵌入在仪表板或 iframe 中的文档，链接可以在父框架（`_parent`）或最顶层框架（`_top`）中打开。
</ParamField>
...
向您的文档站点底部添加可点击的社交媒体和社区链接，以提高可发现性和参与度。

<Note>
  底部链接为您的社交渠道提供视觉导航。要在页面或站点级别配置 SEO 元数据和社交媒体标签，请参阅[元数据配置](/learn/docs/seo/setting-seo-metadata)。
</Note>
...
<ParamField path="typography.bodyFont" type="object" required={false} toc={true}>
  用于所有正文文本的字体，包括段落、列表和一般内容。
  为获得最佳性能，请使用 WOFF2 格式。
</ParamField>

<ParamField path="typography.headingsFont" type="object" required={false} toc={true}>
  用于标题、标题和其他突出文本元素的字体。如果您喜欢统一的外观，可以与正文字体相同。
  支持不同标题级别的多种字重。
</ParamField>

<ParamField path="typography.codeFont" type="object" required={false} toc={true}>
  用于代码块和内联代码的字体。建议使用等宽字体以提高代码可读性。
  受欢迎的选择包括 JetBrains Mono、Fira Code 和 Source Code Pro。
</ParamField>
...
<ParamField path="layout.header-height" type="string" required={false} toc={true}>
  设置头部的高度。默认为 `4rem`（`64px`）。有效选项为 `{number}rem` 或 `{number}px`。
</ParamField>
...
<ParamField path="layout.searchbar-placement" type="string" required={false} default="sidebar" toc={true}>
  设置搜索栏的位置。可以是 `header`、`sidebar` 或 `header-tabs`（将搜索栏放在头部但在标签行）之一。

  <Note>当 `disable-header` 设置为 true 时，此设置被忽略。</Note>
</ParamField>

<ParamField path="layout.tabs-placement" type="string" required={false} default="sidebar" toc={true}>
  设置标签的位置。可以是 `header` 或 `sidebar` 之一。
...
<ParamField path="layout.disable-header" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，头部不会被渲染。相反，logo 将作为侧边栏的一部分渲染，
  1px 边框将分隔侧边栏和内容。
</ParamField>

<ParamField path="layout.hide-nav-links" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，页面底部的导航链接（上一页、下一页）不会被渲染。这可以通过使用[前言](/learn/docs/configuration/page-level-settings#navigation-links)在逐页基础上覆盖。
</ParamField>

<ParamField path="layout.hide-feedback" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，反馈表单不会被渲染。这可以通过使用[前言](/learn/docs/configuration/page-level-settings#on-page-feedback)在逐页基础上覆盖。
</ParamField>

<ParamField path="layout.mobile-toc" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，在移动和平板视口（在 `xl` 断点下）的头部下方显示粘性目录栏。该栏显示滚动进度指示器和当前标题，点击时展开到完整目录。

  仅适用于使用[`guide` 和 `overview` 布局](/learn/docs/configuration/page-level-settings#layout)的页面。当页面少于两个标题或在[页面前言](/learn/docs/configuration/page-level-settings#table-of-contents)中设置 `hide-toc: true` 时，该栏被隐藏。
</ParamField>
...
<ParamField path="theme.sidebar" type="string" required={false} default="default" toc={true}>
  设置导航侧边栏的视觉样式。`default` 显示带有节标题和图标的完整侧边栏；`minimal` 显示没有装饰元素的更简洁的简化侧边栏。
</ParamField>

<ParamField path="theme.body" type="string" required={false} default="default" toc={true}>
  设置主要内容区域的视觉样式。`default` 使用平面背景；`canvas` 在内容区域周围添加微妙的卡片式容器。
</ParamField>

<ParamField path="theme.tabs" type="string | object" required={false} default="default" toc={true}>
  配置导航标签。接受字符串（`default` 或 `bubble`）用于仅样式配置，或带有 `style`、`alignment` 和 `placement` 属性的对象用于完全控制。

  当设置为字符串时，`default` 使用下划线标签，`bubble` 显示带有圆形药丸背景的标签。
</ParamField>

<ParamField path="theme.tabs.style" type="string" required={false} default="default" toc={true}>
  设置导航标签的视觉样式。`default` 使用下划线标签；`bubble` 显示带有圆形药丸背景的标签。
</ParamField>

<ParamField path="theme.tabs.alignment" type="string" required={false} default="left" toc={true}>
  当 `placement` 为 `header` 时设置标签的水平对齐方式。`left` 将标签对齐到左侧；`center` 水平居中。当 `placement` 为 `sidebar` 时，对齐没有效果。
</ParamField>

<ParamField path="theme.tabs.placement" type="string" required={false} default="sidebar" toc={true}>
  设置标签的位置。`header` 在顶部导航头部显示标签；`sidebar` 在左侧边栏显示标签。
  
  当同时设置时，优先于 `layout.tabs-placement`。当 `layout.disable-header` 设置为 true 时，此设置被忽略。
</ParamField>

<ParamField path="theme.page-actions" type="string" required={false} default="default" toc={true}>
  设置页面操作按钮的视觉样式。`default` 显示单独的图标按钮；`toolbar` 将操作分组到紧凑的水平工具栏中。
</ParamField>

<ParamField path="theme.footer-nav" type="string" required={false} default="default" toc={true}>
  设置底部导航链接的视觉样式。`default` 显示带有标题和描述的完整上一页/下一页卡片；`minimal` 显示简单的文本链接。
</ParamField>
...
配置站点范围的行为，如搜索、代码显示、404 处理和环境变量替换。
...
<ParamField path="settings.disable-explorer-proxy" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，API Explorer 将在直接向您的 API 发送请求时绕过代理。
  
  <Warning>
  启用此功能时，您的 API 必须启用跨源资源共享（CORS）以允许来自文档域的请求。
  </Warning>
</ParamField>

<ParamField path="settings.dark-mode-code" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，代码块将以深色模式显示，无论选择何种主题。
</ParamField>

<ParamField path="settings.default-search-filters" type="boolean" required={false} default="false" toc={true}>
  控制搜索结果默认是否限定在用户当前产品和版本的范围内。

  当设置为 `false`（默认值）时，搜索返回所有产品和版本的结果。当设置为 `true` 时，[搜索 UI](/learn/docs/customization/search) 自动过滤结果以匹配用户当前浏览的产品和版本。用户可以从搜索 UI 中删除这些过滤器以扩大搜索范围。
</ParamField>

<ParamField path="settings.http-snippets" type="boolean | list of objects" required={false} default="true" toc={true}>
  控制在 API Reference 中显示 [HTTP 片段](/docs/api-references/http-snippets)。默认为所有语言启用 HTTP 片段。
  
  - 设置为 `false` 完全禁用 HTTP 片段
  - 提供语言列表以仅为特定语言启用片段
...
<ParamField path="settings.use-javascript-as-typescript" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，TypeScript 片段将在 API Reference 中显示为 JavaScript 片段。
</ParamField>

<ParamField path="settings.disable-analytics" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，在[`analytics` 配置](#analytics-configuration)中配置的所有分析集成都将被禁用。这包括 Google Analytics 4、Google Tag Manager、PostHog 以及您配置的任何其他分析提供商。
</ParamField>

<ParamField path="settings.folder-title-source" type="'filename' | 'frontmatter'" required={false} default="filename" toc={true}>
  设置在所有[文件夹导航](/learn/docs/configuration/navigation#add-a-folder)中派生页面和节标题的默认方法。默认情况下（`filename`），标题从文件名派生。设置为 `frontmatter` 以使用每个文件前言中的 `title` 字段（如果未设置则回退到文件名）。单个文件夹可以使用每个文件夹的 [`title-source`](/learn/docs/configuration/navigation#title-source) 设置覆盖此全局默认值。
</ParamField>

<ParamField path="settings.substitute-env-vars" type="boolean" required={false} default="false" toc={true}>
  如果设置为 true，将在构建时用环境变量值替换 `${ENV_VAR}` 表达式。对于将 API 密钥、基础 URL 或版本号注入到您的文档中很有用。
  ...
  替换在 Markdown/MDX 文件和 API 规范（OpenAPI、Fern 定义）中有效。JavaScript 文件被排除以避免与模板字面量冲突。

  要输出字面量 `${VAR}`，请转义为 `\$\{VAR\}`。

  <Note>
  在本地预览（`fern docs dev`）期间，未定义的变量解析为空字符串。在发布期间，未定义的变量会导致构建失败。
  </Note>
</ParamField>

## 页面操作配置

配置在您的文档中出现的页面操作按钮。默认情况下，**复制页面**（`copy-page`）、**查看为 Markdown**（`view-as-markdown`）、**Ask AI**（`ask-ai`）、**ChatGPT**（`chatgpt`）、**Claude**（`claude`）、**Claude Code**（`claude-code`）和 **Cursor**（`cursor`）已启用。

要在单个页面上隐藏页面操作，请使用 [`hide-page-actions` 前言属性](/learn/docs/configuration/page-level-settings#page-actions)。
...
<ParamField path="page-actions.options.copy-page" type="boolean" required={false} default="true" toc={true}>
  启用时，显示一个按钮，允许用户将整个页面内容复制到剪贴板以便于共享或参考。要控制复制输出中出现的内容，请使用 [`<llms-ignore>`](/learn/docs/ai-features/customize-llm-output#content-for-humans-only) 标签。
</ParamField>

<ParamField path="page-actions.options.view-as-markdown" type="boolean" required={false} default="true" toc={true}>
  启用时，显示一个按钮，允许用户查看当前页面的原始 Markdown 源码。用户也可以[通过在页面 URL 后附加 `.md` 来查看 Markdown](/learn/docs/developer-tools/view-markdown)。要控制 Markdown 输出中出现的内容，请使用 [`<llms-ignore>`](/learn/docs/ai-features/customize-llm-output#content-for-humans-only) 标签。
</ParamField>

<ParamField path="page-actions.options.ask-ai" type="boolean" required={false} default="true" toc={true}>
  启用时，显示"Ask AI"按钮，允许用户使用 AI 驱动的助手询问有关页面内容的问题。
</ParamField>

<ParamField path="page-actions.options.chatgpt" type="boolean" required={false} default="true" toc={true}>
  控制"在 ChatGPT 中打开"按钮，将页面内容发送到 ChatGPT 进行进一步探索和问答。设置为 `false` 隐藏它。
</ParamField>

<ParamField path="page-actions.options.claude" type="boolean" required={false} default="true" toc={true}>
  控制"在 Claude 中打开"按钮，将页面内容发送到 Claude 进行进一步探索和问答。设置为 `false` 隐藏它。
</ParamField>

<ParamField path="page-actions.options.claude-code" type="boolean" required={false} default="true" toc={true}>
  控制"连接到 Claude Code"按钮，将 `claude mcp add` 命令复制到剪贴板，以便用户可以将文档站点的 [MCP 服务器](/learn/docs/ai-features/mcp-server)注册到 Claude Code。当启用 [Ask Fern](/learn/docs/ai-features/ask-fern/overview) 时默认显示。设置为 `false` 隐藏它。
</ParamField>

<ParamField path="page-actions.options.cursor" type="boolean" required={false} default="true" toc={true}>
  控制"连接到 Cursor"按钮，通过深度链接在 Cursor 中安装文档站点的 [MCP 服务器](/learn/docs/ai-features/mcp-server)。当启用 [Ask Fern](/learn/docs/ai-features/ask-fern/overview) 时默认显示。设置为 `false` 隐藏它。
</ParamField>
...
使用您自己的标题、图标和 URL 定义自定义页面操作按钮。自定义操作与内置页面操作一起出现，可以链接到外部工具、编辑器或任何 URL。
...
<ParamField path="page-actions.options.custom[].default" type="boolean" required={false} default="false" toc={true}>
  ...
  如果您在自定义操作上设置 `default: true`，请不要同时设置 [`page-actions.default`](/learn/docs/configuration/site-level-settings#page-actionsdefault) 以避免冲突。
</ParamField>

## "编辑此页面"配置

在每个文档页面底部添加"编辑此页面"链接，让读者轻松建议更改。在您的 `docs.yml` 文件中为每个实例配置它。
...
<Warning>
当使用 `launch: github` 时，GitHub 仓库必须是**公开的**，"编辑此页面"链接才能正常工作。使用 `launch: dashboard` 时，具有编辑权限的用户可以通过 Fern Editor 进行编辑，无论仓库可见性如何。
</Warning>
...
<ParamField path="github.branch" type="string" required={true} toc={true}>
  您希望 GitHub 编辑器向其开放 PR 的仓库分支。默认为 `main`。
</ParamField>

<ParamField path="launch" type="'github' | 'dashboard'" required={false} default="github" toc={true}>
  控制["编辑此页面"按钮](/learn/docs/user-feedback#edit-this-page)的行为。当设置为 `dashboard` 时，点击按钮会打开一个屏幕，用户可以选择为该页面启动 [Fern Editor](/learn/docs/writing-content/fern-editor) 会话或直接转到 GitHub 中的源码（最适合内部站点）。默认为 `github`，直接链接到 GitHub 上的文件（最适合面向公众的站点）。
</ParamField>

## 着陆页配置

设置一个专门的着陆页作为您的文档站点的入口点。
...
<ParamField path="slug" type="string" required={false} toc={true}>
  ...
  slug 也可以在着陆页 Markdown 文件的前言中覆盖。
</ParamField>

## SEO 元数据配置

配置站点范围的 Open Graph 和 Twitter Card 元数据，以控制您的文档在社交媒体预览和搜索结果中的显示方式。

<Note title="想要为单个页面设置元数据？">
 [在页面前言中使用 `keywords` 属性](/docs/configuration/page-level-settings#seo-metadata)。
</Note>
...
metadata:
  ...
  og:description: "使用 Square 的 API 和 SDK 套件进行构建。接受支付、管理库存、创建忠诚度计划并访问金融服务。为构建商务未来的开发者提供完整文档。"
  ...
  twitter:description: "使用 Square 的开发者平台将支付、销售点、库存和金融服务集成到您的应用中。开始使用我们的 API、SDK 和全面指南。"
